...
! variables
  ...
  real(dp) :: &
    ...
    eta_p(nmodes) = [ 1.0_dp, 0.1_dp ],    & ! polymer viscosities
    lambda(nmodes) = [ 1.e-8_dp, 1.e-10_dp ], & ! relaxation times
...
!    lambda, deltat, beta, rs_gup, is_gup, rs_c, is_c, U
...
! read mesh
  ...
  call read_mesh_sepran ( mesh, filename='sphere1.mout' )
...
!   write monitor data
    ...
    write(*,*) step, step*deltat, maxval(solc_np1(1,1)%u), &
      minval(solc_np1(1,1)%u)
