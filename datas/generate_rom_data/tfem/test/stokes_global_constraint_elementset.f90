...
! Physical quantities.
! integral p dA = 0 using a constraint on an elementset
...
! add elementset for integral p dA =0

  e1 = mesh%nelem-nx+1
  e2 = mesh%nelem
  e3 = 1
  call add_to_mesh ( mesh, elementset='elements', elements=[(i,i=e1,e2,e3)] )
...
! constraint for integral p dA = 0

  call define_constraint ( mesh, input_probdef, &
    physq=2, elementset1=1, nglobalc=1 )
...
! create system matrix
  ...
  call create_sysmatrix_structure_base ( sysmatrix, mesh, problem, symmetric=.true. )
...
! build (assemble) matrix and vector from elements
  ...
  call build_system_constraint ( mesh, problem, sysmatrix, rhsd, &
    constraint1=1, elemsub=stokes_zero_pressure_int_elementset, &
...
! create the structure oldvectors
  ...
  print *, sum(abs(velocity%u))/size(velocity%u), maxval(pressure%u), maxval(vorticity%u)
