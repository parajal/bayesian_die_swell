! Elastic problem on a unit cube. Mooney-Rivlin solid.
...
! constants
  ...
  integer, parameter :: &
    ...
    numinc=2,           & ! number of increments
    itermax=1            ! maximum interations in an increment
  ...
  real(dp), parameter :: &
    ...
    epsdd(numinc)=[(1e-5_dp,i=1,numinc-1),1e-15_dp], &
...
! problem definition
  ...
  call define_essential ( mesh, input_probdef, surfaces=[2,4], physq=1 )
...
! postprocessing, create and obtain vectors
  ...
  print *, 'max energy =',  maxval(energy%u)
  print *, 'max volumech =',  maxval(volumech%u)
