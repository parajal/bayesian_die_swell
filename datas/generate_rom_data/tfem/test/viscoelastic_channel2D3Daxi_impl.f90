...
! variables
  ...
  integer :: &
    nx=5,               & ! number of elements in z
    ny=5,               & ! number of elements in r
  ...
  real(dp) :: &
    oy = 2._dp,        & ! left lower corner in r-direction
...
!    logc, model, eta_s, eta_p, lambda, alphapar, deltat, beta, U, &
...
! Set velocity to a very small value to avoid divide by zero in SUPG
!  call fill_sysvector ( mesh, problem, sol, &
!    curve1=3, physq=physqvel, degfd=1, value=1e-20_dp )
...
!    filename='channel11.vtk' )
!
!  call write_vector_vtk ( mesh, problem, filename='channel11.vtk', &
...
!    filename='channel11.vtk', dataname='gammadot', append=.true. )
!
!  call write_tensor_vtk ( mesh, problem, filename='channel11.vtk', &
...
!
!  call write_tensor_vtk ( mesh, problemc, filename='channel11.vtk', &
