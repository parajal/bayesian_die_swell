...
!  ILUk
!  call set_solver_options ( so, printlevel=2, type_prec=2, &
...
! ILUt
  call set_solver_options ( so, printlevel=2, type_prec=2, &
    maxmvm=300, itsolver=8, mgmres=30, prec_store=6._dp, preconditioner=1, &
    droptol=3e-4_dp, fillin=4.0_dp, eps_rel = 1e-6_dp, eps_abs = 1.e-6_dp )
...
!   write to a vtk file for further post-processing
    ...
    call write_scalar_vtk ( mesh, problem, vector=pressure, &
      dataname='pressure', filename='cavity11_ma41_metis.vtk' )

    call write_vector_vtk ( mesh, problem, filename='cavity11_ma41_metis.vtk', &
      dataname='velocity_vector', sysvector=sol, append=.true. )

    call write_scalar_vtk ( mesh, problem, vector=dudz, &
      filename='cavity11_ma41_metis.vtk', dataname='dudz', append=.true. )
