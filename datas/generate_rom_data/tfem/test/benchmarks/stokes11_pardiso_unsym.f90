...
program stokes11_pardiso_unsym
...
!   write to a vtk file for further post-processing
    ...
    call write_scalar_vtk ( mesh, problem, vector=pressure, &
      dataname='pressure', filename='cavity11_pardiso_unsym.vtk' )

    call write_vector_vtk ( mesh, problem, &
      filename='cavity11_pardiso_unsym.vtk',&
      dataname='velocity_vector', sysvector=sol, append=.true. )

    call write_scalar_vtk ( mesh, problem, vector=dudz, &
      filename='cavity11_pardiso_unsym.vtk', dataname='dudz', append=.true. )
...
end program stokes11_pardiso_unsym
