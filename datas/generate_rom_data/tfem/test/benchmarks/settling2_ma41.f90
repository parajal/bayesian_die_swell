...
! constants
  ...
  logical :: use_refined_mesh = .false. ! normal mesh: 2 GBytes (MA41)
                                        ! refined mesh: 20 GBytes (MA41)
                                        !               and 64 bit integers
  real(dp), parameter :: &
    G     = 1.0_dp,    & ! modulus
...
! definitions
  ...
  real :: t_previous, t_current, t_build, t_solve
...
! Surface 6: cylinder wall
  ...
  if ( use_refined_mesh ) then
    call read_mesh_gmsh ( mesh, filename='meshp_3d_refined.msh', ndim=3 )
  else
    call read_mesh_gmsh ( mesh, filename='meshp_3d.msh', ndim=3 )
  end if
...
!       set to zero off-diagonal conformation blocks
      ...
      t_build = t_build + (t_current - t_previous )
...
!     solve
      ...
      call cpu_time ( t_current )
      t_solve = t_solve + (t_current - t_previous )
...
!   write max and mean values of conformation and b-tensor tensor to a file
  ...
  write ( *, '(a,f0.5,a)' ) 'Elapsed CPU time = ', t_build, ' in build '
  write ( *, '(a,f0.5,a)' ) 'Elapsed CPU time = ', t_solve, ' in solve '
