...
program steady5
  ...
  model = 3              ! 2: Oldroyd model 3: Giesekus
  G      = 10            ! modulus G
  lambda = 0.1           ! relaxation time
  mobility = 0.1         ! Mobility parameter for Giesekus
  tau_y  = 0.3           ! tau_y
  maxnumiterations = 100 ! maximum number of iterations
  start_rate = 0.01     ! start data point
  end_rate = 1           ! end data point
...
!  open ( unit=14, file='steady5.out', recl=300 )
...
! initialize c, depending on flow type
  ...
  if ( alam_model == 2 ) then
    ...
    case(1) ! shear
      do mode = 1, nmodes
        c(1,:,mode) = [ 1, 0, 1 ] + &
                      [ 0, 2, 0 ]*tau_y(mode)/G(mode)
      end do
    case(2) ! planar extension
      do mode = 1, nmodes
        c(1,:,mode) = [ 1, 0, 1 ] + &
                      [ 2, 0, 0 ]*tau_y(mode)/G(mode)
      end do
    case(3) ! combined
      do mode = 1, nmodes
        c(1,:,mode) = [ 1, 0, 1 ] + &
...
!   stress tensor
    ...
    if ( nmodes == 1 ) then
      ...
      if ( includesolvent) then
        write(*,'(9es16.8,1x,i0)') rate, tau + tau_s(L), vonmises_2D(tau)
      else
        write(*,'(9es16.8,1x,i0)') rate, tau, vonmises_2D(tau)
      end if
