...
! definitions
  ...
  type(sysvector_t), dimension(ncompc,nmodes), target :: solcn, solcnm1, rhsc
  type(lu_ma57_t) :: lu_u
  type(lu_ma41_t) :: luc
...
! variables
  ...
  integer :: &
    ...
    numtimesteps = 6,  & ! number of time steps
...
! create the structure oldvectors_ve
  ...
  call create_oldvectors ( oldvectors_ve, nsysvec=2, nprob=2, nsysvec2=2 )
...
! time stepping
  ...
  do step = 1, numtimesteps
    ...
    if ( step == 4 ) then
!     change time integration scheme
...
!   build (assemble) matrix and vector for conformation problem
    ...
    call build_system ( mesh, problemc, sysmatrixc, m2sysvector=rhsc, &
      ...
      coefficients=coefficients )
    call check_filled_sysmatrix (sysmatrixc)
...
!  close(unit=13)
  ...
  print *, maxval( abs(solcn(1,1)%u) ), maxval( abs(soln%u(vely%s)) )
