...
! functions module for diffusion7

module functions_df7_m
...
! constants
  ...
  integer, parameter :: &
    ...
    nx=11,              & ! number of elements in x
    ny=11,              & ! number of elements in y
    ...
    gamma_weights=0       ! how to compute the weights (gammaw):
                          ! 0: equally (gammaw=0.5)
                          ! 1: fully on the first layer (gammaw=0)
                          ! 2: fully on the second layer (gammaw=1)
                          ! 3: non-equally
  ...
  real(dp), parameter :: &
    ...
    gamma_par = 0.5_dp, & ! weight factor gammaw in case of gamma_weights=3
    ...
    dx = 0.61, dy = 1e-2, & ! displacement of the cylinder relative to size of
                            !! an element
    ...
    cjump = 1._dp, & ! imposed scalar jump (ibar) on the cylinder if
                     ! coefficients%i(45) = 0
    fjump = 2._dp, & ! imposed scalar flux jump (jbar) on the cylinder if
                     ! coefficients%i(47) = 0
    epsvol = 1e-4_dp, & ! elements with integration area smaller are removed
  ...
  logical, parameter :: Nitsche = .false., transposed = .true., &
    symmetric = .true., embedded = .true., Baumann_Oden = .false.
...
! set gammaw for the weighting between the two sides of the interface

  select case (gamma_weights)
  case(0)
    gammaw = 0.5_dp
  case(1)
    gammaw = 0
...
! fill coefficients
  ...
  coefficients%i(45) = 0   ! -1: impose zero ibar, 0: impose non-zero
                           ! ibar cjump
...
! problem definition
  ...
  call define_essential ( mesh, input_probdef, curve1=1, curve2=4, layer=1 )
...
! ================================
  ...
  coefficients%i(49) = 1 ! outside (layer=1)
...
! quantities have been defined).
  ...
  call build_system ( mesh, problem, sysmatrix, rhsd, &
    ...
    factormat=1-gammaw, factorvec=gammaw, &
...
! transposed term from the outside
  ...
  if ( transposed ) then
    ...
    call build_system ( mesh, problem, sysmatrix, rhsd, &
      ...
      oldvectors=oldvectors, coefficients=coefficients, &
      factormat=1-gammaw, factorvec=1-gammaw, &
      addmatvec=.true., elementset=1, order='DN' )
...
! stabilizing term from the outside (layer=1)
  ...
  if ( embedded ) then
    ...
    call build_system ( mesh, problem, sysmatrix, rhsd, &
      ...
      oldvectors=oldvectors, coefficients=coefficients, &
      factormat=(1-gammaw)**2, factorvec=(1-gammaw)**2, &
      addmatvec=.true., elementset=1, order='DN' )
...
! ================================
  ...
  coefficients%i(49) = 2 ! inside (layer=2)
...
! flux at the interface
  ...
  call build_system ( mesh, problem, sysmatrix, rhsd, &
    elemsub=scalar_diffusion_flux_interface_eltree, &
    oldvectors=oldvectors, coefficients=coefficients, &
    factormat=gammaw, factorvec=1-gammaw, &
    addmatvec=.true., elementset=1, order='DN' )
...
!   NOTE minus sign for rhs (important only if cjump /= 0).
    ...
    call build_system ( mesh, problem, sysmatrix, rhsd, &
      elemsub=scalar_diffusion_flux_interface_transposed_eltree, &
      oldvectors=oldvectors, coefficients=coefficients, &
      factormat=gammaw, factorvec=-gammaw, &
      addmatvec=.true., elementset=1, order='DN' )
...
! stabilizing term from the inside (layer=2)
  ...
  if ( embedded ) then
    ...
    call build_system ( mesh, problem, sysmatrix, rhsd, &
      elemsub=scalar_diffusion_embedded_interface_eltree, &
      oldvectors=oldvectors, coefficients=coefficients, &
      factormat=gammaw**2, factorvec=gammaw**2, &
      addmatvec=.true., elementset=1, order='DN' )
