! Sequence of steady states of flow around a cylinder for a b-representation of
! an incompressible EGP model.
...
!  type(oldvectors_t) :: oldvectors, oldvectors_ve
  ...
  integer :: step, iter, m, i, nsteps(ndU), numsteps, j, nn, mode1, mode2
...
! fill coefficients
  ...
  if ( alam_model == 4 ) then
    coefficients%r(501:500+(2+nn)*nmodes) = &
              [ ( G/nmodes, lambda, tau_ref, m=1,nmodes ) ]
  else if ( alam_model == 5 ) then
    coefficients%r(501:500+(2+nn)*nmodes) = &
...
!     set to zero off-diagonal blocks gradient-b and pressure-b
      call build_b_zero

!     build constraints for b
      call build_b_constraint
...
!   global von Mises
  ...
  print *, 'maxval(vonmises%u)', maxval(vonmises%u)
...
contains
  ...
  subroutine fill_offdiag_physqmask
    ...
    if ( nmodes > 1 ) then
      do i = 1, nmodes
        do j = 1, nmodes
          if ( (i<msplit.and.j<msplit) .or. (i>=msplit.and.j>=msplit) ) cycle
...
!     set to zero off-diagonal blocks pressure-b
      call build_system ( mesh, problem, sysmatrix, rhsd, addmatvec=.true., &
        physqrow=[physqpress], physqcol=[(physqb+m-1,m=1,nmodes)], &
        buildvector=.false., zeromatvec=.true. )
      call build_system ( mesh, problem, sysmatrix, rhsd, addmatvec=.true., &
        physqrow=[(physqb+m-1,m=1,nmodes)], physqcol=[physqpress], &
        buildvector=.false., zeromatvec=.true. )
    end if

  end subroutine build_b_zero


  subroutine build_b_constraint

!   periodical condition on conformation tensor
    call build_system_constraint ( mesh, problem, sysmatrix, &
      sysvector=rhsd, constraint1=4, constraint2=3+nmodes, &
      elemsub=stokes_constr_node_conn, addmatvec=.true. )

  end subroutine build_b_constraint
