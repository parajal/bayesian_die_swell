...
! inside fluid
  ...
  call build_system ( mesh, problem, sysmatrix, rhsd, &
    elemsub=stokes_elem, oldvectors=oldvectors, addmatvec=.true., &
...
! sample and plot velocity on mesh_plot
  ...
  print *, sum(abs(pressure%u))/size(pressure%u), &
           sum(abs(velocity%u))/size(velocity%u), &
           sum(abs(vorticity%u))/size(vorticity%u)
