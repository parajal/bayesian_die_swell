...
! variables
  ...
  real(dp) :: &
    ...
    rs_c  = 2.0_dp,    & ! real_storage for the conformation LU (HSL)
    is_c  = 2.0_dp       ! integer_storage for conformation LU (HSL)
...
! velocities
  call define_constraint ( mesh, input_probdef, &
    physq=physqvel, surface1=3, surface2=5, discretization='collocation', &
      excludecurves=[2,6,7,10] )

! gradients
  call define_constraint ( mesh, input_probdef, &
    physq=physqgrad, surface1=3, surface2=5, discretization='collocation' )
...
! constraint for periodical boundary conditions of the conformation
  call define_constraint ( mesh, input_probdefc, surface1=3, surface2=5, &
...
! create the structure oldvectors_ve
  ...
  call create_oldvectors ( oldvectors_ve, nsysvec=1, nprob=2, nsysvec2=1 )
