...
! variables
  ...
  so%printlevel = 0    ! print level for MUMPS
  so%ordering   = 7
...
! define constraints on the object
  ...
  call toc1 ( mumps_par%myid, 'problem definition' )
...
! create an instance of
  ...
  call toc1 ( mumps_par%myid, 'vectors' )
...
!     update reference coordinates of object
      ...
      call toc1 ( mumps_par%myid, 'find_refcoor_objects' )
...
!   create system matrix
    ...
    call toc1 ( mumps_par%myid, 'create_sysmatrix' )
...
!   build (assemble) matrix and vector from elements
    ...
    call toc1 ( mumps_par%myid, 'build', delta )
    ...
    call solve_system_mumps ( sysmatrix, rhsd, sol, mumps_par, so )

    call delete ( mumps_par )
    call create ( mumps_par )

    call toc1 ( mumps_par%myid, 'solve', delta )
...
contains
  ...
  subroutine toc1 ( proc, string, delta )
...
!   if delta is present: the incremental time
    ...
    if ( present(string) ) then
      if ( present(delta) ) then
        call toc ( string, delta )
      else
        call toc ( string )
      end if
    else
      if ( present(delta) ) then
        call toc ( delta=delta )
      else
        call toc
      end if
    end if
