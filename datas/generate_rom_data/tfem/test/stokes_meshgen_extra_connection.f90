...
! Non-conforming coupling of two domains (vertical velocity)
! Connection of two domains (horizontal velocity, slip layer)
...
!     zero block for first velocity componet

      elemmat(1:ndflb,1:ndf) = 0
      elemmat2(1:ndflb,1:ndf) = 0

!     second velocity component

      do i = 1, ndflb
        do j = 1, ndf
          elemmat(i,ndf+j) = psi(1,i) * phi1(1,j) * &
                                   curvel(1)*mesh%objects(object)%wg(node)
          elemmat2(i,ndf+j) = - psi(1,i) * phi2(1,j) * &
                                   curvel(1)*mesh%objects(object)%wg(node)
        end do
      end do
...
! connection element for horizontal velocity
  ...
  subroutine elementcn ( mesh, problem, conn, elem, node, matrix, vector, &
    ...
    integer :: object, i, j
    real(dp) :: phi1(1,ndf), phi2(1,ndf), xr(1,2), x(nodalpb,2)
    real(dp) :: theta(1,ndfb), dtheta(1,ndfb,1), dxdxi(1,2), curvel(1)
    real(dp) :: etaslip
...
!     first velocity component only
      ...
      do i = 1, ndf
        do j = 1, ndf
          elemmat11(i,j) = etaslip * phi1(1,i) * phi1(1,j) * &
                                   curvel(1)*mesh%objects(object)%wg(node)
          elemmat12(i,j) = - etaslip * phi1(1,i) * phi2(1,j) * &
                                   curvel(1)*mesh%objects(object)%wg(node)
          elemmat22(i,j) = etaslip * phi2(1,i) * phi2(1,j) * &
                                   curvel(1)*mesh%objects(object)%wg(node)
        end do
      end do
...
!    pintpl = 4,         & ! Q1 pressures
    ...
    nx1=4,              & ! number of elements in x (lower domain)
    ny1=4,              & ! number of elements in y (lower domain)
...
!   linear Lagrange multiplier
  ...
  call define_connection ( mesh, input_probdef, object=1, &
