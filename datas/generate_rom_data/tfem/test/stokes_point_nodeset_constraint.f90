...
! create mesh
  ...
  call add_to_mesh ( mesh, nodeset='nodes', nodes=mesh%curves(2)%nodes(3:2*ny) )
  call add_to_mesh ( mesh, nodeset='nodes', nodes=mesh%curves(5)%nodes(3:2*ny) )
  call add_to_mesh ( mesh, point=[0._dp,1._dp/ny/2] ) ! P5
  call add_to_mesh ( mesh, point=[1._dp,1._dp/ny/2] ) ! P6
...
! write mesh (read by streamfunction computation)
  ...
  !call plot_points_curves ( plot_options, mesh, filename='mesh.fig' )
...
!    physq=1, curve1=2, curve2=5, discretization='collocation', exclude=3 )
  call define_constraint ( mesh, input_probdef, &
    physq=1, nodeset1=1, nodeset2=2, discretization='collocation' )
  call define_constraint ( mesh, input_probdef, &
    physq=1, point1=5, point2=6, discretization='collocation' )
...
! build (assemble) matrix and vector from elements
  ...
  call build_system_constraint ( mesh, problem, sysmatrix, rhsd, &
    constraint1=2, constraint2=3, elemsub=stokes_constr_node_conn, addmatvec=.true., &
    coefficients=coefficients  )
