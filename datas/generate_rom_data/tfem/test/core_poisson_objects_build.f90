...
! Internal element routine for the Poisson Equation
  ...
  subroutine poisson_elem ( mesh, problem, elgrp, elem, matrix, vector, &
    ...
    integer :: i, j, ip
    real(dp), allocatable, dimension(:), save :: wg, fg, detF, work
    real(dp), allocatable, dimension(:,:), save :: xig, phi, x, xg
...
!     first element in this group
      ...
      allocate ( xig(ninti,ndim), phi(ninti,ndf), x(nodalp,ndim), work(ninti) )
...
!      end do
      ...
      do i = 1, ndf
        do j = i, ndf
          do ip = 1, ninti
            work(ip) = dot_product ( dphidx(ip,i,:), dphidx(ip,j,:) )
          end do
...
! Internal element routine for the Poisson Equation on an object

  subroutine poisson_elemo ( mesh, problem, ei, matrix, vector, &
    first, last, coefficients, oldvectors, elemmat, elemvec )
    ...
    integer :: i, j, elgrp, elem, object, elemo, nintps, ip

    real(dp) :: detF(ei%nintps), work(ei%nintps)
    real(dp) :: xr(ei%nintps,ndim), phi(ei%nintps,ndf), x(nodalp,ndim)
    real(dp) :: dphi(ei%nintps,ndf,ndim), F(ei%nintps,ndim,ndim)
    real(dp) :: Finv(ei%nintps,ndim,ndim), dphidx(ei%nintps,ndf,ndim)
    real(dp) :: theta(ei%nintps,ndf), dtheta(ei%nintps,ndf,ndim)
...
!   shape function in object
    ...
    xr = mesh%objects(object)%xig(ei%intps,:)
    call shape_quad_Q2 ( xr, theta, dtheta )
    ...
    if ( matrix ) then
      ...
      do i = 1, ndf
        do j = i, ndf
          do ip = 1, nintps
            work(ip) = 10 * dot_product ( dphidx(ip,i,:), dphidx(ip,j,:) )
          end do
          elemmat(i,j) = sum ( work * detF * mesh%objects(object)%wg(ei%intps) )
          elemmat(j,i) = elemmat(i,j) ! symmetry
        end do
      end do
...
!              (/4,2/) )
  ...
  call set_mesh_options ( mesh_options, elshape=6, nx=1, ny=1, &
...
!  call finalize_sysmatrix_structure ( sysmatrix )
  ...
  call build_system ( mesh, problem, sysmatrix, rhsd, &
    object=1, elemsub1=poisson_elemo, addmatvec=.true. )
  call build_system ( mesh, problem, sysmatrix, rhsd, &
    object=2, elemsub1=poisson_elemo, addmatvec=.true. )
...
!  plot_options%minvalue=-10
  ...
  !call plot_mesh ( plot_options, mesh, 'sol.fig', append=.true. )
...
!  print *, sample%coor
  ...
  call fill_sample ( mesh, problem, sample, ndegfd=1, object=2, &
    elemsub=sample_node )
