...
! Choose either first of second-order time integration for the pressure
! constitutive equation. Choice handled on element level.

program compressible_fluid5
...
! constants
  ...
  integer, parameter :: &
    ...
    timeint = 2,        & ! time integration with convection prediction:
...
!    ncy=2*ny+1            ! number of sample points in y
  ...
  real(dp), parameter :: &
    Lx = 2._dp, Ly = 1.0_dp,  & ! size of the domain
    eta = 1._dp,  & ! viscosity
...
!  type(plot_options_t) :: plot_options
  ...
  type(vector_t), target :: meshveln, meshvelnm1
...
! fill coefficients
  ...
  coefficients%i(301) = 1 ! start with first order time integration
  coefficients%i(302) = 2 ! position sol_n in oldvectors
  coefficients%i(304) = 1 ! compute convection prediction on element level
...
! create system vectors (solution and right-hand side)
  ...
  call create_sysvector ( problem, sol, sol_n, sol_nm1 )
...
! create mesh velocity vectors
  call create ( problem, meshveln, vec=physqvel )
  meshveln%u(meshvelx%s) = U
  meshveln%u(meshvely%s) = 0
  call copy ( meshveln, meshvelnm1)

! create the structure oldvectors

  call create_oldvectors ( oldvectors, nsysvec=3, nvec=2 )

  oldvectors%s(1)%p => sol
  oldvectors%s(2)%p => sol_n
  oldvectors%s(3)%p => sol_nm1
  oldvectors%v(1)%p => meshveln
  oldvectors%v(2)%p => meshvelnm1
...
! time stepping
  ...
  do step = 1, numtimesteps
    ...
    if ( timeint == 2 .and. step > 1 ) &
                   coefficients%i(301) = 2 ! second-order time integration
...
!
  ...
  print *, 'sum(abs(velocity%u))/velocity%n  ', sum(abs(velocity%u))/velocity%n
  print *, 'sum(abs(pressure%u))/pressure%n  ', sum(abs(pressure%u))/pressure%n
  print *, 'sum(abs(vorticity%u))/vorticity%n', sum(abs(vorticity%u))/vorticity%n
  print *, 'sum(abs(divu%u))/divu%           ', sum(abs(divu%u))/divu%n
...
end program compressible_fluid5
