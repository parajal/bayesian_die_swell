...
! Dirichlet boundary conditions
  ...
  call define_essential ( mesh, input_probdef, curves=[1,3], physq=physqvel )
...
! set velocity = -U/2 on lower boundary
  call fill_sysvector ( mesh, problem, sol, &
    curve1=1, physq=physqvel, degsfd=[1,2], vvalue=[-U/2,0._dp] )
! set velocity_x = U/2 on upper boundary
  call fill_sysvector ( mesh, problem, sol, &
    curve1=3, physq=physqvel, degsfd=[1,2], vvalue=[U/2,0._dp] )
...
end program diffuse_interface1
