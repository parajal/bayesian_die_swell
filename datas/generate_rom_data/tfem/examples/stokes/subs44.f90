...
! element for connection: slip on the particle boundary
  ...
  subroutine elementcn ( mesh, problem, conn, elem, node, matrix, vector, &
    ...
    real(dp) :: phi1(ninti,ndf), phi2(ninti,ndf), xr(ninti,1), x(nodalp,ndim)
    real(dp) :: dphi(ninti,ndf,1), normal(ninti,ndim), xg(ninti,ndim), wg(ninti)
...
!     first velocity component only
      ...
      do i = 1, ndf
        do j = 1, ndf
          elemmat11(i,j) = &
            etaslip * sum ( phi1(:,i) * phi1(:,j) * curvel * Inn(:,1,1) * wg )
          elemmat11(i,j+ndf) = &
            etaslip * sum ( phi1(:,i) * phi1(:,j) * curvel * Inn(:,1,2) * wg )
          elemmat11(i+ndf,j) = &
            etaslip * sum ( phi1(:,i) * phi1(:,j) * curvel * Inn(:,2,1) * wg )
          elemmat11(i+ndf,j+ndf) = &
            etaslip * sum ( phi1(:,i) * phi1(:,j) * curvel * Inn(:,2,2) * wg )

          elemmat12(i,j) = &
            -etaslip * sum ( phi1(:,i) * phi2(:,j) * curvel * Inn(:,1,1) * wg )
          elemmat12(i,j+ndf) = &
            -etaslip * sum ( phi1(:,i) * phi2(:,j) * curvel * Inn(:,1,2) * wg )
          elemmat12(i+ndf,j) = &
            -etaslip * sum ( phi1(:,i) * phi2(:,j) * curvel * Inn(:,2,1) * wg )
          elemmat12(i+ndf,j+ndf) = &
            -etaslip * sum ( phi1(:,i) * phi2(:,j) * curvel * Inn(:,2,2) * wg )

          elemmat22(i,j) = &
            etaslip * sum ( phi2(:,i) * phi2(:,j) * curvel * Inn(:,1,1) * wg )
          elemmat22(i,j+ndf) = &
            etaslip * sum ( phi2(:,i) * phi2(:,j) * curvel * Inn(:,1,2) * wg )
          elemmat22(i+ndf,j) = &
            etaslip * sum ( phi2(:,i) * phi2(:,j) * curvel * Inn(:,2,1) * wg )
          elemmat22(i+ndf,j+ndf) = &
            etaslip * sum ( phi2(:,i) * phi2(:,j) * curvel * Inn(:,2,2) * wg )
        end do
      end do
...
! element for normal velocity coupling
  ...
  subroutine stokes_constr_normal_vel ( mesh, problem, constr, elem, &
    ...
    integer :: i, j, curve, coorsys
    real(dp) :: phi(ninti,ndf), psi(ninti,ndfl), xr(ninti,1), x(nodalp,ndim)
    real(dp) :: dphi(ninti,ndf,1), normal(ninti,ndim), xg(ninti,ndim), wg(ninti)
