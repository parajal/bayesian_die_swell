...
! Physical quantities.
! This is the stokes1 problem with the following changes:
!  1) Q1isoQ2 shape functions
!  2) subdomain Gauss integration (using 4 subdomains)
...
! constants
  ...
  integer, parameter :: &
    ...
    gauss = 2,          & ! 2x2 integration of sub quads
...
! create mesh
  ...
  meshgen_options%elshape = 6 ! use elshape=6 to create the mesh
  meshgen_options%nx = nx
  meshgen_options%ny = ny

  call quadrilateral2d ( mesh, meshgen_options )

  mesh%element(:)%elshape = 34 ! change elshape to 34 for isoparametric mapping
...
! build (assemble) matrix and vector from elements
  ...
  call build_system ( mesh, problem, sysmatrix, rhsd, elemsub=stokes_elem, &
    order='DN', coefficients=coefficients )
