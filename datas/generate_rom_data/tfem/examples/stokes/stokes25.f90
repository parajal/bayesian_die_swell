...
! Physical quantities.
! Identical to stokes1 but now using a user Gauss rule.

module generalized_stokes_usergauss_m
...
!   ninti. NOTE: gauss%globalshape already set

    gauss%intrule = 4  ! 4x4 gauss
    gauss%nsubint = 1  ! no macro
...
program stokes25
  ...
  use generalized_stokes_usergauss_m
  use io_utils_m
  use figplot_m
...
! fill coefficients
  ...
  coefficients%i(1:11) = &
    [ uintpl,   pintpl,     0,     0,         0,  &
      ...
      gauss ]
  coefficients%i(12:) = 0
  coefficients%i(37) = 2  ! user gauss routines
