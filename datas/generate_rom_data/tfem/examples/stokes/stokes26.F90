...
! Physical quantities.
! Iterative solver with block preconditioning (using pressure mass matrix).
...
! build (assemble) matrix from elements for pressure mass matrix
  ...
  call build_system ( mesh, problemp, sysmatrix_S, elemsub=pressure_mass_elem, &
    coefficients=coefficients, factormat=factor/eta, buildvector=.false. )
