...
! Similar to stokes2, but now using nodesets instead of curves for the constraint.
...
! create mesh
  ...
  call add_to_mesh ( mesh, nodeset='nodes', nodes=mesh%curves(2)%nodes(2:2*ny) ) ! exclude the end points
  call add_to_mesh ( mesh, nodeset='nodes', nodes=mesh%curves(5)%nodes(2:2*ny) ) ! exclude the end points
