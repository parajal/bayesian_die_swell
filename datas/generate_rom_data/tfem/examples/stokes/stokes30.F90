...
! Stokes+mass problem on a 3D square cavity. Only half of the domain is solved
! due to symmetry conditions.
! Iterative solver with block preconditioning (using both pressure mass and
! diffusion matrix). M_p^{-1}F_pA_p^{-1} form for inverse Schur complement.
...
! create system matrices M_p and A_p
...
! build (assemble) matrix from elements for pressure mass and diffusion matrix
  ...
  call copy ( sysmatrix_Mp, sysmatrix_Fp )

! F_p = a M_p + eta * A_p

  sysmatrix_Fp%Suu%a = factor_a * sysmatrix_Mp%Suu%a + eta * sysmatrix_Ap%Suu%a
  ...
  if ( second_solve ) then
    ...
    call solve_system3_mi20 ( sysmatrix, rhsd, sol, ssv%s, ssp%s,  &
      sysmatrix_Mp, sysmatrix_Fp, sysmatrix_Ap, prec=prec, &
