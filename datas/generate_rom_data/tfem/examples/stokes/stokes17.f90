...
! Weak constraint on the particle boundary.
! Use of subscript to get constraint data (lagrangian multipliers and
! additional unknowns).
...
! definitions
  ...
  type(subscriptcon_t) :: scon

  integer :: i, elem
  real(dp) :: up(3), deltat, unm1(3), un(3), pp(3), time, x(2)
...
! create mesh for particle boundary
  ...
  call mesh_skeleton ( mesh_particle, nnodes=40, nelem=20, elshape=2, ndim=2 )
...
! subscript for additional unknowns

  call create ( mesh, problem, scon, constraints=[1], &
    lagrangian_multipliers=.false., fillnodes=.true. )
...
!   extract info on particle
    ...
    up = sol%u(scon%s) ! velocity and rotation rate
...
! write lagrangian multipliers

  call create ( mesh, problem, scon, constraints=[1], &
    addunknowns=.false., fillnodes=.true. )

  open ( unit=11, file='lm.out' )

  do i = 1, size(scon%nodes)
    x = mesh%objects(1)%coor(scon%nodes(i),:)
    write(11,*) x, sol%u(scon%s(2*i-1)), sol%u(scon%s(2*i))
  end do
