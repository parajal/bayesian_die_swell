...
! One-sided connected boundary conditions (collocation)
! Channel flow.
! The velocities on the inflow are set to the velocities on the outflow using
! a connection.
...
! problem definition
  ...
  call define_connection ( mesh, input_probdef, curve1=2, curve2=5, &
    discretization='collocation', physq=1 )
...
! subscript for velocity degrees on curve 5.

  call create ( mesh, problem, cv5, physqarr=[1], curves=[5] )
...
! clear rows in matrix and rhsd of degrees of freedom on curve 5

  call clear_rows ( sysmatrix, cv5%s )
  call clear_rows ( rhsd, cv5%s )

! fill rows of degrees of freedom on curve 5 for equality to curve 2
