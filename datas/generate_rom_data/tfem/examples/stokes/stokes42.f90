! Stokes problem on a rectangular domain with a disk at the center.
...
! read mesh from gmsh output file
  call read_mesh_gmsh ( mesh_ext, filename='mesh42.msh', ndim=2, &
    physgeom=.true. )

! create particle mesh from curve=5
  call mesh_convert ( mesh_ext, mesh_int, from_curve=5 )
...
! define constraint on the disk: rigid-body motion
  call define_constraint ( mesh, input_probdef, curve1=6, physq=physqvel, &
...
! normal velocity coupling
  call define_constraint ( mesh, input_probdef, physq=physqvel, &
    curve1=5, curve2=6, discretization='weak', elementdof=[1,1,1] )

! define connection: slip
  call define_connection ( mesh, input_probdef, curve1=5, curve2=6, &
...
! coupling of normal velocities
  ...
  call build_system_constraint ( mesh, problem, sysmatrix, rhsd, &
    constraint1=2, elemsub=stokes_constr_normal_vel, coefficients=coefficients,&
    addmatvec=.true. )
...
! get velocity
  ...
  write ( *, '(1X,A,3F10.6)' ) 'velocity = ', up(1:2)
  write ( *, '(1X,A,3F10.6)' ) 'omega = ', up(3)
