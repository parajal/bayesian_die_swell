! 3D Stokes problem on a cubic domain with a spherical shaped drop
...
! Similar to xfem16 but now 3D.

! functions module for xfem17
...
! integration volume near interface elements  (on the positive side)
...
!   intrule_sub_small: the integration rule for small integration volumess
...
! constants
  ...
  integer, parameter :: &
    ...
    gauss = 3,          & ! 3x3x3 Gauss integration
...
!                         ! center position of the sphere
    !center(3) = [ dx*lx/nx, dy*ly/ny, dz*lz/nz ], &
    center(3) = [ 0._dp, 1._dp, 1._dp ], &
...
! object for sampling/plotting the sphere
...
! nodesets for the layers
  ...
  call plot_points_curves ( plot_options, mesh, 'curves_xf17.fig' )

  call plot_mesh ( plot_options, mesh, 'mesh_xf17.fig', surfaces=[3,4,6] )
...
! problem definition
  ...
  call define_essential ( mesh, input_probdef, surface1=1, surface2=2, &
    physq=1, layer=1 )
  call define_essential ( mesh, input_probdef, surface1=4, physq=1, layer=1 )
  call define_essential ( mesh, input_probdef, surface1=6, physq=1, layer=1 )
  call define_essential ( mesh, input_probdef, point=1, physq=2, layer=1 )

! constraint for flow rate

  call define_constraint ( mesh, input_probdef, &
    physq=1, surface1=5, nglobalc=1, layer=1 )
...
! velocities
  call define_constraint ( mesh, input_probdef, &
    ...
    excludecurves=[2,6,7,10], layer=1 )
...
! build missing matrix elements with zero
  ...
  if ( symmetric ) then
    solver_options_ma57%real_storage=1.2
    call solve_system_ma57 ( sysmatrix, rhsd, sol, solver_options=solver_options_ma57 )
  else
...
! sample pressure on sphere (circle)

  call sample_pressure_on_circle ( layer=1, obj=1, &
    filename='sample_p_out_xf17.out' )
  call sample_pressure_on_circle ( layer=2, obj=1, &
    filename='sample_p_in_xf17.out' )

! sample velocity on sphere (circle)

  call sample_velocity_on_circle ( layer=1, obj=1, &
    filename='sample_v_out_xf17.out' )
  call sample_velocity_on_circle ( layer=2, obj=1, &
    filename='sample_v_in_xf17.out' )

! sample dudy on sphere (circle)

  call sample_dudy_on_circle ( layer=1, obj=1, &
    filename='sample_dudy_out_xf17.out' )
  call sample_dudy_on_circle ( layer=2, obj=1, &
    filename='sample_dudy_in_xf17.out' )
...
! post-processing: create mesh_plot for plotting
  ...
  call plot_mesh ( plot_options, mesh_plot, 'mesh_plot_xf17.fig' )
...
!   loop all elements
  ...
  subroutine sample_pressure_on_circle ( layer, obj, filename )
...
!   sample on sphere (errorobject=1 to warn for points not in layer)
  ...
  end subroutine sample_pressure_on_circle

  subroutine sample_velocity_on_circle ( layer, obj, filename )
...
!   sample on sphere (errorobject=1 to warn for points not in layer)
    ...
    call fill_sample ( mesh, problem, sample_v, ndegfd=3, object=obj, &
  ...
  end subroutine sample_velocity_on_circle

  subroutine sample_dudy_on_circle ( layer, obj, filename )
...
!   sample on sphere (errorobject=1 to warn for points not in layer)
    ...
    call fill_sample ( mesh, problem, sample_dudy, ndegfd=1, object=obj, &
  ...
  end subroutine sample_dudy_on_circle
...
!   sample outside (use layer as argument to detect points not in layer)
    ...
    call fill_sample ( mesh, problem, sample_v, ndegfd=3, object=plotobj1, &
...
!   sample inside (use layer as argument to detect points not in layer)
    ...
    call fill_sample ( mesh, problem, sample_v, ndegfd=3, object=plotobj2, &
      elemsub=stokes_sample_velocity, coefficients=coefficients, &
      oldvectors=oldvectors, layer=2, errorobject=1, allnodes=.true. )

    velocity%u(n+1:) = reshape ( transpose(sample_v%u), [3*sample_v%nnodes] )

  end subroutine sample_velocity

! sample dudy on mesh_plot

  subroutine sample_dudy_plot
...
!   sample outside (use layer as argument to detect points not in layer)
    ...
    call fill_sample ( mesh, problem, sample_dudy, ndegfd=1, object=plotobj1, &
      elemsub=stokes_sample_deriv, coefficients=coefficients, &
      oldvectors=oldvectors, layer=1, errorobject=1, allnodes=.true. )

    n = sample_dudy%nnodes

    dudy%u(1:n) = sample_dudy%u(1:n,1)

!   sample inside (use layer as argument to detect points not in layer)

    coefficients%i(38) = 2

    call fill_sample ( mesh, problem, sample_dudy, ndegfd=1, object=plotobj2, &
      elemsub=stokes_sample_deriv, coefficients=coefficients, &
      oldvectors=oldvectors, layer=2, errorobject=1, allnodes=.true. )

    dudy%u(n+1:) = sample_dudy%u(1:sample_dudy%nnodes,1)

  end subroutine sample_dudy_plot
