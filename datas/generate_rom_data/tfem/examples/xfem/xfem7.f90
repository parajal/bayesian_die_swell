...
! functions module for xfem7

module functions_xf7_m
...
!     composite integration scheme for levelset >= 0 or levelset <= 0
      ...
      if ( intregion == 1 ) then
!       outside region
        call integration_points ( eltree_array(elem)%p, xig, wg, lsign=[1], ninti=ninti_e, &
...
!       outside region
        call eltree_to_mesh ( eltree_array(elem)%p, elmesh, lsign=[1], mapcoor=mapcoor )
...
! blocks for fast searching of mesh intersections
  ...
  call plot_points_curves ( plot_options, mesh, 'curves_xf7.fig' )

  if ( plot_nodesets ) then

    call plot_mesh_nodeset ( nodeset=1, filename='mesh_outside_xf7.fig' )
    call plot_mesh_nodeset ( nodeset=2, filename='mesh_inside_xf7.fig' )

  else

    call plot_mesh ( plot_options, mesh, 'mesh_xf7.fig' )
...
! post-processing: create mesh_plot for plotting
  ...
  call plot_mesh ( plot_options, mesh_plot, 'mesh_plot_xf7.fig' )
...
! sample and plot pressure on mesh_plot
  ...
  call plot_contour_on_mesh_plot ( filename='pressure_xf7.fig', vector=pressure )

! sample and plot vorticity on mesh_plot

  call sample_vorticity
  call plot_contour_on_mesh_plot ( filename='vorticity_xf7.fig', vector=vorticity )

! sample and plot velocity on mesh_plot

  call sample_velocity
  call plot_contour_on_mesh_plot ( filename='velocity_x_xf7.fig', &
    vector=velocity, degfd=1 )
  call plot_contour_on_mesh_plot ( filename='velocity_y_xf7.fig', &
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh_plot, problem_plot, vector=pressure, &
    dataname='pressure', filename='drop_in_flow_xf7.vtk' )

  call write_vector_vtk ( mesh_plot, problem_plot, &
    filename='drop_in_flow_xf7.vtk', &
...
!   post-processing: plot in a layer
    ...
    call plot_in_layer ( 'velocity_xf7_layer1.fig', &
      'pressure_contour_xf7_layer1.fig', &
      'vorticity_contour_xf7_layer1.fig', layer=1 )
    call plot_in_layer ( 'velocity_xf7_layer2.fig', &
      'pressure_contour_xf7_layer2.fig', &
      'vorticity_contour_xf7_layer2.fig', layer=2 )
