! 3D Stokes problem on a cubic domain with a sphere
! eXtended FEM approach with embedded Dirichlet condition on the sphere.
...
! Optional non-zero imposed velocity of the sphere.
! Optional symmetric matrix.
! Optional Nitsche's method.
! Similar to xfem8/xfem9 but now 3D.
...
! functions module for xfem12

module functions_xf12_m
...
! constants
  ...
  real(dp), parameter :: &
    ...
    U(3) = [ 1._dp, 2._dp, 3._dp ], & ! imposed velocity of the sphere if
                                 ! coefficients%i(42) = 0
...
! fill coefficients
  ...
  coefficients%i(42) = -1  ! -1: impose zero velocity,
                           !  0: impose non-zero velocity U
...
! object for plotting the nodeset
  ...
  call plot_points_curves ( plot_options, mesh, 'curves_xf12.fig' )

  if ( full_figplot_mesh ) then

    call plot_mesh ( plot_options, mesh, 'mesh_xf12.fig' )
    plot_options%objectpointcolor=4
    plot_options%objectpointsize=0.4
    call plot_objects ( plot_options, mesh, 'mesh_xf12.fig', append=.true., &
      object1=1 )
    ...
    call plot_mesh ( plot_options, mesh, 'mesh_xf12.fig', surfaces=[3,4,6] )
...
! sampling for gnuplot
  ...
  open( unit=15, file='sample_p_xf12.out', recl=300 )
...
! sample pressure on object
  ...
  open( unit=15, file='sample_v_xf12.out', recl=300 )
...
! sample velocity on sphere
  ...
  open( unit=15, file='sample_dudy_xf12.out', recl=300 )
...
! post-processing: create mesh_plot for plotting
  ...
  call plot_mesh ( plot_options, mesh_plot, 'mesh_xf12_plot.fig' )

  call write_mesh_vtk ( mesh_plot, filename='mesh_xf12_plot.vtk' )
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh_plot, problem_plot, vector=pressure, &
    dataname='pressure', filename='sphere_xf12.vtk' )

  call write_vector_vtk ( mesh_plot, problem_plot, &
    filename='sphere_xf12.vtk', &
