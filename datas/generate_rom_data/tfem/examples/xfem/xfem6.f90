...
! Fluid described by a layer.
! Plot a layer.
! Prebuild eltree for all elements crossing the interface.

! functions module for xfem6

module functions_xf6_m
...
!   Set ninti
    ...
    if ( associated(eltree_array(elem)%p) ) then

!     element crosses the interface

      if ( printnumel ) then
        print *, 'number of subelements outside = ', &
          number_of_subelements(eltree_array(elem)%p,lsign=[1])
      end if
...
!     composite integration scheme for levelset >= 0
      ...
      ninti = number_of_integration_points ( eltree_array(elem)%p, lsign=[1],&
...
!     fully in
!     NOTE: not needed in the case of layers
...
!     composite integration scheme for levelset >= 0
      ...
      call integration_points ( eltree_array(elem)%p, xig, wg, lsign=[1], &
...
!     add only elements in the outside region
      ...
      call eltree_to_mesh ( eltree_array(elem)%p, elmesh, lsign=[1], &
        mapcoor=mapcoor )
...
! constants
  ...
  real(dp), parameter :: &
    ...
    split_threshold = 1.e-2_dp, & ! levelset value for being "close" enough to
...
! allocate array of pointers to an eltree (eltree for all elements)

  allocate ( eltree_array(mesh%grpnumel(1)) )

! nodeset for layer of interface + external nodes
...
! object for plotting the nodeset
  ...
  call plot_mesh ( plot_options, mesh, 'mesh_xf6.fig' )
  plot_options%objectpointcolor=4
  plot_options%objectpointsize=0.4
  call plot_objects ( plot_options, mesh, 'mesh_xf6.fig', append=.true., &
    object1=1 )
...
! define constraints on the object
  ...
  call define_constraint ( mesh, input_probdef, object=1, layer=1, physq=1, &
...
! sample pressure on cylinder (errorobject=1 to warn for points not in layer)

  call fill_sample ( mesh, problem, sample_p, ndegfd=1, object=1, layer=1, &
...
! sample velocity on cylinder (errorobject=1 to warn for points not in layer)

  call fill_sample ( mesh, problem, sample_v, ndegfd=2, object=1, layer=1, &
...
! sample vorticity on cylinder (errorobject=1 to warn for points not in layer)

  coefficients%i(13)=5

  call fill_sample ( mesh, problem, sample_vort, ndegfd=1, object=1, layer=1, &
...
! sample pressure (use layer as argument to detect points not in layer)
...
! sample velocity (use layer as argument to detect points not in layer)
...
! sample vorticity (use layer as argument to detect points not in layer)
...
!   post-processing: plot in a layer
    ...
    call plot_in_layer ( 'velocity_xf6_layer.fig', &
      'pressure_contour_xf6_layer.fig', &
      'vorticity_contour_xf6_layer.fig', layer=1 )
...
! delete all data including all allocated memory
  ...
  deallocate ( nodes, d, eltree_array )
...
!       fill root node of the eltree
        ...
        call fill_node_eltree ( eltree_array(elem)%p, coor )
...
!       divide root reference domain into subdomains
        ...
        call subdivide ( eltree_array(elem)%p, levelset=levelset, numsplit=numsplit, &
          split_threshold=split_threshold, numsplitmin=numsplitmin, &
          mapcoor=mapcoor, submesh=.true. )

!       check whether interface is in element

        if ( any( number_of_subelements_vector(eltree_array(elem)%p,lsign=[-1,1]) == 0 ) ) then

!         no interface, remove eltree

          call delete(eltree_array(elem)%p)  ! delete actual eltree
          deallocate( eltree_array(elem)%p ) ! delete pointer target (pointer becomes disassociated)
...
! find outside layer nodes

  subroutine find_outside_layer_nodes
...
!   add nodes connected to elements at the interface
    ...
    do elem = 1, mesh%grpnumel(1)
      if ( associated(eltree_array(elem)%p) ) then
         nodes(mesh%topology(1)%a(:,elem)) = 1
       end if
    end do
...
!   count and collect the nodes (overwrite array nodes along the way)
  ...
  end subroutine find_outside_layer_nodes
