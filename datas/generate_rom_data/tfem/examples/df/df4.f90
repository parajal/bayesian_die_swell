...
! constants
  ...
  logical, parameter :: &
    equidistant = .true. ! Use equidistant grid in age (tau).
...
!    fintpltau = 2,      & ! P2 interpolation in age (tau)
    nintvaltau = 100,   & ! number of age intervals
    ncompf = 4,         & ! number of components of deformation tensor F
    ninttau = fintpltau+1, & ! number of fields within an interval
    numtimesteps = 110, & ! number of time steps
    ns = 3                ! number of samples

  real(dp), parameter :: &
    deltat = 2.e-2_dp,   & ! time step
    beta = 1,            & ! SUPG factor
    dtau1 = 2e-2_dp,     & ! length of first interval in age (tau) direction
    tauc = 2.0_dp,       & ! maximum age (cutoff) (only for equidistant=.false.)
...
! definitions
  ...
  type(vector_t) :: Ffield, Ffieldtensor, Bfieldtensor
...
! delete all data including all allocated memory
  ...
  call delete ( Ffield, Ffieldtensor, Bfieldtensor )
  call delete ( fval )
