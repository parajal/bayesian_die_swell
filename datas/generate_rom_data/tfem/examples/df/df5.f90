...
! Decoupled fields, explicit DG with P1 interpolation for tau-convection
!   (similar to the 2001 paper).
! Extrapolated BDF2 time discretization.
! Problem based on df1 with the extension:
...
! problem definition deformation tensor
  ...
  input_probdef_f%vec_elementdof(1)%a =   &
      reshape ( [ 1,0,1,0,1,0,1,0,0,    &  ! f
                  ...
                  4,4,4,4,4,4,4,4,4 ], &  ! symmetric tensor for plotting
                  [9,3] )
...
! create the structure oldvectors_f
  ...
  call create_oldvectors ( oldvectors_f, nsysvec=1, nsysvec3=2, nprob=2 )
...
! post processing of the stress tensor
  ...
  coefficients%i(361)=1 ! location in oldvectors of the fields is s3
...
! delete all data including all allocated memory
  ...
  call delete ( sol_f, solm1_f, rhs_f )
