...
! Incompressible EGP model using the conformation tensor formulation.
...
! constants
  ...
  real(dp), parameter :: &
    ...
    U_avg = 0.05_dp         ! average velocity in the channel

  real(dp), parameter :: &
    deltat = 10.0_dp,     & ! time step
...
! definitions
  ...
  type(vector_t) :: ctensor, vonmises, viscous_stress, viscoelastic_stress

  integer :: step, i, iter, m, nn
  integer :: vertices(4) = [1,3,5,7]
  real(dp) :: alpha, epsu, epsp, epsc, Wi, flowrate, H
...
! fill coefficients
  ...
  coefficients%r(10) = U_avg ! global scaling velocity for SUPG, when needed
...
! initial solution conformation tensor
  ...
  do m = 1, nmodes
    if ( logc == 0 ) then ! standard
      ...
      soln%u(cvalzz(m)%s) = 1 ! initial cyy
...
!     convergence test
      ...
      epsu = maxval(abs(dsol%u(vel%s))) / U_avg
      epsp = maxval(abs(dsol%u(pres%s))) * H / ( U_avg * eta_0 )
...
! derive vectors
  ...
  call write_vector_vtk ( mesh, problem, filename='poiseuille5.vtk', &
    dataname='velocity', vector=velocity )
  call write_scalar_vtk ( mesh, problem, filename='poiseuille5.vtk', &
    dataname='pressure', vector=pressure, append=.true. )
  call write_scalar_vtk ( mesh, problem, filename='poiseuille5.vtk', &
    dataname='vorticity', vector=vorticity, append=.true. )

  call printtofile ( mesh, problem, filename='velocity_c2.out', curve=2, &
    vector=velocity )
  call printtofile ( mesh, problem, filename='pressure_c1.out', curve=1, &
    vector=pressure )
  call printtofile ( mesh, problem, filename='pressure_c2.out', curve=2, &
    vector=pressure )

! viscous stress

  call create_vector ( problem, viscous_stress, vec=6+nmodes )
  ...
  call write_tensor_vtk ( mesh, problem, filename='poiseuille5.vtk', &
    dataname='viscous_stress', vector=viscous_stress, append=.true. )

  call delete ( viscous_stress )
...
! conformation tensor
  ...
  call printtofile ( mesh, problem, filename='conformation_c2.out', curve=2, &
    vector=ctensor )
...
! von Mises stress
  ...
  call printtofile ( mesh, problem, filename='vonmises_c2.out', curve=2, &
    vector=vonmises )
...
! viscoelastic stress
  ...
  call create_vector ( problem, viscoelastic_stress, vec=5+nmodes )

  call derive_vector ( mesh, problem, viscoelastic_stress, &
    elemsub=deriv_viscoelastic_stress_tensor, &
    coefficients=coefficients, oldvectors=oldvectors_ve )

  call write_tensor_vtk ( mesh, problem, filename='poiseuille5.vtk', &
    dataname='viscoelastic_stress', vector=viscoelastic_stress, &
    append=.true., assume33=.true. )

  call delete ( viscoelastic_stress )
