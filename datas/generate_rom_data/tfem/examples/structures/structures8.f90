...
! definitions
  ...
  type(mesh_t) :: mesh1, mesh2, mesh3, mesh4, mesh
...
! fill coefficients
  ...
  coefficients(1)%r(4) = -q(1)

  coefficients(2) = coefficients(1)
  coefficients(2)%r(4) = -q(2)

  coefficients(3) = coefficients(1)
  coefficients(3)%r(4) = -q(3)
...
! create mesh
  ...
  call mesh_merge ( mesh3, mesh4, mesh, point1=3, point2=1, &
    nogroupmerge=.true. )
...
! problem definition
  ...
  do i = 1, 3
    input_probdef%elementdof(i)%a = 2          ! (v,phi) in two end nodes
...
! at end points of beam sections:
  ...
  do i = 1, 3
    if ( support(i) ) then
      call define_essential ( mesh, input_probdef, point=i+1, degfd=[1,0] )
    end if
  end do
...
! fill solution vector with essential boundary conditions
  ...
  do i = 1, 3
    if ( support(i) ) then
      call fill_sysvector ( mesh, problem, sol, point=i+1, degfd=1, &
         value=0._dp )
    end if
  end do
...
! fill rhs vector with discrete forces/monents at end points of beam sections:
  ...
  do i = 1, 3
    ...
    call fill_sysvector ( mesh, problem, load, point=i+1, degfd=2, value=M(i) )
  end do
...
! post processing
  ...
  call postprocessing_beam1 ( mesh, problem, mcoefficients=coefficients, &
    mshfilename='structures8.mso', sol=sol, load=load, reacf=reacf )
