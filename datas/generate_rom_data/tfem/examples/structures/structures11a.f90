...
! P2/P4 interpolation. 2D elements.
! Gmsh mesh.

program structures11a
...
! fill coefficients
  ...
  coefficients%i(3) = 1 ! P4 Hermite interpolation for v
...
! read mesh
  ...
  call read_mesh_gmsh ( mesh, filename='triangle2.msh', ndim=2 )
...
! (u,v,phi) in two end nodes + (u,bubble v) in the middle node
  input_probdef%elementdof(1)%a = [ 3, 2, 3 ]
  input_probdef%vec_elementdof(1)%a = 1      ! scalar quantity
  input_probdef%vec_elementdof(1)%a(:,2) = 2 ! vector quantity
  input_probdef%vec_elementdof(1)%a(:,3) = 3 ! all nodes all degrees
  input_probdef%vec_elementdof(1)%a(:,4) = [ 3, 0, 3 ] ! end nodes all degrees
...
! post processing
  ...
  call postprocessing_beam2 ( mesh, problem, coefficients=coefficients, &
    mshfilename='structures11a.mso', sol=sol, load=load, reacf=reacf )
