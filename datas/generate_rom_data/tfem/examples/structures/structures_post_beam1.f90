...
! Post processing routines for 1D beam structures

module structures_post_beam1_m
...
! generic routine for post-processing all data for a 1D beam structure

  subroutine postprocessing_beam1 ( mesh, problem, mshfilename, sol, load, &
...
!   direct write of sysvectors load and reacf
    ...
    if ( present(load) ) call write_sysvector_beam1 ( mesh, problem, &
      mshfilename, dataname='load', sysvector=load )

    if ( present(reacf) ) call write_sysvector_beam1 ( mesh, problem, &
...
!     additional nodes present: write displacements via deriv
      ...
      call derive_write_vector_beam1 ( mesh, problem, mshfilename, &
...
!     only end nodes present: directly write from sysvector
      ...
      call write_vector_gmsh ( mesh, problem, filename=mshfilename, &
        dataname='displacement', sysvector=sol, degfd=[0,1,0], &
        append=present(load).or.present(reacf) )
...
!   write of beam scalar via deriv
    ...
    call derive_write_scalar_beam1 ( mesh, problem, mshfilename, &
  ...
  end subroutine postprocessing_beam1


! direct write of system vectors (load and readcf) for a 1D beam structure

  subroutine write_sysvector_beam1 ( mesh, problem, mshfilename, dataname, &
    sysvector, append )
...
!     forces
      call write_vector_gmsh ( mesh, problem, filename=mshfilename, &
        dataname=dataname1, vector=allnodes, degfd=[0,1,0], &
        append=append )
!     moments
      call write_vector_gmsh ( mesh, problem, filename=mshfilename, &
        dataname=dataname2, vector=allnodes, degfd=[0,0,2], &
        append=.true. )
...
!     forces
      call write_vector_gmsh ( mesh, problem, filename=mshfilename, &
        dataname=dataname1, sysvector=sysvector, degfd=[0,1,0], &
        append=append )
!     moments
      call write_vector_gmsh ( mesh, problem, filename=mshfilename, &
        dataname=dataname2, sysvector=sysvector, degfd=[0,0,2], &
        append=.true. )
...
!   requires vec=3 and vec=4 to be defined.
 ...
 !    create vector (vec=4) that contains only end nodes, e.g. [2,0,2]
      call create ( problem, endnodes, vec=4 )

      call transfer_data ( mesh, problem, sysvector1=sysvector, &
        vector2=endnodes, degfd1=[1,2], degfd2=[1,2] )

      allnodes%u = 0 ! set all values to zero, including internal nodes

      call transfer_data ( mesh, problem, vector1=endnodes, &
        vector2=allnodes, degfd1=[1,2], degfd2=[1,2] )

      call delete ( endnodes )

    end subroutine transfer_data_endnodes

  end subroutine write_sysvector_beam1


! post-processing scalar data using derive for a 1D beam structure

  subroutine derive_write_scalar_beam1 ( mesh, problem, mshfilename, &
...
!   print
  ...
  end subroutine derive_write_scalar_beam1


! post-processing vector data using derive for a 1D beam structure

  subroutine derive_write_vector_beam1 ( mesh, problem, mshfilename, &
    dataname, sysvector, coefficients, mcoefficients, append )
...
!     derive
      call derive_vector ( mesh, problem, dvec, elemsub=beam_displacement1, &
        coefficients=coefficients, mcoefficients=mcoefficients, &
...
!     derive
      call derive_vector ( mesh, problem, dvec, elemsub=beam_q1, &
        coefficients=coefficients, mcoefficients=mcoefficients, &
        oldvectors=oldvectors, order='ND' )
...
!   write
    ...
    call write_vector_gmsh ( mesh, problem, filename=mshfilename, &
      dataname=dataname, vector=dvec, assume2D=.true., append=append )
...
!   print
  ...
  end subroutine derive_write_vector_beam1

end module structures_post_beam1_m
