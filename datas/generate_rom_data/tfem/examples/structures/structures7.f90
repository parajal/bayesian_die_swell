...
! constants
  ...
  real(dp), parameter :: &
    ...
    q1 = 1._dp,   &  ! distributed load on the left part
    q2 = 0._dp       ! distributed load on the right part
...
! fill coefficients
  ...
  coefficients(1)%r(4) = -q1

  coefficients(2) = coefficients(1)
  coefficients(2)%r(4) = -q2
...
! post processing
  ...
  call postprocessing_beam1 ( mesh, problem, mcoefficients=coefficients, &
    mshfilename='structures7.mso', sol=sol, load=load, reacf=reacf )
