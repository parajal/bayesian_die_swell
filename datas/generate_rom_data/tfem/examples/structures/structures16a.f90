...
! Gmsh mesh.
! P2/P4/P2 interpolation for axial displacement, transverse displacement
! and axial rotation, respectively.
! Extension of structures11 to a 3D beam structure.
! Identical to structures16, except using P2/P4/P2 interpolation.

program structures16a
...
! fill coefficients
  ...
  coefficients%i(3) = 1 ! P3 Hermite interpolation for v
  coefficients%i(4) = 0 ! constant distributed force
  coefficients%i(12) = 6 ! P2 interpolation for u
  coefficients%i(13) = 6 ! P2 interpolation for theta
...
! read mesh
  ...
  call read_mesh_gmsh ( mesh, filename='triangle2.msh', ndim=3 )
...
! problem definition
  ...
  input_probdef%elementdof(1)%a = [6,4,6]
  input_probdef%vec_elementdof(1)%a = 1      ! scalar quantity
  input_probdef%vec_elementdof(1)%a(:,2) = 3 ! vector quantity
  input_probdef%vec_elementdof(1)%a(:,3) = 6 ! allnodes
  input_probdef%vec_elementdof(1)%a(:,4) = [6,0,6] ! endnodes
...
! post processing
  ...
  call postprocessing_beam3 ( mesh, problem, coefficients=coefficients, &
    mshfilename='structures16a.mso', sol=sol, load=load, reacf=reacf )
