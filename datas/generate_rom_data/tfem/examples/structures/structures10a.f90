...
! fill coefficients
  ...
  coefficients(1)%i(12) = 6 ! P2 interpolation for u
...
!   (u,v,phi) in two end nodes + (u,bubble v) in the middle node
    input_probdef%elementdof(i)%a = [ 3, 2, 3 ]
...
! post processing
  ...
  call postprocessing_beam2 ( mesh, problem, mcoefficients=coefficients, &
    mshfilename='structures10a.mso', sol=sol, load=load, reacf=reacf )
