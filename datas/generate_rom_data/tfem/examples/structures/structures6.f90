...
! Function for distributive load.
! P4 interpolation
...
! constants
  ...
  integer, parameter :: &
    ...
    funcnr=4      ! function number for the distributed load
  ...
  real(dp), parameter :: &
    ...
    F = 0_dp, &     ! Force at x=L
    M = 0_dp, &     ! Moment at x=L
    qb = 1_dp       ! distributed load constant
...
! fill coefficients
  ...
  coefficients%i(4) = 1 ! distributed force is a function
  coefficients%i(5) = funcnr ! distributed force function number
...
! post processing
  ...
  call postprocessing_beam1 ( mesh, problem, coefficients=coefficients, &
    mshfilename='structures6.mso', sol=sol, load=load, reacf=reacf )
