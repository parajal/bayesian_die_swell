...
! constants
  ...
  integer :: &
    maxnumnriter = 20, & ! maximum number of Newton-Raphson iterations
    restart = 0,       & ! restart=0: no restart
                         ! restart=1: normal restart
                         ! restart=2: read restart file + post processing
                         ! restart=3: restart with solnm1=soln (read soln only)
    restart_every = 1, & ! write restart file every restart_every steps.
                         ! -1: means none
    timeint1 = 8,      & ! first time step time-integration scheme.
    timeint2 = 10,     & ! time-integration scheme for time step 2 and beyond
  ...
  real(dp) :: &
    ...
    U = 0.02_dp,        & ! average velocity in the channel
    deltat = 0.4_dp,    & ! time step
    epsdconf = 1e-12_dp,& ! Newton-Raphson convergence difference threshold
    epsrconf = 1e-12_dp,& ! Newton-Raphson convergence threshold residual
  ...
  logical :: &
    ...
    JacobianSUPG = .false., & ! take Jacobian of SUPG test function into account
    ...
    ctime = .true., & ! continue with the time read from the restart file
...
! definitions
  ...
  type(subscript_t), dimension(nmodes) :: bval, res_bval
  ...
  integer :: step, i, iter, m, ncoefr
  integer :: vertices(3) = [1,3,5]
  real(dp) :: alpha, epsu, epsp, epsb, Wi, K_drag, U_prev, time, time_prev
  real(dp) :: eta_p, eta_s, eta_0, Ru, Rb

! namelist for input of variables; read from standard input

  namelist /comppar/ timer, meshfile, inrestartfile, outrestartfile, &
    restart, restart_every, model, G, lambda, beta_s, mobility, U, &
    deltat, numtimesteps, timeint1, timeint2, rs, is, &
    postprocess, ctime, epsdconf, epsrconf, maxnumnriter, &
    metisnodes, metisdegrees
...
!   excluding essential and constraint degrees for computing the residual Rb
    call create_subscript ( mesh, problem, res_bval(m), &
      physqarr=[physqb+m-1], essentialpart=.false., excludecurves=[4,12] )
    do i = 1, ncompb
...
! Weisenberg number
  ...
  open ( unit=16, file='max_cxx.out', recl=300 )
...
!     compute residual norms
      ...
      Rb   = norm2(rhsd%u(res_bval(1)%s)) &
                / sqrt(real(size(res_bval(1)%s),dp)) * R / U
...
!     convergence test
      ...
      if ( printscreen) print *, iter, epsu, epsp, epsb, Ru, Rb
      write(17,*) iter, epsu, epsp, epsb, Ru, Rb

      if ( maxval([ epsu, epsp, epsb ]) < epsdconf .and. &
...
!   write max and mean values of conformation and b-tensor tensor to a file
    ...
    write(11, fmt=*) time, maxval(cten%u(cxx%s)), &
                           maxval(cten%u(cxy%s)), &
                           maxval(cten%u(cyy%s)), &
    ...
    if ( printscreen ) &
         print *, 'max cxx = ', maxval(cten%u(cxx%s))
    write(16,*) step, time, maxval(cten%u(cxx%s))
...
! delete all data including all allocated memory
  ...
  do m = 1, nmodes
    call delete ( bval(m), res_bval(m) )
    do i = 1, ncompb
      call delete ( bvalcmp(i,m) )
    end do
  end do
...
! output restart data
  ...
  subroutine output_restart
    ...
    write(17,*) 'Output restart data at step ', step, ' time = ', time, &
      ' U = ', U, 'Wi = ', Wi
    if ( printscreen) print *, 'Output restart data at step ', step, &
      ' time = ', time, ' U = ', U, 'Wi = ', Wi

    open ( unit=15, form='unformatted', file=outrestartfile )

    write(15) U, time, soln%u, solnm1%u
...
! open files and input restart data
  ...
  subroutine input_restart
    ...
    if ( restart == 3 ) then
      read(15) U_prev, time_prev, soln%u
      solnm1%u = soln%u
    else
      read(15) U_prev, time_prev, soln%u, solnm1%u
    end if

    close(unit=15)

    write(17,*) 'Input restart data, U = ', U_prev, ' time = ', time_prev
    if ( printscreen) &
      print *, 'Input restart data, U = ', U_prev, ' time = ', time_prev
...
! post-processing
  ...
  subroutine postprocessing
    ...
    type(vector_t) :: velocity, pressure, vorticity, ctensor, btensor
