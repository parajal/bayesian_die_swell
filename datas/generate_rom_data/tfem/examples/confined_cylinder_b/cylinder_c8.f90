! Sequence of steady states of flow around a cylinder for a Giesekus model
! using the conformation tensor formulation.
...
! constants
  ...
  integer, parameter :: &
    ndU = 3           ! number intervals with different dU increment

  real(dp), parameter :: &
!   increments in imposed average velocity
    dU(ndU) = [ 0.1_dp, 0.01_dp, 0.001_dp ], & ! increments in imposed average velocity
!   end values of the intervals in U
    Ue(ndU) = [ 1.1_dp, 1.18_dp, 1.19_dp ], & ! end values of the intervals in U
!   Newton-Raphson convergence threshold for each interval
    epsconfin(ndU) = [ 1e-8_dp, 1e-8_dp, 1e-8_dp ]
