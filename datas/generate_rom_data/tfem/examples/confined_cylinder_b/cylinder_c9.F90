...
! Startup of flow around a cylinder for an Oldroyd-B or Giesekus model using
! the conformation tensor formulation.
...
! definitions
  ...
  type(sysvector_t), target :: soln, solnm1, soliter, reacf
  ...
  real(dp) :: alpha, epsu, epsp, epsc, Wi, K_drag, U_prev, time, time_prev
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ timer, meshfile, inrestartfile, outrestartfile, &
    ...
    postprocess, ctime, epsdconf, epsrconf, maxnumnriter, thetapar, &
...
!   write max and mean values of conformation tensor to a file
    ...
    write(11, fmt=*) step * deltat, maxval(cten%u(cxx%s)), &
                                    maxval(cten%u(cxy%s)), &
                                    maxval(cten%u(cyy%s)), &
