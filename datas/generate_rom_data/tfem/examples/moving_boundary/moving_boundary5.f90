! Fluid column between two plates loaded under gravity forces.
! Surface tension on fluid-air interface and fluid-wall interface.
! Perfect slip on the walls. Axisymmetric. Newtonian fluid (Stokes).

program moving_boundary5
...
! variables
  ...
  integer :: &
    coorsys = 1, & ! axisymmetric coor. system
...
! read mesh2b
  ...
  if ( abs(mesh%coor(mesh%points(2),2)) > tiny(0._dp) ) then
    write(*,'(a/)') &
      'Error: r coordinates of the center line non-zero. Change mesh.'
    stop
  end if
  if ( mesh%coor(mesh%points(2),2) < 0._dp ) then
...
! center line
  call define_essential ( mesh, input_probdef, &
    curves=[8], physq=physqv, degsfd=[2] )
! walls (perfect slip)
  call define_essential ( mesh, input_probdef, &
    curves=[10], physq=physqv, degsfd=[1] )
...
! create subscript for velocity sampling on curve 9

  call create_subscript ( mesh, problem, vel9, physqarr=[physqv], curves=[9] )
...
! create mesh for surface advection
  ...
  call curve_to_1Dmesh ( mesh, mesh%curves(9), dim1D=1, pnts=[6,1], &
    mesh1D=mesh_sf_adv )
...
! fill solution vector with essential boundary conditions
  ...
  sol_sf_adv%u = mesh%coor(mesh%points(6),2)
...
! create subscript for the height values
  ...
  call create ( mesh_sf_adv, problem_sf_adv, hgt_end, points=[2,1] )
...
!     build surface integral on free surface
      ...
      call add_boundary_elements ( mesh, problem, rhsd, &
        ...
        curve=9, coefficients=coefficients, physq=[physqv] )
...
!     build surface integral on walls
      ...
      call add_boundary_elements ( mesh, problem, rhsd, &
        elemsub=surface_tension_curve, &
        curve=10, coefficients=coefficients, physq=[physqv] )
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh, problem, vector=pressure, dataname='pressure', &
    filename='moving_boundary5.vtk' )

  call write_vector_vtk ( mesh, problem, filename='moving_boundary5.vtk', &
    dataname='velocity_vector', vector=velocity, append=.true. )

  call write_scalar_vtk ( mesh, problem, vector=gammadot, &
    filename='moving_boundary5.vtk', dataname='gammadot', append=.true. )
...
!   predictor for surface height
    ...
    disp = sol_sf_adv_pred%u - mesh%coor(mesh%curves(9)%nodes,2)

!   update nodes of the mesh (only in the second (=r) direction)

    call update_mesh_nodes ( mesh, problem_update_mesh, disp, &
      el=mesh_sf_adv%points([2,1]), eg=mesh%points([1,6]), cf=9, df=2 )
...
!   get velocities on the moving curve
    ...
    velocity_sf_adv%u = sol%u(vel9%s)
