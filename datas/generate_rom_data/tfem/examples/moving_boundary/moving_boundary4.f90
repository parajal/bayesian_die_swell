! Moving boundary due to second normal stresses.
! 2D domain, with 3D velocities.
! Perfect slip (in plane) on the walls.
! Dirichlet on the bottom.
! Imposed flow rate in third direction.
...
! Add the "open boundary" term of the partial integration of the surface
! tension. It doesn't seem to do much and has not been included by default.
! It should also not be included, if "real" surface tension is modelled.
  logical, parameter :: add_wall_surface_force = .false.
...
! variables
  ...
  real(dp) :: &
    ...
    flowrate = 1.0_dp, & ! flow rate through the channel
  ...
  integer :: icomp, step, i, npar, pointf, curvef
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ numtimesteps, plot_mesh_every, &
    rho, grv, eta_s, eta_p, lambda, alphapar, gamma_fa, dgamma_w, flowrate,&
...
! fill coefficients
  ...
  call create_coefficients ( coefficients, ncoefi=502, ncoefr=500+npar*nmodes )
...
! bottom
  ...
  if ( symmetric ) then
!   upper wall (perfect slip in z-direction)
    call define_essential ( mesh, input_probdef, &
      curve1=6, physq=physqv, degfd=[0,1,1] )
!   lower wall (perfect slip in z and theta-direction)
    call define_essential ( mesh, input_probdef, &
      curve1=1, physq=physqv, degfd=[0,1,0] )
  else
...
! constraint for flow rate
  ...
  call define_constraint ( mesh, input_probdef, &
    physq=physqv, surface1=1, nglobalc=1 )
...
!     build surface integral on free surface
      ...
      if ( add_wall_surface_force ) then

        curvef = 8  ! free surface curve
        pointf = 5  ! point at the end

        coefficients%i(501) = curvef  ! curve of the free surface
        coefficients%i(502) = mesh%curves(curvef)%nelem ! last element on curve

        call add_boundary_elements_point ( mesh, problem, rhsd, &
          elemsub=surface_tension_boundary_point1, &
          point=pointf, coefficients=coefficients, physq=[physqv] )
...
! Used together with add_boundary_elements_point
  ...
  subroutine surface_tension_boundary_point1 ( mesh, problem, point, matrix, &
    ...
    xig(1,1) = 1._dp  ! assume last point
