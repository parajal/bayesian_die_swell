...
! Gmsh generated mesh.
!
! The final solution of the conformation is projected onto a new mesh.
...
! constants
  ...
  integer, parameter :: &
    ...
    ncompv = 3,     & ! number of components input vector for projection
...
! definitions
  ...
  type(vector_t) :: conformation_tensor
  ...
  type(sysvector_t), dimension(ncompv) :: sol2, rhsd2
...
! conformation tensor
  ...
  call write_tensor_vtk ( mesh, problemc, filename='cylinder.vtk', &
    dataname='conformation_tensor', vector=conformation_tensor )
...
! create vector and copy data for input of projection problem
  ...
  do icomp = 1, ncompv
    call transfer_data ( mesh, problemc, sysvector1=solc(icomp,1), &
      vector2=vec_p, degfd1=[1], degfd2=[icomp] )
  end do
...
! problem definition for projection
  ...
  input_probdef2%elementdof(1)%a(vertices) = 1  ! one component
...
! problem definition conformation tensor
  ...
  solc(1,1)%u(cval%s) = sol2(1)%u
  solc(2,1)%u(cval%s) = sol2(2)%u
  solc(3,1)%u(cval%s) = sol2(3)%u
