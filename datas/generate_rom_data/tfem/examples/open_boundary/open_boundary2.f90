...
! build (assemble) matrix and vector from elements
  ...
  if ( pintpl == 2) then                     ! P1 pressure
    call build_system_constraint ( mesh, problem, sysmatrix, rhsd, &
      constraint1=1, elemsub=stokes_zero_pressure_int_object, addmatvec=.true.,&
      coefficients=coefficients )
  else if ( pintpl == 4 ) then               ! Q1 pressure
    call build_system_constraint ( mesh, problem, sysmatrix, rhsd, &
      constraint1=1, elemsub=stokes_zero_pressure_int_curve, addmatvec=.true., &
      coefficients=coefficients )
  end if
...
! write divergence on curve 2 to a file
  ...
  call printtofile ( mesh, problem, filename='divergence_on_c2.out2', curve=2, &
    vector=divergence )

! write pressure on curve 2 to a file

  call printtofile ( mesh, problem, filename='pressure_on_c2.out2', curve=2, &
    vector=pressure )

! write vorticity on curve 2 to a file

  call printtofile ( mesh, problem, filename='vorticity_on_c2.out2', curve=2, &
    vector=vorticity )
