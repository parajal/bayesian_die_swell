...
! Compute the bulk stress in case of an elastic soft solid limit.
...
! constants
  ...
  integer, parameter ::  &
    ...
    ncomp_P2 = 9,        & ! number of components for P2 projection:
                           ! x,y-coordinates at n and n-1
    ncomp_P1 = 12          ! number of components for P1 projection:
...
! Variables
  ...
  logical ::     &
    ...
    do_remeshing = .true.,     & ! Remeshing when elements are too distorted
    ctime = .false.,        & ! continue time in output after restart
    timeron = .false.,      & ! print cpu time
    mesh_plot = .false.,    & ! write mesh plot files/info (curves, mesh etc.)
    compute_error = .false.,& ! Compute error. Only makes sense if:
                              ! vfuncnr= 0 (no flow) or 3 (rotation).
  ...
  real(dp) ::  &
    ...
    Ca = 1.0_dp,          & ! elastic capillary number
    ...
    lambda_s = 1.0e12_dp, & ! "solid" relaxation time
    gammac = 0.01_dp,     & ! surface tension coefficient
    ...
    is_c  = 1.8_dp,       & ! integer_storage conformation LU (HSL)
    ...
    betai = 0.5_dp,       & ! factor beta in SUPG of interface equation
...
! some more misc variables
  ...
  integer :: nnodes, nelem, elshape, step, post, c_cpl, c_perx, c_pery, &
    c_perz, c_flr
  integer :: vertices(4)=[1,3,5,10]
  real(dp) :: xp(1,ndim), rp(1), dx_box, dx_part, alpha, G, lambda, &
    resultsum(1+ndim), int_stokes_stress_domain(6), int_ve_stress_domain(6), &
    bulk_extra_stress(6), particle_stress(6)
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ solid, periodic, triperiodic, periodic_channel, &
    flowrate, move_mesh_x, monitor_func, do_remeshing, mesh_plot, &
    compute_error, compute_volume, n, nc, model, gauss, gaussb, gauss_proj, &
    inttype_proj, choice_um, vfuncnr, vtkevery, ctime, restart, &
    restart_every, logc, timeint1, timeint2, timeint, numtimesteps, &
    ox, oy, oz, lx, ly, lz, xc, rc, Ca, eta_e, eta_i, eta_p, lambda_p, &
    lambda_s, gammac, factor, U_avg, shear_rate, rs_gup, is_gup, rs_c, is_c, &
    deltat, betai, fac_min, fac_max, volume_threshold, &
    aspect_ratio_threshold, timeron
...
! External fluid (group 1)
  input_probdef_c_proj%vec_elementdof(1)%a = 0  ! no degress

! Internal fluid (group 2)
  input_probdef_c_proj%vec_elementdof(2)%a(:,1) = 0
  input_probdef_c_proj%vec_elementdof(2)%a(vertices,1) = 1 ! c

  input_probdef_c_proj%physq = [1]
  input_probdef_c_proj%probnr = 3

! Problem definition and create sysmatrix, vectors of conformation projection
! problem
...
! problem definition of the interface
  ...
  input_probdef_b%vec_elementdof(1)%a(:,1) = ndim  ! 3D vector
...
! start time stepping
  ...
  do step = 1, numtimesteps
    ...
    call toc ( 'start new time step' )

    write(*,'(a,i0)') 'step = ', step0+step

    if ( do_remeshing ) then
...
!   integrate stress
    ...
    if ( modulo ( step0+step, vtkevery ) == 0 ) then

!     Stokes stress (2*eta*D)
      call integrate ( mesh, problem, int_stokes_stress_domain, &
        elemsub=stokes_integrate_stress, &
        mcoefficients=coefficients, oldvectors=oldvectors_c )

!     Viscoelastic stress internal fluid (G*(c-I))
      call integrate ( mesh, problem_c, int_ve_stress_domain, &
        elgroup1=2, elemsub=viscoelastic_integrate_tau_tensor, &
        coefficients=coefficients(2), oldvectors=oldvectors_c )

      bulk_extra_stress = ( int_stokes_stress_domain + &
                          int_ve_stress_domain ) / ( lx * ly * lz )

      particle_stress = int_ve_stress_domain / resultsum(1)

      write(100, '(i5,7es16.8)') step0+step, time0+step*deltat, &
        bulk_extra_stress
      write(101, '(i5,7es16.8)') step0+step, time0+step*deltat, &
        particle_stress
...
!   merge internal and external meshes
    ...
    call merge_internal_external_meshes
...
! merge internal and external meshes
  ...
  subroutine merge_internal_external_meshes
...
!    call add_to_mesh ( mesh, matchingsurface=[8,7], replace=8 )
  ...
  end subroutine merge_internal_external_meshes
...
!   Generate permutation arrays with metis
    ...
    solver_options_u%pivot_order=1 ! use pivot order from the permutation arrays
...
!   Generate permutation arrays with metis
    ...
    if ( .not. sysmatrix_c%renumber ) call renumber_metis ( sysmatrix_c )
...
!   solve conformation, keeping LU decomposition in the loop over components
    ...
    solver_options_c%pivot_order=1 ! use pivot order from the permutation arrays
...
!   Solve projection problem, keep LU decomposition in the loop over components
    ...
    do m = 1, nmodes
      do i = 1, ncompc
        ...
        call add_effect_of_essential_to_rhs ( problem_c_proj, sysmatrix_c_proj,&
           sol_c_proj(i,m), rhs_c_proj(i,m) )
...
!   Updating of interface mesh coordinates with new xnp1
    ...
    call toc ( 'build_solve_interface' )
...
!       BDF2
      ...
      if ( step < 4 ) cvv(1) = ( xc(2) - oy )*shear_rate
...
!       open files for stress

        if ( ctime ) then
          open (unit = 100, file='bulkstress.ou', recl=300, status='old', &
            position='append' )
          open (unit = 101, file='particle_stress.out', recl=300, &
            status='old', position='append' )
        else
          open (unit = 100, file='bulkstress.out', recl=300 )
          open (unit = 101, file='particle_stress.out', recl=300 )
        end if
...
!   Creation of system matrix for gradients/velocity/pressure problem
...
!   Creation of system vectors (solution and right-hand side) for
!   conformation and initialisation of vectors with zero stress
    call create ( problem_c, sol_c, sol_cm1, rhs_c )

    if ( logc == 0 ) then ! standard
      sol_c(1,1)%u = 1   ! initial cxx
      sol_c(2,1)%u = 0   ! initial cxy
      sol_c(3,1)%u = 0   ! initial cxz
      sol_c(4,1)%u = 1   ! initial cyy
      sol_c(5,1)%u = 0   ! initial cyz
      sol_c(6,1)%u = 1   ! initial czz
    else if ( logc == 1 ) then ! log scheme
      sol_c(1,1)%u = 0   ! initial sxx
      sol_c(2,1)%u = 0   ! initial sxy
      sol_c(3,1)%u = 0   ! initial sxz
      sol_c(4,1)%u = 0   ! initial syy
      sol_c(5,1)%u = 0   ! initial syz
      sol_c(6,1)%u = 0   ! initial szz
    end if

!   Creation of system matrix for conformation problem
...
!   Creation and inizialisation of system vectors (solution and right-hand side)
!   for exp(s) projection
    ...
    sol_c_proj(3,1)%u = 0   ! initial cxz
    sol_c_proj(4,1)%u = 1   ! initial cyy
    sol_c_proj(5,1)%u = 0   ! initial cyz
    sol_c_proj(6,1)%u = 1   ! initial czz
...
!   create meshes at t_n and t_nm1 and update the old coordinates
    ...
    meshcoor_n(:,3) = sol_proj(3)%u
    meshcoor_nm1(:,1) = sol_proj(4)%u
    meshcoor_nm1(:,2) = sol_proj(5)%u
    meshcoor_nm1(:,3) = sol_proj(6)%u
!   update sol at t_n
    ...
    sol%u(velx%s) = sol_proj(7)%u
    sol%u(vely%s) = sol_proj(8)%u
    sol%u(velz%s) = sol_proj(9)%u
...
!   transfer the conformation tensor at n
    do i = 1, ncomp_P1/2
      call transfer_data ( mesh_temp, problem_c_temp, &
        sysvector1=sol_c_temp(i,1), vector2=vec_proj, physq1=[1], degfd2=[i] )
    end do

!   transfer the conformation tensor at n-1
    do i = 1, ncomp_P1/2
      call transfer_data ( mesh_temp, problem_c_temp, &
        sysvector1=sol_cm1_temp(i,1), vector2=vec_proj, physq1=[1], &
        degfd2=[i+ncomp_P1/2] )
    end do
...
!   problem definition for projection
    ...
    input_probdef_proj%elementdof(2)%a = [1,0,1,0,1,0,0,0,0,1]
    call problem_definition ( input_probdef_proj, mesh, problem_proj )
...
!   create meshes at t_n and t_nm1 and update the old coordinates
    ...
    do i = 1, ncomp_P1/2
      sol_c(i,1)%u(cc%s) = sol_proj(i)%u(cc_proj%s)
      sol_cm1(i,1)%u(cc%s) = sol_proj(i+ncomp_P1/2)%u(cc_proj%s)
    end do
