...
!   subscript for extracting velocities, pressures and tensors

    physqvel = coefficients(1)%i(6)

    call create_subscript ( mesh, problem, ssvel_ext, groups=[1], &
      physqarr=[physqvel] )
    call create_subscript ( mesh, problem, ssprs_ext, groups=[1], vec=3 )
    call create_subscript ( mesh, problem, ssten_ext, groups=[1], vec=4 )
    call create_subscript ( mesh, problem, ssvel_int, groups=[2], &
      physqarr=[physqvel] )
    call create_subscript ( mesh, problem, ssprs_int, groups=[2], vec=3 )
    call create_subscript ( mesh, problem, ssten_int, groups=[2], vec=4 )
...
!   adjust the correct tensor size for the axisymmetric case
    if ( coefficients(1)%i(23) == 1 ) then
      axs = 1
    else
      axs = 0
    end if
...
!   problem definition in external fluid for plotting
    ...
    input_probdef_ext%vec_elementdof(1)%a(:,3) = ndim*(ndim+1)/2+axs  ! sym. ten
...
!   problem definition in internal fluid for plotting
    ...
    input_probdef_int%vec_elementdof(1)%a(:,3) = ndim*(ndim+1)/2+axs  ! sym. ten
