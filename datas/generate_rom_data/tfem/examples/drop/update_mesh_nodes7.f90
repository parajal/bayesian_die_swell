! update mesh routine for drop7

module update_mesh_nodes7_m
...
!   local definitions
    ...
    type(subscript_t), dimension(size(disp)) :: disp5, disp6
...
!     problem definition
      ...
      call define_essential ( mesh, input_probdef, curve1=1, curve2=4+2*nobj )
...
!   creation of subscripts for interface displacement
    ...
    do j = 1, nobj
      call create_subscript ( mesh, problem, disp5(j), curves=[4+j] )
      call create_subscript ( mesh, problem, disp6(j), curves=[4+nobj+j] )
    end do
...
!     fill solution vector with essential boundary conditions
      ...
      do j = 1, nobj
        sol(i)%u(disp5(j)%s) = disp(j)%a(:,i)
        sol(i)%u(disp6(j)%s) = disp(j)%a(:,i)
      end do
...
end module update_mesh_nodes7_m
