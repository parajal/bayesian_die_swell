...
! variables
  ...
  integer ::  &
    ...
    timeint_sc=1,        & ! time integration scheme for surfactant
                           ! concentration
  ...
  real(dp) ::  &
    ...
    Dcoef = 1.0_dp,       & ! surfactant concentration diffusion coefficient
...
! some more misc variables
  ...
  logical :: w1, w2, euler_step, euler_step_sc
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ periodic, periodic_channel, flowrate, move_mesh_x, &
    ...
    ctime, restart, restart_every, timeint, timeint_sc, numtimesteps, &
    ox, oy, oz, lx, ly, lz, xc, rc, fac_min, fac_max, eta_e, eta_i, &
    beta_st, gammac, c0, Dcoef, factor, U_avg, rs_up, is_up, deltat, beta, &
...
! Velocity/pressure problem definition in external and internal fluid
  ...
  input_probdef%vec_elementdof(1)%a(:,5) = 1         ! surfactant concentration
...
!   Surface tension
    ...
    call add_boundary_elements ( mesh, problem, rhsd, &
      elemsub=surface_tension_surfactants, surface=7, &
      coefficients=coefficients(1), physq=[physqvel], oldvectors=oldvectors_c )
...
!   update mesh nodes
    ...
    mesh_c%coor = mesh%coor(mesh%surfaces(7)%nodes,:)
...
!   save surfactant concentration
    ...
    allocate ( c_temp(mesh%surfaces(7)%nnodes ) )
    c_temp = sol_c%u
...
!  Creation of system vectors (solution and right-hand side)
   ...
   if ( timeint_sc == 2 ) call create_sysvector ( problem_c, sol_cm1 )
