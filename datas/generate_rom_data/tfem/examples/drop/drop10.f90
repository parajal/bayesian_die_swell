...
!
! Similar to drop6, but extended with inertia.
...
! constants
  ...
  integer, parameter ::  &
    ...
    ncomp_P2 = 12          ! number of components for P2 projection:
...
! variables
  ...
  integer ::  &
    ...
    restart_every = 100, & ! write restart file every restart_every steps.
                           ! -1: means none
    timeint=1,           & ! time integration scheme for the interface.
    ...
    maxiter = 100,       & ! maximum number of iterations allowed to obtain
                           ! convergence
    num_euler = 1          ! number of Euler time steps (at least one) for the
                           ! inertia terms.
  ...
  real(dp) ::  &
    ...
    rho_e = 1.0_dp,      & ! external fluid density
    rho_i = 1.0_dp,      & ! internal fluid density
...
! flow problem
  ...
  type(subscriptvec_t) :: meshvelx, meshvely, meshvelz
...
! some more misc variables
  ...
  integer :: vertices(4)=[1,3,5,10], iter
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ periodic, periodic_channel, flowrate, move_mesh_x, &
    ...
    gammac, factor, U_avg, startup_time, rs_up, is_up, deltat, beta, &
    volume_threshold, aspect_ratio_threshold, rho_e, rho_i
...
!       periodical condition on velocities in x-direction
        ...
        call build_system_constraint ( mesh, problem, sysmatrix, rhsd, &
          constraint1=c_perx, elemsub=stokes_constr_node_conn, &
          addmatvec=.true. )
...
!   Create mesh velocity vectors
    ...
    call create_subscript_vector ( mesh, problem, meshvelz, vec=1, degfd=3 )
...
!  delete the temporary definitions
 ...
 subroutine delete_old_problems
   ...
   call delete ( meshvel )
   call delete ( meshvelx, meshvely, meshvelz )
...
!  transfer the coordinates at t_n
   call transfer_data ( mesh_temp, problem_temp, &
     vector1=coords_n_temp, vector2=vec_proj, degfd2=[1,2,3] )

!  transfer the velocities at t_n
   call transfer_data ( mesh_temp, problem_temp, &
     sysvector1=sol_n_temp, vector2=vec_proj, physq1=[1], degfd1=[1,2,3], &
     degfd2=[4,5,6] )

!  transfer the coordinates at t_nm1
   call transfer_data ( mesh_temp, problem_temp, &
     vector1=coords_nm1_temp, vector2=vec_proj, degfd2=[7,8,9] )

!  transfer the velocities at t_nm1
   call transfer_data ( mesh_temp, problem_temp, &
     sysvector1=sol_nm1_temp, vector2=vec_proj, physq1=[1], degfd1=[1,2,3], &
     degfd2=[10,11,12] )
...
!  old coordinates
   ...
   meshcoor_nm1(:,1) = sol_proj(7)%u
   meshcoor_nm1(:,2) = sol_proj(8)%u
   meshcoor_nm1(:,3) = sol_proj(9)%u

!  update the old velocities
   sol_n%u(velx%s) = sol_proj(4)%u
   sol_n%u(vely%s) = sol_proj(5)%u
   sol_n%u(velz%s) = sol_proj(6)%u
   sol_nm1%u(velx%s) = sol_proj(10)%u
   sol_nm1%u(vely%s) = sol_proj(11)%u
   sol_nm1%u(velz%s) = sol_proj(12)%u
