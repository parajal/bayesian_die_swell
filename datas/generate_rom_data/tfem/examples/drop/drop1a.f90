...
! Optionally compute the area tensor.
...
! variables
  ...
  logical ::     &
    ...
    compute_area_tensor = .false., & ! Compute area tensor
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ periodic, flowrate, move_mesh_x, mesh_plot, &
    ...
    rs_up, is_up, deltat, beta, compute_area_tensor
...
! External fluid (element group 1)
  ...
  coefficients(1)%i(59) = 1 ! use nn-I/ndim for area tensor
...
!   area tensor
    ...
    if ( compute_area_tensor ) then

!     compute area tensor

      call integrate_boundary_elements ( mesh, problem, resultsum=q, &
        elemsub=stokes_integrate_area_tensor, &
        curve=5, coefficients=coefficients(1) )

      time = time0 + step * deltat

      write(unit=14,fmt=*) time, q
...
!   write data for restart
  ...
  if ( compute_area_tensor ) close(unit=14)
...
!       open area tensor file

        if ( ctime ) then
          open ( unit=14, file='area_tensor.out', recl=300, status='old', &
            position='append' )
        else
          open ( unit=14, file='area_tensor.out', recl=300, status='unknown' )
        end if
...
!       open area tensor file
        ...
        open ( unit=14, file='area_tensor.out', recl=300 )
