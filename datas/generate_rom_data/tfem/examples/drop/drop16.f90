! Axisymetric drop consisting of a Newtonian fluid suspended in a Newtonian
! fluid with complex interfacial rheology as a boundary condition.
! Lagrangian description of the interface.
! Note in this example (x,y) coordinates should be read as (z,r).
...
program drop16
  ...
  use interfacial_rheology_elements_2D_curve_m
...
! variables
  ...
  logical ::     &
    ...
    mansol = .false.,       & ! apply method of manufacturing solutions
    compute_error = .false.,& ! Compute error. Only makes sense for
                              ! mansol=.true. or eta_e=eta_i
...
! interface tracking problem
  ...
  type(vector_t) :: xnp1, error, pressure
...
! some more misc variables
  ...
  real(dp) :: dropvolume(1), L2norm(2), intpress(1)
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ periodic, flowrate, move_mesh_x, monitor_func, mesh_plot, &
    compute_error, compute_volume, remesh, mansol, n, nc, gauss, gaussb, &
    choice_um, vfuncnr, vtkevery, ctime, restart, restart_every, timeint, &
    numtimesteps, ox, oy, lx, ly, xc, rc, fac_min, fac_max, eta_e, eta_i, &
    gammac, factor, U_avg, rs_up, is_up, deltat, beta, area_threshold, &
    aspect_ratio_threshold, imodel, surfimpl, muc, kappac, Gc, Kc
...
! Interface tracking
  ...
  coefficients_b%i(10) = 0 ! tangential grid velocity using Poisson problem
...
!   output L2-norm of pressure and velocity in drop
    ...
    write(unit=15,fmt=*) time0+step*deltat, sqrt(L2norm/dropvolume(1))
...
!   generate external and internal mesh
    ...
    call execute_command_line ( 'gmsh -2 -format msh2 -o mesh_conform.msh &
...
!   Surface tension and interfacial rheology
    ...
    call add_boundary_elements ( mesh, problem, rhsd, &
      elemsub=interfacial_rheology_curve, &
      sysmatrix=sysmatrix, curve=2, coefficients=coefficients_i, &
      oldvectors=oldvectors, physq=[physqvel] )

!   Cancel jump in traction up to numerical errors
    if ( mansol ) then
      call add_boundary_elements ( mesh, problem, rhsd, &
        elemsub=interfacial_rheology_curve_mansol, curve=2, &
...
!   create pressure vector for post processing

    call create_vector ( problem, pressure, vec=3 )

!   output pressure on drop symmetry line and interface

    call derive_vector ( mesh, problem, pressure, elemsub=stokes_pressure, &
      mcoefficients=coefficients, oldvectors=oldvectors )

    write(filename,'(a,i4.4,a)') 'pressure_int', post, '.out'
    call printtofile ( mesh, problem, filename=filename, curve=7, &
      vector=pressure )
...
!     open L2-norm of pressure and velocity file
      ...
      if ( ctime ) then
        open ( unit=15, file='L2norm.out', recl=300, status='old', &
          position='append' )
      else
        open ( unit=15, file='L2norm.out', recl=300, status='unknown' )
      end if
...
!     open L2-norm of pressure and velocity file
      ...
      open ( unit=15, file='L2norm.out', recl=300 )
...
!   save coordinates of the boundary mesh
    ...
    call execute_command_line ( 'gmsh -2 -bin -format msh2 -o &
...
! Cancel jump in traction for method of manufactured solutions.
  ...
  subroutine interfacial_rheology_curve_mansol ( mesh, problem, curve, elem, &
...
!     allocate arrays
      ...
      allocate ( dphi(ninti,ndf,1), dxdxis(ninti,ndim,ndim-1) )
      allocate ( rhsf(ninti,ndim), work(ninti) )
...
!   surface tension coefficient
    ...
    if ( matrix ) then
      write(*,'(/3(a/))') 'Error in interfacial_rheology_curve_mansol:', &
...
!       determine exact position
        err = func_error_mansol_2D ( vfuncnr, xg(j,:) )
        xe(j,:) = xg(j,:) + err * xg(j,:) / sqrt ( xg(j,1)**2 + xg(j,2)**2 )
!       surface tension term
        rhsf(j,:) = gammac * divsIs2D ( xe(j,:) )
      end do
...
!     constant surface tension
      ...
      case ( 1,3,5 ) ! Boussinesq-Scriven or Kelvin-Voigt
        do j = 1, ninti
          rhsf(j,:) = muc * divsDs2D ( xe(j,:) ) + &
            ( kappac - muc ) * divsdivsuIs2D ( xe(j,:) )
        end do
        do j = 1, ndim
          do N = 1, ndf
            work = phi(:,N) * rhsf(:,j)
            tmp(N,j) = tmp(N,j) + sum ( work * curvel * wg )
          end do
        end do
      case ( 0,2,4 ) ! Constant surface tension or elastic model
      case default
        write(*,'(a,a,i0)') 'Error in interfacial_rheology_curve_mansol: ', &
      ...
      case ( 2:5 ) ! Elastic or Kelvin-Voigt
        do j = 1, ninti
          rhsf(j,:) = Kc * divstauhyd2D ( imodel, xe(j,:) ) + &
                      Gc * divstaudev2D ( imodel, xe(j,:) )
        end do
        do j = 1, ndim
          do N = 1, ndf
            work = phi(:,N) * rhsf(:,j)
...
!     last element on this surface
  ...
  end subroutine interfacial_rheology_curve_mansol
