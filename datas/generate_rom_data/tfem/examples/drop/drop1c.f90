...
! Similar to drop1 but now:
!   P1isoP2 for velocities
!   P1isoP2 line elements for position of the interface.
...
! variables flow problem

  integer ::  &
    uintpl = 9,          & ! P1isoP2 velocity interpolation
!    uintpl = 6,          & ! P2 velocity interpolation
    ...
    gauss  = 3,          & ! Gauss integration for triangular elements
    nsubint = 2,         & ! number of subintegration domains
!    gauss  = 6,          & ! Gauss integration for triangular elements
!    nsubint = 0,         & ! single subintegration domain
    gaussb = 2,          & ! number of integration points of line elements
    nsubintb = 2           ! number of subintegration domains
!    gaussb = 3             ! number of integration points of line elements
!    nsubintb = 0           ! single subintegration domain

! variables interface tracking

  integer :: &
    xintpl = 9,      & ! P1isoP2 position
!    xintpl = 6,      & ! P2 position
    vintpl = 9,      & ! P1isoP2 velocity
!    vintpl = 6,      & ! P2 velocity
!    vintpl = 0,      & ! velocity the same as position
    gauss_it = 2,    & ! Gauss for interface tracking
    nsubint_it = 2     ! number of subintegration domains
!    gauss_it = 3,    & ! Gauss for interface tracking
!    nsubint_it = 0     ! single subintegration domain
...
! External fluid (element group 1)
  ...
  [ uintpl,   pintpl,     0,     0,         0,  &
    ...
    gaussb ]
  coefficients(1)%i(40) = 0 ! standard Gauss-Legendre

  coefficients(1)%r = 0
  coefficients(1)%r(1) = eta_e
  coefficients(1)%r(6) = U_avg * ly
  coefficients(1)%r(19) = gammac
  coefficients(1)%i(32) = nsubint
  coefficients(1)%i(33) = nsubintb
...
! Interface
  ...
  coefficients_b%i(3) = gauss_it
  coefficients_b%i(4) = nsubint_it
  coefficients_b%i(5) = 0  ! standard Gauss-Legendre
...
!   generate fluid mesh using gmsh
    ...
    if ( uintpl == 9 ) then
!     change elshape to 32, 33 for isoparametric mapping
!     Only needed for finding refcoors in object sampling
      mesh%curves(:)%element%elshape = 32
      mesh%element(:)%elshape = 33
    end if
