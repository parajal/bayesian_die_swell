...
! interface meshes itself are not modified.
...
! variables
  ...
  real(dp) ::  &
    ...
    rc = 0.1_dp,          & ! radius of the initial circles
...
! some more misc variables
  ...
  integer :: nnodes, nelem, elshape, step, post, i, c_cpl(nobj), c_per, c_flr, j
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ periodic, flowrate, move_mesh_x, monitor_func, &
    mesh_plot, compute_volume, n, nc, gauss, gaussb, choice_um, vfuncnr, &
    vtkevery, ctime, restart, restart_every, timeint, numtimesteps, &
    ox, oy, lx, ly, rc, fac_min, fac_max, eta_e, eta_i, gammac, &
    factor, U_avg, rs_up, is_up, deltat, beta, area_threshold, &
...
! generate interface meshes from the fluid mesh

  do j = 1, nobj

    nnodes = mesh%curves(4+j)%nnodes
    nelem = mesh%curves(4+j)%nelem
    elshape = mesh%curves(4+j)%element%elshape

    call mesh_skeleton ( mesh_b(j), nnodes, nelem, elshape, ndim )

    mesh_b(j)%topology(1)%a=mesh%curves(4+j)%topology(:,:,1)
    mesh_b(j)%coor=mesh%coor(mesh%curves(4+j)%nodes,:)
...
! Internal fluid - external fluid coupling through Lagrangian multipliers
  ...
  do j = 1, nobj
    call define_constraint ( mesh, input_probdef, curve1=4+j, curve2=4+nobj+j, &
      discretization='collocation', physq=physqvel, num=c_cpl(j) )
  end do
...
!   composite curves for the interfaces

!    call add_to_mesh ( mesh, curve=[(i,i=5,4+nobj)] ) ! curve 5+2*nobj
!    call add_to_mesh ( mesh, curve=[(i,i=5+nobj,4+2*nobj)] ) ! curve 6+2*nobj
...
!   Surface tension
    ...
    do j = 1, nobj
      call add_boundary_elements ( mesh, problem, rhsd, &
        elemsub=surface_tension_curve, curve=4+j, &
...
!       Curvature problem on interface mesh
        ...
        call curvature_gd_ma57 ( mesh_b(j), problem_curv(j), coefficients_b, &
          sysmatrix_curv(j), curvature=f_monitor(j)%a )

        f_monitor(j)%a = dx_part / rc / f_monitor(j)%a

        dx_min = fac_min * dx_part
        dx_max = fac_max * dx_part

        f_monitor(j)%a = min ( dx_max, max(f_monitor(j)%a,dx_min) )
...
!   mesh coordinates
    ...
    do j = 1, nobj
      disp(j)%a = mesh_b(j)%coor - mesh%coor(mesh%curves(4+j)%nodes,:)
    end do
...
!   subscript for extracting velocities from mesh nodes on interface
    ...
    do j = 1, nobj
      call create_subscript ( mesh, problem, ssvel_b(j), curves=[4+j], &
        physqarr=[physqvel] )
    end do
...
! Problem definition of interface system, sysmatrix and vectors
  ...
  subroutine problem_definition_interface_create_sysmatrix_vectors
    ...
    do j = 1, nobj
      ...
      call create_input_probdef ( mesh_b(j), input_probdef_b(j), nvec=1 )

      input_probdef_b(j)%elementdof(1)%a = 1
      input_probdef_b(j)%vec_elementdof(1)%a(:,1) = ndim  ! 2D vector
...
!   save coordinates of the boundary mesh
    ...
    do j = 1, nobj
      allocate ( coor_boun(j)%a(mesh%curves(4+j)%nnodes,mesh%ndim) )
      coor_boun(j)%a = mesh%coor(mesh%curves(4+j)%nodes,:)
    end do

    call execute_command_line ( 'gmsh -2 -bin -o mesh_conform.msh &
      &mesh_conform.geo &
                   & > outputmesh_conform.out' )
...
!  element shape).
   ...
   do j = 1, nobj
     mesh%coor(mesh%curves(4+j)%nodes,:) = coor_boun(j)%a
     mesh%coor(mesh%curves(4+nobj+j)%nodes,:) = coor_boun(j)%a
   end do
