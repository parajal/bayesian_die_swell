...
! Similar to drop2 but now:
!   P1isoP2 for velocities
!   P1isoP2 elements for position of the interface.
...
!    uintpl = 6,          & ! P2 velocity interpolation
    ...
    gauss  = 4,          & ! Gauss integration for tetrahedral elements
    nsubint = 2,         & ! number of subintegration domains
!    gauss  = 6,          & ! Gauss integration for tetrahedral elements
...
!   generate fluid mesh using gmsh
    ...
    if ( uintpl == 9 ) then
!     change elshape to 35 for isoparametric mapping
!     Only needed for finding refcoors in object sampling
      mesh%surfaces(:)%element%elshape = 33
      mesh%element(:)%elshape = 35
    end if
