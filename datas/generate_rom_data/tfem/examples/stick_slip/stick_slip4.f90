...
! 1) An open boundary at the entry of the domain on the left.
...
! second-order Gear time integration/conformation prediction
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ timeint1, timeint2, numtimesteps, logc, eta_s, eta_p, &
    lambda, mobility, deltat, beta, htype, Uscaling, &
    rs_gup, is_gup, rs_c, is_c, U_avg, restart, R
...
! outflow
  call define_essential ( mesh, input_probdef, curve1=6, physq=2, degfd=[0,1] )
! slip surface
  call define_essential ( mesh, input_probdef, curve1=7, physq=2, degfd=[0,1] )
! wall
...
! create system vectors (solution, old solution and right-hand side)
...
! restart: read solution from file
  ...
  if ( restart >= 1 ) then
    ...
    read(10) sol%u, solm1%u
    read(10) (solc(i,1)%u, i=1,ncompc)
    read(10) (solcm1(i,1)%u, i=1,ncompc)

    close(unit=10)

    if ( restart == 1 ) coefficients%i(22) = timeint2 ! first time step 2nd order
...
!   open boundary
    ...
    call build_system ( mesh, problem, sysmatrix, rhsd, &
      ...
      object=1, addmatvec=.true., oldvectors=oldvectors_ve, &
