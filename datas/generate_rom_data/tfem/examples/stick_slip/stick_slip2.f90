...
! 1) An internal periodic boundary at the entry of the domain on the left.
...
! open_curve = 4 (at inflow) or open_curve=14 (internal curve) for open integrals
! for the internal boundary condition
  integer :: open_curve = 14

  logical :: weak_v = .true. ! weak connection of velocities or collocation

! namelist for input of variables; read from standard input

  namelist /comppar/ timeint, numtimesteps, logc, eta_s, eta_p, &
    lambda, mobility, deltat, beta, htype, Uscaling, &
    rs_gup, is_gup, rs_c, is_c, U_avg, restart, R, weak_v, &
    open_curve
...
! read mesh
  ...
  call add_to_mesh ( mesh, object='curve', objectcurve=open_curve, topology=.true., &
    intrule=3 )

  !mesh%objects(1)%coor(:,1) = mesh%objects(1)%coor(:,1) + 1e-7  ! shifts a bit to the right for intersection
...
! center line
  if ( weak_v ) then
    ...
    call define_essential ( mesh, input_probdef, curve1=13, physq=2, degfd=[0,1] , exclude=1 )
  end if
! outflow
...
! velocities
  if ( weak_v) then
    call define_constraint ( mesh, input_probdef, &
      physq=2, curve1=14, curve2=4, discretization='weak', elementdof=[2,0,2] )
  else
    call define_constraint ( mesh, input_probdef, &
      physq=2, curve1=14, curve2=4, discretization='collocation', exclude=1 )  !  exclude boundary point
  end if
...
! constraint for periodical boundary conditions of the conformation
  call define_constraint ( mesh, input_probdefc, curve1=14, curve2=4, &
...
! internal boundary
  ...
  call build_system(mesh, problem, sysmatrix, rhsd, &
    elemsub1=stokes_open_boundary, physqrow=[2,3], physqcol=[2,3], &
...
! periodical condition on velocities and gradients
  ...
  if ( weak_v ) then
    ...
    call build_system_constraint ( mesh, problem, sysmatrix, rhsd, &
      constraint1=2, elemsub=stokes_constr_node_conn, &
      addmatvec=.true., coefficients=coefficients )
  end if
...
!   internal boundary
    ...
    call add_boundary_elements ( mesh, problem, rhsd, &
      elemsub=rhs_tau_n_curve, curve=open_curve, coefficients=coefficients, &
      oldvectors=oldvectors_ve, physq=[2] )
