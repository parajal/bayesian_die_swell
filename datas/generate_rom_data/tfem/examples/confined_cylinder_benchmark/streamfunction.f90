! streamfunction problem for the flow around a cylinder
...
! build (assemble) matrix and vector from elements
  ...
  call add_boundary_elements ( mesh, problem, rhsd, curve=18, &
    elemsub=streamfunction_natboun_curve, coefficients=coefficients, &
    oldvectors=oldvectors )

  call add_boundary_elements ( mesh, problem, rhsd, curve=4, &
    elemsub=streamfunction_natboun_curve, coefficients=coefficients, &
    oldvectors=oldvectors )

  call add_boundary_elements ( mesh, problem, rhsd, curve=23, &
    elemsub=streamfunction_natboun_curve, coefficients=coefficients, &
    oldvectors=oldvectors )
  ...
  call plot_color_contour ( plot_options, mesh, problem, &
    'streamfunction.fig', sysvector=sol )

  call write_scalar_vtk ( mesh, problem, sysvector=sol, &
    dataname='streamfunction', filename='cylinder.vtk', append=.true. )
