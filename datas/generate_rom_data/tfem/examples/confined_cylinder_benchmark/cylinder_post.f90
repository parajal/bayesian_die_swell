...
! definitions
  ...
  type(vector_t) :: dudx, dudy, dvdx, dvdy
...
! velocity vector
  ...
  call printtofile ( mesh, problem, filename='vx_cross_section.out', &
    curve=9, vector=velocity )

  call printtofile ( mesh, problem, filename='vy_cross_section.out', &
    curve=9, vector=velocity )
...
! write binary file for reading by streamfunction
  ...
  call write_vector_vtk ( mesh, problem, filename='cylinder.vtk', &
    dataname='velocity_vector', vector=velocity )
...
! pressure
  ...
  call write_scalar_vtk ( mesh, problem, vector=pressure, dataname='pressure', &
    filename='cylinder.vtk', append=.true. )
...
! vorticity
  ...
  call printtofile ( mesh, problem, filename='vorticity_cross_section.out', &
    curve=9, vector=vorticity )

  call write_scalar_vtk ( mesh, problem, vector=vorticity, &
    dataname='vorticity', filename='cylinder.vtk', append=.true. )

  call delete ( vorticity )

! gradients

  call create_vector ( problem, dudx, vec=4 )
  call create_vector ( problem, dudy, vec=4 )
  call create_vector ( problem, dvdx, vec=4 )
  call create_vector ( problem, dvdy, vec=4 )

  coefficients%i(13)=1
  call derive_vector ( mesh, problem, dudx, elemsub=stokes_deriv, &
    coefficients=coefficients, oldvectors=oldvectors_d )
  coefficients%i(13)=2
  call derive_vector ( mesh, problem, dudy, elemsub=stokes_deriv, &
    coefficients=coefficients, oldvectors=oldvectors_d )
  coefficients%i(13)=3
  call derive_vector ( mesh, problem, dvdx, elemsub=stokes_deriv, &
    coefficients=coefficients, oldvectors=oldvectors_d )
  coefficients%i(13)=4
  call derive_vector ( mesh, problem, dvdy, elemsub=stokes_deriv, &
    coefficients=coefficients, oldvectors=oldvectors_d )

  call plot_color_contour ( plot_options, mesh, problem, &
    'dudx_contour.fig', vector=dudx )
  call plot_color_contour ( plot_options, mesh, problem, &
    'dudy_contour.fig', vector=dudy )
  call plot_color_contour ( plot_options, mesh, problem, &
    'dvdx_contour.fig', vector=dvdx )
  call plot_color_contour ( plot_options, mesh, problem, &
    'dvdy_contour.fig', vector=dvdy )

  call printtofile ( mesh, problem, filename='dudx_on_centerline.out', &
    curve=22, vector=dudx )

  call printtofile ( mesh, problem, filename='dudy_cross_section.out', &
    curve=9, vector=dudy )

  call write_scalar_vtk ( mesh, problem, vector=dudx, dataname='dudx', &
    filename='cylinder.vtk', append=.true. )
  call write_scalar_vtk ( mesh, problem, vector=dudy, dataname='dudy', &
    filename='cylinder.vtk', append=.true. )
  call write_scalar_vtk ( mesh, problem, vector=dvdx, dataname='dvdx', &
    filename='cylinder.vtk', append=.true. )
  call write_scalar_vtk ( mesh, problem, vector=dvdy, dataname='dvdy', &
    filename='cylinder.vtk', append=.true. )

  call delete ( dudx, dudy, dvdx, dvdy )
