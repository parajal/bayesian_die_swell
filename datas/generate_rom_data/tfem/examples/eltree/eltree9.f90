! Example program for using the eltree module for triangles.
...
! constants
  ...
  integer, parameter :: &
    ...
    gauss_ve    = 6     ! Gauss integration rule for the volume elements
...
! read mesh
  ...
  call read_mesh_gmsh ( mesh, filename='mesh9.msh', ndim=2 )

  call fill_mesh_parts ( mesh )

  call write_mesh_vtk ( mesh, filename='mesh9.vtk' )
...
! integrate on volume of the circle
...
! analytical solutions
  ...
  areae = pi * radius**2
  intge = pi/4 * radius**4 !21 + 1/3

  print *, 'computed area = ', resultsum(1)
...
!   plot the interface mesh (double nodes removed)
    ...
    call write_mesh_vtk ( mesh2, filename='imesh9.vtk' )
...
!   plot the eltrees
    ...
    call write_mesh_vtk ( mesh_plot, filename='mesh_eltree9.vtk' )
    call delete(mesh_plot)
