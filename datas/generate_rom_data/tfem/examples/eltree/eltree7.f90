! Example program for using the eltree module for triangles and tetrahedra
! Use eltree to subdivide the region and integrate over the interface of a
! circle.
...
! constants
  ...
  integer, parameter :: &
    numsplit    = 3, &  ! relative size of all subelements will be at least
                        ! as small as 1/2^numsplitmin
    numsplitmin = 1, &  ! relative size of all subelements will be at least
                        ! as small as 1/2^numsplitmin
    uintpl      = 2, &  ! P1 interpolation
    gauss_ie    = 3     ! Gauss integration rule for the interface elements
...
! mesh
  ...
  call read_mesh_gmsh ( mesh, filename='mesh7.msh', ndim=2 )

  call fill_mesh_parts ( mesh )

  call write_mesh_vtk ( mesh, filename='mesh7.vtk' )
...
! analytical solutions
  ...
  print *,  ''
  print *, 'computed normal integral circle = ', resultsum(3)
  print *, '                                  ', resultsum(4)
...
!   plot the interface mesh (double nodes removed)
    ...
    call add_to_mesh ( mesh1, nodblocks=[4,4] )
    call mesh_convert ( mesh1, mesh2, remove_double_nodes=.true., &
      warn=.false. )
    call fill_mesh_parts ( mesh2 )
    call write_mesh_vtk ( mesh2, filename='imesh7.vtk' )
...
!   plot the eltrees
    ...
    call write_mesh_vtk ( mesh_plot, filename='mesh_eltree7.vtk' )
    call delete(mesh_plot)
...
!       NOTE: for triangles, eltype = 1 and a configuration must be given
!             for the root element: conf = 1 and the coordinates:
...
!             for more info see eltree_t!
        ...
        call subdivide ( eltree_array(1,elem)%p, levelset=levelset, &
          ...
          numsplitmin=numsplitmin, mapcoor=mapcoor_triangle, &
...
!       element crosses the interface (triangle with ref vol 1/2).
        vol(elem) = volume ( eltree_array(1,elem)%p, lsign=[1] ) * 2
