...
! steady state solution
  ...
  do m = 1, nmodes
    if ( logc == 0 ) then ! standard
      call random_number (solc(1,m)%u)
      solc(1,m)%u = csteady(1,m) + 1e-3*solc(1,m)%u  ! initial cxx
      call random_number (solc(2,m)%u)
      solc(2,m)%u = csteady(2,m) + 1e-3*solc(2,m)%u  ! initial cxy
      call random_number (solc(3,m)%u)
      solc(3,m)%u = csteady(3,m) + 1e-3*solc(3,m)%u   ! initial cyy
    else if ( logc == 1 ) then ! log scheme
      call eig2x2 ( csteady(:,m), lsteady, eigv )
      lsteady = log ( lsteady )
      call inveig2x2 ( csteady(:,m), lsteady, eigv )
      call random_number (solc(1,m)%u)
      solc(1,m)%u = csteady(1,m) + 1e-3*solc(1,m)%u   ! initial sxx
      call random_number (solc(2,m)%u)
      solc(2,m)%u = csteady(2,m) + 1e-3*solc(2,m)%u   ! initial sxy
      call random_number (solc(3,m)%u)
      solc(3,m)%u = csteady(3,m) + 1e-3*solc(3,m)%u   ! initial syy
    end if
  end do
  do i = 1, 3
...
!   solve conformation and keep LU decomposition in the loop over components
    ...
    do m = 1, nmodes
      do i = 1, ncompc
        call solve_system_ma41 ( sysmatrixc, rhsc(i,m), solc(i,m), luc, &
...
!   write monitor data
    ...
    write(unit=13,fmt=*) &
      ...
      sum( ( solc(1,m)%u(cval%s) - csteady(1,m) )**2 + &
           ( solc(2,m)%u(cval%s) - csteady(2,m) )**2 + &
           ( solc(3,m)%u(cval%s) - csteady(3,m) )**2 ) &
             / 3 / size(cval%s)), &
      minval(solc(1,m)%u(cval%s)), m=1,nmodes ), &
