...
! NOTE: similar to couette2, but omitting DEVSS.
...
! constants
  ...
  integer, parameter :: &
    ...
    ncompg = 4,     & ! number of velocity gradient components
...
! definitions
  ...
  type(vector_t), target :: gradientsn, gradientsnm1
  type(lu_ma57_t) :: lu_g
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ nx, ny, timeint1, timeint2, numtimesteps, logc, eta_s, &
...
! create system vectors for velocity/pressure (solution and right-hand side)

  call create_sysvector ( problem, soln, solnm1, rhsd )

! create gradient vector

  call create ( problem, gradientsn, vec=3 )
  call create ( problem, gradientsnm1, vec=3 )


! fill solution vector with essential boundary conditions

! set velocity = 0 on lower boundary
  call fill_sysvector ( mesh, problem, soln, curve1=1, physq=physqvel, &
    value=0._dp )
! set velocity = (1,0) on upper boundary
  call fill_sysvector ( mesh, problem, soln, curve1=3, physq=physqvel, &
    degfd=1, value=1._dp )
  call fill_sysvector ( mesh, problem, soln, curve1=3, physq=physqvel, &
    degfd=2, value=0._dp )
! set pressure level = 0 in lower left corner
  call fill_sysvector ( mesh, problem, soln, point=1, physq=physqpress, &
    value=0._dp )
...
! solve for gradientsn

  call build_and_solve_proj_grad ( first=.true. )
...
! steady state solution
  ...
  if ( logc == 0 ) then ! standard
    call random_number (solcn(1,1)%u)
    solcn(1,1)%u = csteady(1) + 1e-3*solcn(1,1)%u  ! initial cxx
    call random_number (solcn(2,1)%u)
    solcn(2,1)%u = csteady(2) + 1e-3*solcn(2,1)%u  ! initial cxy
    call random_number (solcn(3,1)%u)
    solcn(3,1)%u = csteady(3) + 1e-3*solcn(3,1)%u   ! initial cyy
  else if ( logc == 1 ) then ! log scheme
    call eig2x2 ( csteady, lsteady, eigv )
    lsteady = log ( lsteady )
    call inveig2x2 ( csteady, lsteady, eigv )
    call random_number (solcn(1,1)%u)
    solcn(1,1)%u = csteady(1) + 1e-3*solcn(1,1)%u   ! initial sxx
    call random_number (solcn(2,1)%u)
    solcn(2,1)%u = csteady(2) + 1e-3*solcn(2,1)%u   ! initial sxy
    call random_number (solcn(3,1)%u)
    solcn(3,1)%u = csteady(3) + 1e-3*solcn(3,1)%u   ! initial syy
  end if
  do i = 1, 3
    solcn(i,1)%u(cvalc2%s) = solcn(i,1)%u(cvalc5%s) ! make disturbance periodic
  end do
...
! store solution vectors and problem structures
  ...
  oldvectors_ve%v(3)%p => gradientsn
  oldvectors_ve%v(4)%p => gradientsnm1
...
!   copy previous velocity/pressure solution to older time step
...
!   copy previous gradients solution to older time step

    call copy ( gradientsn, gradientsnm1 )
...
!   write monitor data
    ...
    write(unit=13,fmt=*) &
      ...
      sum( ( solcn(1,1)%u(cval%s) - csteady(1) )**2 + &
           ( solcn(2,1)%u(cval%s) - csteady(2) )**2 + &
           ( solcn(3,1)%u(cval%s) - csteady(3) )**2 ) / 3 / size(cval%s) ), &
      minval(solcn(1,1)%u(cval%s)), &
...
! delete all data including all allocated memory
  ...
  call delete ( gradientsn, gradientsnm1 )
...
!   solve
    ...
    do i = 1, ncompg
      ...
      call transfer_data ( mesh, problem_grad, problem, sysvector1=sol_grad, &
        vector2=gradientsn, degfd2=[i] )

    end do

  end subroutine build_and_solve_proj_grad

end program couette10
