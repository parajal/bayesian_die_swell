...
! Explicit div tau term.
! linear Couette flow
! DEVSS-G/SUPG
! second-order time integration
! NOTE: This is similar to couette2, but now using the b-formulation.
...
! definitions
  ...
  type(mesh_t) :: mesh
  type(input_probdef_t) :: input_probdef, input_probdefb
  type(problem_t), target :: problem, problemb
  type(sysmatrix_t) :: sysmatrix, sysmatrixb
  ...
  type(sysvector_t), dimension(ncompb,nmodes), target :: solbn, solbnm1
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ nx, ny, timeint1, timeint2, numtimesteps, dist_step, &
    eta_s, eta_p, lambda, deltat, critval_reinit, ampl, beta, &
    rs_gup, is_gup, rs_b, is_b
...
! initialize vectors with steady stress and a small random perturbation.
  ...
  call create ( problemb, solbn, solbnm1, rhsb )
...
! steady state solution
  ...
  if ( any ( bvariant == [1,2] ) ) then
    ...
    solbn(1,1)%u = scsteady(1) ! initial bxx
    solbn(2,1)%u = scsteady(2) ! initial bxy
    solbn(3,1)%u = solbn(2,1)%u ! initial byx
    solbn(4,1)%u = scsteady(3) ! initial byy
  else if ( bvariant == 3 ) then
    scsteady = chol ( csteady )
    solbn(1,1)%u = scsteady(1) ! initial bxx
    solbn(2,1)%u = 0           ! initial bxy
    solbn(3,1)%u = scsteady(2) ! initial byx
    solbn(4,1)%u = scsteady(3) ! initial byy
  else if ( bvariant == 4 ) then
    scsteady = chol ( csteady )
    solbn(1,1)%u = log(scsteady(1)) ! initial bxx
    solbn(2,1)%u = 0                ! initial bxy
    solbn(3,1)%u = scsteady(2)      ! initial byx
    solbn(4,1)%u = log(scsteady(3)) ! initial byy
  end if
  do i = 1, 4
    solbn(i,1)%u(bvalc2%s) = solbn(i,1)%u(bvalc5%s) ! make disturbance periodic
  end do
...
! open monitor file
  ...
  allocate ( tmpar(size(solbn(1,1)%u)) )
...
!   disturbance
    ...
    if ( step == dist_step ) then
      ...
      solbn(1,1)%u = solbn(1,1)%u + ampl*tmpar   ! initial bxx
      call random_number (tmpar)
      solbn(2,1)%u = solbn(2,1)%u + ampl*tmpar   ! initial bxy
      call random_number (tmpar)
      solbn(3,1)%u = solbn(3,1)%u + ampl*tmpar   ! initial bxy
      call random_number (tmpar)
      solbn(4,1)%u = solbn(4,1)%u + ampl*tmpar   ! initial byy
...
!   solve conformation and keep LU decomposition in the loop over components
    ...
    do icomp = 1, ncompb
      call solve_system_ma41 ( sysmatrixb, rhsb(icomp,1), solbn(icomp,1), lub, &
...
!   reinitialize b to b' = sqrt(c)
    ...
    if ( bvariant == 1 ) call reinitialize_b
...
! delete all data including all allocated memory
  ...
  call delete ( lu_u )
  call delete ( solbn, solbnm1, rhsb )
...
!     store b and bn-1 in solution vectors
      ...
      do i = 1,ncompb
        solbn(i,1)%u(bval%s) = b(:,i)
        solbnm1(i,1)%u(bval%s) = bm1(:,i)
      end do
