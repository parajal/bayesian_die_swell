...
! NOTE: This is similar to couette9, but now using the b-formulation.
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ nx, ny, timeint1, timeint2, numtimesteps, dist_step, &
    cproj, eta_s, eta_p, lambda, deltat, critval_reinit, ampl, beta, &
...
! exact steady state solution
  ...
  if ( any ( bvariant == [1,2] ) ) then
    ...
    solb(1,1)%u = scsteady(1) ! initial bxx
    solb(2,1)%u = scsteady(2) ! initial bxy
    solb(3,1)%u = solb(2,1)%u ! initial byx
    solb(4,1)%u = scsteady(3) ! initial byy
  else if ( bvariant == 3 ) then
    scsteady = chol ( csteady )
    solb(1,1)%u = scsteady(1) ! initial bxx
    solb(2,1)%u = 0           ! initial bxy
    solb(3,1)%u = scsteady(2) ! initial byx
    solb(4,1)%u = scsteady(3) ! initial byy
  else if ( bvariant == 4 ) then
    scsteady = chol ( csteady )
    solb(1,1)%u = log(scsteady(1)) ! initial bxx
    solb(2,1)%u = 0                ! initial bxy
    solb(3,1)%u = scsteady(2)      ! initial byx
    solb(4,1)%u = log(scsteady(3)) ! initial byy
  end if
  do i = 1, 4
    solb(i,1)%u(bvalc2%s) = solb(i,1)%u(bvalc5%s) ! make disturbance periodic
  end do
...
! open monitor file
  ...
  allocate ( tmpar(size(solb(1,1)%u)) )
...
!   disturbance
    ...
    if ( step == dist_step ) then
      ...
      solb(1,1)%u = solb(1,1)%u + ampl*tmpar   ! initial bxx
      call random_number (tmpar)
      solb(2,1)%u = solb(2,1)%u + ampl*tmpar   ! initial bxy
      call random_number (tmpar)
      solb(3,1)%u = solb(3,1)%u + ampl*tmpar   ! initial bxy
      call random_number (tmpar)
      solb(4,1)%u = solb(4,1)%u + ampl*tmpar   ! initial byy
