...
! variables
  ...
  integer :: step, i, m
  real(dp) :: alpha, G(nmodes), csteady(3,nmodes)
  real(dp) :: lsteady(2), eigv(2,2)
...
! steady state solution
  ...
  do m = 1, nmodes
    if ( logc == 0 ) then ! standard
      call random_number (solcn(1,m)%u)
      solcn(1,m)%u = csteady(1,m) + 1e-3*solcn(1,m)%u  ! initial cxx
      call random_number (solcn(2,m)%u)
      solcn(2,m)%u = csteady(2,m) + 1e-3*solcn(2,m)%u  ! initial cxy
      call random_number (solcn(3,m)%u)
      solcn(3,m)%u = csteady(3,m) + 1e-3*solcn(3,m)%u   ! initial cyy
    else if ( logc == 1 ) then ! log scheme
      call eig2x2 ( csteady(:,m), lsteady, eigv )
      lsteady = log ( lsteady )
      call inveig2x2 ( csteady(:,m), lsteady, eigv )
      call random_number (solcn(1,m)%u)
      solcn(1,m)%u = csteady(1,m) + 1e-3*solcn(1,m)%u   ! initial sxx
      call random_number (solcn(2,m)%u)
      solcn(2,m)%u = csteady(2,m) + 1e-3*solcn(2,m)%u   ! initial sxy
      call random_number (solcn(3,m)%u)
      solcn(3,m)%u = csteady(3,m) + 1e-3*solcn(3,m)%u   ! initial syy
    end if
  end do
  do i = 1, 3
...
!   write monitor data
    ...
    write(unit=13,fmt=*) &
      ...
      sum( ( solcn(1,m)%u(cval%s) - csteady(1,m) )**2 + &
           ( solcn(2,m)%u(cval%s) - csteady(2,m) )**2 + &
           ( solcn(3,m)%u(cval%s) - csteady(3,m) )**2 ) &
             / 3 / size(cval%s)), &
      minval(solcn(1,m)%u(cval%s)), m=1,nmodes ), &
