! Linear Couette flow for an integral UCM model.
! Problem 1:
...
!   a steady state initial condition for the fields.
...
!    fintpltau = 2,      & ! P2 interpolation in age (tau)
  ...
  real(dp), parameter :: &
    deltat = 5.e-1_dp,   & ! time step
    beta = 1,            & ! SUPG factor
    dtau1 = 3*lambda/nintvaltau, & ! length of first interval in
                                   ! age (tau) direction
    tauc = 15._dp*lambda,   & ! maximum age (cutoff)
...
! definitions
  ...
  type(subscript_t) :: fval, fvals(ninttau)
  ...
  real(dp) :: x(0:nintvaltau), alpha, stress_steady(4)
...
! for post processing the data
  ...
  call create_subscript ( mesh, problem_f, fvals(1), physqarr=[1], degfd=1 )
  call create_subscript ( mesh, problem_f, fvals(2), physqarr=[1], degfd=2 )
...
! initial solution ( steady state shear )

  call dfm_tau_nodal_points ( coefficients, tau_nodal_points )

  do j = 1, nintvaltau
    soln_f(1,j)%u = 1 ! steady F_xx
    soln_f(3,j)%u = 0 ! steady F_yx
    soln_f(4,j)%u = 1 ! steady F_yy
    soln_f(2,j)%u = 0 ! initialize all to 0, including lm's
    do i = 1, ninttau
      soln_f(2,j)%u(fvals(i)%s) = tau_nodal_points(i,j) ! steady F_xy
    end do
  end do
...
! post processing of the stress tensor
  ...
  call copy ( soln_f, sol_f ) ! derive reads from sol_f
...
! steady state stress solution

  stress_steady = stress_tensor%u(1:4)

! add a small random perturbation to the steady state fields

  do j = 1, nintvaltau
    call random_number (soln_f(1,j)%u)
    soln_f(1,j)%u = 1 + 1e-3 * soln_f(1,j)%u ! initial F_xx
    call random_number (soln_f(3,j)%u)
    soln_f(3,j)%u = 0 + 1e-3 * soln_f(3,j)%u ! initial F_yx
    call random_number (soln_f(4,j)%u)
    soln_f(4,j)%u = 1 + 1e-3 * soln_f(4,j)%u ! initial F_yy
    call random_number (soln_f(2,j)%u)
    do i = 1, ninttau
      soln_f(2,j)%u(fvals(i)%s) = tau_nodal_points(i,j) &
                           + 1e-3 * soln_f(2,j)%u(fvals(i)%s) ! initial F_xy
    end do
  end do

  call delete ( stress_tensor )
...
!   write monitor data
    ...
    write(unit=14,fmt=*) &
      ...
      sum( ( sol_s_proj(1)%u - stress_steady(1) )**2 + &
           ( sol_s_proj(2)%u - stress_steady(2) )**2 + &
           ( sol_s_proj(3)%u - stress_steady(3) )**2 + &
           ( sol_s_proj(4)%u - stress_steady(4) )**2 ) / 4 / sol_s_proj(1)%n ),&
      maxval(sol%u(vely%s)), minval(sol%u(vely%s))
...
!   periodical condition on velocities and gradients
    ...
    call build_system_constraint ( mesh, problem, sysmatrix, rhsd, &
      constraint1=1, constraint2=2, elemsub=stokes_constr_node_conn, &
      addmatvec=.true., coefficients=coefficients  )
