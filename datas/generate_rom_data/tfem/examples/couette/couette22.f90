...
! NOTE: Similar to couette15, but now fully implicit
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ printscreen, nx, ny, timeint1, timeint2, &
    numtimesteps, maxnumiterations, dist_step, thetapar, epsconf, eta_s, &
    eta_p, lambda, deltat, critval_reinit, ampl, beta, rs, is
...
! initial solution conformation tensor
  ...
  soln%u(bvalcmp(1,1)%s) = scsteady(1) ! initial bxx
  soln%u(bvalcmp(2,1)%s) = scsteady(2) ! initial bxy
  soln%u(bvalcmp(3,1)%s) = scsteady(2) ! initial byx
  soln%u(bvalcmp(4,1)%s) = scsteady(3) ! initial byy
...
!   disturbance
    ...
    if ( step == dist_step ) then
      ...
      soln%u(bvalcmp(1,1)%s) = &
          soln%u(bvalcmp(1,1)%s) + ampl*tmpar   ! initial bxx
      call random_number (tmpar)
      soln%u(bvalcmp(2,1)%s) = &
          soln%u(bvalcmp(2,1)%s) + ampl*tmpar   ! initial bxy
      call random_number (tmpar)
      soln%u(bvalcmp(3,1)%s) = &
          soln%u(bvalcmp(3,1)%s) + ampl*tmpar   ! initial bxy
      call random_number (tmpar)
      soln%u(bvalcmp(4,1)%s) = &
          soln%u(bvalcmp(4,1)%s) + ampl*tmpar   ! initial byy
