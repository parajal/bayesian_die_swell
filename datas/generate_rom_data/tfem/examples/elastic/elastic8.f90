...
! Optionally use global P1 interpolation for pressure.
...
! constants
  ...
  integer, parameter :: &
    ...
    itermax=20            ! maximum interations in an increment
...
!    epspd(numinc)=(/(1e-4_dp,i=1,numinc-1),1e-12_dp/)
    epspd(numinc)=[(1e-4_dp,i=1,numinc-1),1e-10_dp]
...
! fill coefficients
  ...
  coefficients%i(1:11) = &
    [ uintpl,   pintpl,      model, 0,     0,  &
      ...
      gauss ]
  coefficients%i(41) = global_shape ! mapped or global shape func for pressure
