...
! Sinusoidal time variation of the "Flow Rate"
!--------------------------------------------------------------------------
! <Numerical Approximation>:
! Quadrilateral Taylor-Hood elements for both phases.
! Distributed Langrange multipliers for entire coupling of the two domains.
...
!   2. constants for the mesh generation & geometry definition
    integer, parameter :: &
    nx_f  = 200,          & ! number of elements of fluid domain x-direction
    ny_f  = 40,           & ! number of elements of fluid domain y-direction
    nx_s  = 3,            & ! number of elements of solid domain x-direction
    ny_s  = 14              ! number of elements of solid domain y-direction
...
!   5. constants for Newton-Raphson method
    ...
    real(dp), parameter :: &
    epsdd = 1e-5_dp,       & ! stop criterion for iterative displacement differences
    epspd = 1e-4_dp          ! stop criterion for iterative pressure differences
    real(dp) :: dd, pd
...
!   6. constants for the HSL solver
    real(dp), parameter :: &
    rs_up = 2.0_dp,        & ! real_storage gradient-velocity-pressure LU (HSL)
    is_up = 2.0_dp           ! integer_storage gradient-velocity-pressure LU (HSL)
...
!   8. constants for Langrange multiplier (LM)
    character, parameter :: clc_or_wk = 'w'     !  'w' or 'W' for weak imposition & 'c' or 'C' for collocation method


!   >>>un-comment out one of the following lines for choosing an approximation for the LM.
!    in case of 'collocation' your choice
!    integer :: lintpl_lm = 0,  ndf_lm = 1, elementdof_lm(9) = (/0,0,0,0,0,0,0,0,2/)   ! constant LM at the middle of element
     integer :: lintpl_lm = 1,  ndf_lm = 4, elementdof_lm(9) = [2,0,2,0,2,0,2,0,0]   ! piecewise linear continuous    LM
!    integer :: lintpl_lm = 2,  ndf_lm = 3, elementdof_lm(9) = (/0,0,0,0,0,0,0,0,6/)   ! piecewise linear discontinuous LM
...
!--------------------------------------------------------------------------
!  Module functions_m
!  Contains function definitions for the velocity profile at both
...
!--------------------------------------------------------------------------
   ...
   contains
    ...
    use kind_defs_m
    use common_definitions, only: time, omega
    ...
    flow_profile = sin(time*omega)*(1.0_dp-4.0_dp*(x(2)-0.5_dp)**2)
...
!--------------------------------------------------------------------------
   ...
   use common_definitions, only: deltat, lintpl_lm, ndf_lm
...
!   local variables
    ...
    real(dp) :: psi(1,ndf_lm), x(nodal_pt,ndim)


    real(dp) :: theta_q(1,ndf_xy), dtheta_q(1,ndf_xy,ndim)
    real(dp) ::   detF(1), F(2,ndim,ndim), Finv(1,ndim,ndim)
...
!     constant Lagrange multiplier
      psi = 1
    else if ( lintpl_lm == 1 ) then
!     linear Lagrange multiplier
      call shape_quad_Q1 &
      ( mesh%objects(object)%xig(node:node,1:ndim), psi )
    else if ( lintpl_lm == 2 ) then
!     discontinuous linear Lagrange multiplier
      psi(1,1) = 1.e0_dp
      psi(1,2) = mesh%objects(object)%xig(node,1)
      psi(1,3) = mesh%objects(object)%xig(node,2)
    end if


!   shape function of the quadratic curve at the integration point
    call shape_quad_Q2 &
    ( mesh%objects(object)%xig(node:node,1:ndim), theta_q, dtheta_q(:,:,1:ndim) )


!   compute geometry of element deformed
    call get_coordinates_object &
    ( mesh, elem, x, object )

    call isoparametric_deformation &
    (x, dtheta_q, F, Finv, detF )
...
!--------------------------------------------------------------------------
    ...
    if ( vector ) then
      ...
      do i = 1, ndf_lm
      elemvec(i)         =  &
      psi(1,i) *( di(1) - dn(1) )/deltat *detF(1)* mesh%objects(object)%wg(node)
      elemvec(i+ndf_lm) =  &
      psi(1,i) *( di(2) - dn(2) )/deltat *detF(1)* mesh%objects(object)%wg(node)
      end do
    ...
    if ( matrix ) then
      ...
      do j = 1, ndf
      elemmat(i,j)             =  &
      psi(1,i) *phi(1,j) *detF(1)*mesh%objects(object)%wg(node)
      elemmat(ndf_lm+i,ndf+j) =  &
      psi(1,i) *phi(1,j) *detF(1)*mesh%objects(object)%wg(node)
      end do
      end do
      ...
      do j = 1, ndf
      elemmat2(i,j)             = &
      - psi(1,i) *phi2(1,j)/deltat *detF(1)*mesh%objects(object)%wg(node)
      elemmat2(ndf_lm+i,ndf+j) = &
      - psi(1,i) *phi2(1,j)/deltat *detF(1)*mesh%objects(object)%wg(node)
      end do
      end do
...
!--------------------------------------------------------------------------
    ...
    call create_coefficients&
    ( coefficients_s, ncoefi=200, ncoefr=200 )
...
!     use full mesh
      ...
      call add_to_mesh &
      ( mesh, object='mesh', objectmesh=mesh_s, typeofobject=2, excludegroups=[2], &
        excludegroups2=[1], excludecurves=[1] )
...
!    interfacial boundary curve
     ...
     case('c','C')
      call create_subscript &
      ( mesh, problem, dispobject, physqarr=[1], groups=[2] )
     case('w','W')
      call create_subscript &
      ( mesh, problem, dispobject, physqarr=[1], groups=[2] )
      case default
...
!--------------------------------------------------------------------------
    ...
    write(*,'(/a,i0,a,f10.4/)') 'step = ', time_step, ' time = ', time
    write(*,'(/a,f10.4/)') 'sin term = ', sin(time*omega)
...
!     eliminate prescribed degrees of freedom
      call add_effect_of_essential_to_rhs &
      ( problem, sysmatrix, sol, rhsd )
...
!   extract velocity from solution vector
    call extract_physvector( mesh, problem, sol, velocity )


!   extract velocity components vx, vy from velocity vector
    do i = 1, mesh%nnodes
...
!   plot velocity vector
    ...
    write(filename,'(a,i4.4,a)') 'vx_fd', time_step, '.fig'
    call plot_color_contour &
    ( plot_options, updatemesh, problem, filename=filename, groups=[1], &
      vector=vel_x )

    write(filename,'(a,i4.4,a)') 'vy_fd', time_step, '.fig'
    call plot_color_contour &
    ( plot_options, updatemesh, problem, filename=filename, groups=[1], &
      vector=vel_y )
...
!     calculate pressure in fluid & solid domains & plot it
      call derive_vector &
      ( updatemesh, problem, pressure, elemsub=stokes_pressure, coefficients=coefficients_f, &
        oldvectors=oldvectors_plt, groups=[1], finalize=.false. )

      call derive_vector &
      ( updatemesh, problem, pressure, elemsub=elastic_pressure, coefficients=coefficients_s, &
