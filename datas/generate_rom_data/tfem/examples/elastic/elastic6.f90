...
! Spectral elements.
! This problem is similar to elastic1, but now with spectral elements.
...
! fill coefficients
  ...
  coefficients%i(1:11) = &
    [ uintpl,   pintpl,      model, 0,     0,  &
      ...
      gaussb ]
  coefficients%i(4) = p ! displacement polynomial order
  coefficients%i(5) = q ! pressure polynomial order
...
! plot mesh before deformation
  ...
  call plot_mesh ( plot_options, mesh_plot, 'mesh_plot.fig' )

! update coordinates of mesh

  call extract_physvector ( mesh, problem, soln, displacement )

  mesh_plot%coor = mesh_plot%coor + &
...
! plot mesh after deformation
  ...
  call plot_mesh ( plot_options, mesh_plot, 'mesh_plot.fig', append=.true. )
...
! write to a fig file for plotting of pressure and energy
  ...
  call plot_color_fill ( plot_options, mesh_plot, problem, &
    'energy_color.fig', vector=energy )
  call plot_color_fill ( plot_options, mesh_plot, problem, &
    'J.fig', vector=volumech )
