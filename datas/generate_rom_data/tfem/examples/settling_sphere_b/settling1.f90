...
! axisymmetric problem on a 2D mesh
...
! definitions
  ...
  type(subscript_t) :: velz, velr, vel, pres, solsc, velzcylinder
  ...
  real(dp) :: alpha, epsconf, U, K_drag
...
! read mesh
  ...
  call read_mesh_gmsh ( mesh1, filename='meshes/meshp.msh', ndim=2 )
...
! all walls
  call add_to_mesh ( mesh, curve=[4,5,6,7,8] ) ! curve 10
! centerline without cylinder
  call add_to_mesh ( mesh, curve=[1,3] ) ! curve 11
...
! inflow
  call define_essential ( mesh, input_probdef, curve1=4, physq=physqb )
...
! velocity degrees in x-direction on the cylinder
  call create_subscript ( mesh, problem, velzcylinder, physqarr=[physqvel], &
    degfd=1, curves=[9] )
...
! create vector subscript for the conformation tensor
  ...
  call create_subscript ( mesh, problem, czz, degfd=1, vec=5+nmodes )
  call create_subscript ( mesh, problem, czr, degfd=2, vec=5+nmodes )
  call create_subscript ( mesh, problem, crr, degfd=3, vec=5+nmodes )
  call create_subscript ( mesh, problem, ctt, degfd=4, vec=5+nmodes )
...
! initial solution b tensor
  ...
  do m = 1, nmodes
    ...
    soliter%u(bvalcmp(5,m)%s) = 1 ! initial btt
  end do
...
!       Also solsupg is constant during the iteration process.
      ...
      if ( iter == 1 ) then
        call fill_sysvector ( mesh, problem, dsol, physq=physqvel, degfd=1, &
          curve1=10, value=-(Uspec(step) - Uspec(step-1)) )
      else
        call fill_sysvector ( mesh, problem, dsol, physq=physqvel, degfd=1, &
          curve1=10, value=0._dp )
      end if
...
!   reaction forces
    ...
    K_drag = sum ( reacf%u(velzcylinder%s) ) / ( 6*pi*eta_0*U*R )
...
!   write max and mean values of conformation and b-tensor tensor to a file
    ...
    write(11, fmt=*) step, maxval(ctensor%u(czz%s)), &
