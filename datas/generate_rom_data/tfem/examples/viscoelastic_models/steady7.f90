...
! open output files
  ...
  open ( unit=14, file='steady7.out', recl=300 )
...
!   stress tensor
    ...
    if ( nmodes == 1 ) then
      ...
      trtau = tau(1) + tau(3) + tau(4)
      tau_e = vonmises_2D(tau)

      if ( includesolvent) then
        write(14,'(7es16.8,1x,i0)') rate, tau + tau_s(L), tau_e, trtau/3, &
          regime(tau_y(1), mu(1), trtau, tau_e )
      else
        write(14,'(7es16.8,1x,i0)') rate, tau, tau_e, trtau/3, &
          regime(tau_y(1), mu(1), trtau, tau_e )
      end if
      ...
      if ( includesolvent) then
        write(14,'(5es16.8)') rate, tau + tau_s(L)
      else
        write(14,'(5es16.8)') rate, tau
      end if
