...
! Similar to startup60, but now with 2D flow.
...
!   constant part of right-hand side
    ...
    call rhs_2D ( vemoptn, vemopt_gammapn )
...
!     right-hand side and Jacobian for all modes at once
      ...
      call rhs_2D ( vemopt, vemopt_gammap )
...
! build rhs for given vemopt
  ...
  subroutine rhs_2D ( vemopt, vemopt_gammap )

    type(vemopt_t), intent(inout) :: vemopt
    type(vemopt_gammap_t), intent(inout) :: vemopt_gammap

!   alpha process
    call rhs_viscoelastic_2D ( vemodel, gradv, c, rhs, mode1=1, &
        mode2=am, vemopt=vemopt )

!   elastic mode
    call rhs_viscoelastic_2D ( vemodel, gradv, c, rhs, mode1=em, &
      mode2=em, vemopt=vemopt )

!   beta process
    call rhs_viscoelastic_2D ( vemodel, gradv, c, rhs, mode1=bm, &
