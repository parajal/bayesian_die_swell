...
! The model is a EGP elastoviscoplastic model
! multi-mode von Mises stress
! Test dependence on J of EGP model and factor h(J).
...
program steady18
  ...
  real(dp) :: eta_s = 0._dp,          & ! solvent viscosity
              ...
              beta = 20._dp,          &  ! exponent beta in h(J)=J^beta
              Jvol = 1.1_dp,          &  ! change in volume for model=24
...
! define the model
  ...
  call create_viscoelastic_model ( model, vemodel, nmodes, coorsys, &
    alam_model=alam_model, vmglobal=.true., alamJ_model=alamJ_model )

  if ( model == 24 .or. alamJ_model==1 ) then
!   EGP compressible or J dependent: input J
...
! set material parameters
  ...
  if ( alamJ_model == 1 ) then
    vemodel%alamJ(1,:) = beta
  end if

! open output files

  open ( unit=13, file='iter.out', recl=300 )
  open ( unit=14, file='steady17.out', recl=300 )
  open ( unit=16, file='detb17.out', recl=300 )
...
!     EGP compressible trL/=0
      if ( model == 24 ) gradv(1,1) = gradv(1,1) + Lxx_add
    case(2)
...
!   Output
    ...
    write(16,*) rate(i), (detb_2D(b(1,:,m)),m=1,nmodes)
...
!   stress tensor
    ...
    if ( includesolvent) then
      write(14,'(8es16.8)') rate, tau + tau_s(L), vonmises_2D(tau)
    else
      write(14,'(8es16.8)') rate, tau, vonmises_2D(tau)
    end if

  end subroutine output

end program steady18
