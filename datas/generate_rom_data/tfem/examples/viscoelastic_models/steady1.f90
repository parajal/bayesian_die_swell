...
! open output files
  ...
  if ( eigen ) open ( unit=15, file='eigen1.out', recl=300 )
...
!   stress tensor
    ...
    if ( nmodes == 1 ) then
      ...
      if ( includesolvent) then
        write(14,'(9es16.8,1x,i0)') rate, tau + tau_s(L), tau_e, trtau/3, &
          regime(tau_y(1), mu(1), trtau, tau_e )
      else
        write(14,'(9es16.8,1x,i0)') rate, tau, tau_e, trtau/3, &
          regime(tau_y(1), mu(1), trtau, tau_e )
      end if
