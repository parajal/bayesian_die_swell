...
! 2D flow with LCR. Combined shear and uniaxial extensional flow.
...
! Similar to startup58_log, but now with 2D flow

program startup64_log

  use viscoelastic_models_2D_log_m
  use limits_m, only: USE_EGP_STRESS_TENSOR_FORM
  use viscoelastic_models_2D_m, only: detc_2D
...
!   right-hand side
    ...
    call rhs_viscoelastic_2D_log ( vemodel, gradv, s, rhs, vemopt=vemopt )
    call rhs_plastic_strain_2D_log ( vemodel, s, gammap, rhs_gammap, &
...
!   right-hand side
    ...
    call rhs_viscoelastic_2D_log ( vemodel, gradv, s+k1, rhs, vemopt=vemopt )
    call rhs_plastic_strain_2D_log ( vemodel, s, gammap + k1gp, rhs_gammap, &
      vemopt_gammap=vemopt_gammap )
...
!   compute stress tensor
    ...
    call stress_viscoelastic_2D_log ( vemodel, s, tau, vemopt=vemopt )

    call conformation_2D_log ( vemodel, s(:,:,1), c(:,:,1) )
