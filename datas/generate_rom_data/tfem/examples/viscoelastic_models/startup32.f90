! Startup from zero stress of uniaxial extensional flow with an imposed normal
...
! (Saramito 2009). Single mode. 3D flow.
...
!                         3  elastoviscoplastic SRM2, parameters: tau_y, K, n
  ...
  real(dp), parameter :: modulus = 10000, & ! modulus
                         ...
                         N_imp = 3600, &  ! stress difference imposed
...
!   velocity gradient
    ...
    L = 0; L(1,1) = epsdot1; L(2,2) = -epsdot1/2; L(3,3) = -epsdot1/2
...
!   velocity gradient
    ...
    L = 0; L(1,1) = epsdot2; L(2,2) = -epsdot2/2; L(3,3) = -epsdot2/2
...
!   compute stress tensor
    ...
    write(13,*) tnp1, extstrain, tau, vonmises_3D(tau(1,:)), &
                abs(tau(1,1)-tau(1,4)-Nxxmyy(zone,tnp1)), exp(extstrain)
...
! extension rate function
  ...
  function epsilondot ( zone, t, c )
    ...
    call rhs_viscoelastic_relax_3D ( vemodel, c, rhs )

    sumfxxmyy = - sum( modulus * ( rhs(1,1,:) - rhs(1,4,:) ) )
    sumcxxyy = sum( modulus * ( 2*c(1,1,:) + c(1,4,:) ) )
