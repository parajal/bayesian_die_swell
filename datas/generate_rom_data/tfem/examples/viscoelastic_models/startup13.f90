...
! Planar extensional or shear flow (or combined)
! The model is a Chilcott-Rallison model
...
program startup13
  ...
  integer, parameter :: model    = 21, & ! Chilcott-Rallison model
                                         ! (choose model=8 or 21)
  ...
  real(dp), parameter :: modulus = 2, & ! modulus
                         ...
                         L2s = 50._dp, &  ! L^2-parameter
                         gammadot = 4.0_dp, &  ! shearrate rate
...
! set material parameters
  ...
  if ( model == 8 ) then
    vemodel%lambda = lambda*L2s/(L2s-3)
    vemodel%modulus = modulus*(L2s-3)/L2s
  else
