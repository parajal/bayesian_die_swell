...
! Similar to startup42, but now with plastic strain softening. Single mode only.
...
!   right-hand side
    ...
    call rhs_viscoelastic_3D ( vemodel, gradv, c+k1, rhs, vemopt=vemopt )
    call rhs_plastic_strain_3D ( vemodel, c, gammap + k1gp, rhs_gammap, &
      vemopt_gammap=vemopt_gammap )
