...
! The model is a EGP elastoviscoplastic model with multi-mode von Mises stress
...
! open output files
  ...
  open ( unit=14, file='steady14.out', recl=300 )
  open ( unit=16, file='detc14.out', recl=300 )

! initialize c

  do mode = 1, nmodes
    c(1,:,mode) = [ 1._dp, 0._dp, 0._dp, 1._dp, 0._dp, 1._dp ]
  end do
...
!   Output
    ...
    write(16,*) rate(i), (detc_3D(c(1,:,m)), m=1,nmodes)
