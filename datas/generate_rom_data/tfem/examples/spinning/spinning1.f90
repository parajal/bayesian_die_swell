! Spinning problem of a Newtonian fluid (Stokes)
! Axisymmetrical flow.
...
! variables
  ...
  integer :: step, i, nnodes, obj_ob, obj_sh
  real(dp) :: flowrate, R
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ numtimesteps, plot_mesh_every, restart, ctime, &
    file_append, eta, deltat, rs_up, is_up, U_avg, DR
...
! fill solution vector with essential boundary conditions
  ...
  call fill_sysvector ( mesh, problem, sol, curve1=6, physq=1, degfd=1, &
     value=DR*U_avg )
...
! set coordinates
  ...
  do i = 1, mesh%curves(7)%nnodes
    mesh_sf_adv%coor(i,1) = mesh%coor(mesh%curves(7)%nodes(i),1)
  end do
...
!     search for the element
      ...
      do elem = 1, mesh_sf_adv%nelem
        ...
        if ( (mesh%coor(i,1) >= x2) .and. (mesh%coor(i,1) <= x1) ) then
...
!         height at position mesh%coor(i,1)
          yp = y2 + (y1-y2)*(mesh%coor(i,1)-x2)/(x1-x2)
