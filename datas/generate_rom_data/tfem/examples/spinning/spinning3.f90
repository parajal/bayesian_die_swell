! Viscoelastic spinning problem
! Axisymmetrical flow for an XPP model
! DEVSS-G/SUPG
...
! constants flow problem
  ...
  integer, parameter :: &
    ...
    model = 16        ! XPP model (single equation)
...
! variables
  ...
  integer :: icomp, step, i, j, nnodes, obj_ob, obj_sh
  real(dp) :: alpha, G
  real(dp) :: flowrate, R
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ numtimesteps, plot_mesh_every, &
    restart, ctime, file_append, logc, eta_s, eta_p, &
    lambda, lambda_s, nu, deltat, htype, Uscaling, &
    rs_gup, is_gup, rs_c, is_c, U_avg, DR
...
! fill coefficients
  ...
  call create_coefficients ( coefficients, ncoefi=150, ncoefr=500+4*nmodes )
  ...
  coefficients%r(501:504) = [ G, lambda, lambda_s, nu ]
...
! set flowrate
  ...
  R = mesh%coor(mesh%points(4),2)  ! radius is given by radial coordinate of P4
...
! outflow: uz=DR*U_avg, ur=0 (no extension on winder)
  call define_essential ( mesh, input_probdef, &
    curve1=6, physq=physqvel, degfd=[1,1] )
! wall
  call define_essential ( mesh, input_probdef, &
...
! fill solution vector with essential boundary conditions
  ...
  call fill_sysvector ( mesh, problem, sol, curve1=6, &
    physq=physqvel, degfd=1, value=DR*U_avg )
...
!   open boundary
    ...
    !coefficients%i(48) = 0 ! do not use mesh velocity for ALE formulation

    call build_system ( mesh, problem, sysmatrix, rhsd, &
      elemsub1=implicit_stress_open_boundary, physqrow=[2], physqcol=[2], &
      object=obj_ob, addmatvec=.true., oldvectors=oldvectors_ve, &
...
!   solve surface advection (corrector)
    ...
    if ( mod(step0+step,plot_mesh_every) == 0 ) then
      write(filename,'(a,i5.5,a)') 'mesh_spinning_', step0+step, '.fig'
...
! solve convection equation for the surface height (corrector)
  ...
  subroutine solve_surface_height_corrector
    ...
    real(dp) :: max_radius, min_radius, end_radius
...
!   compute the maximum, minimum and end radius
    ...
    max_radius = maxval ( sol_sf_adv%u )
    min_radius = minval ( sol_sf_adv%u )
    end_radius = sol_sf_adv%u(hgt_end%s(1))

!   write fiber radius

    write(11,'(i6,4es16.8)') step0+step, time0+step*deltat, &
                             max_radius, min_radius, end_radius
...
!     open file for writing the radius as a function of time
      ...
      if ( file_append ) then
        open ( unit=11, file='spinning_radius.out', status='old', &
               position='append' )
      else
        open ( unit=11, file='spinning_radius.out', status='unknown' )
      end if
