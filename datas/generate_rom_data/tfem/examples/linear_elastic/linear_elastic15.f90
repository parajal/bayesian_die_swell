...
! constants
  ...
  real(dp), parameter :: &
    ...
    or = 1._dp,  &   ! inner radius of the domain
...
! post-processing
  ...
  call postprocessing_linear_elastic ( mesh, problem, 'le15.vtk', sol, &
    ...
    select case ( nr )
      case(1)
        vfunc = [ x + x**5/5 + y**2/2 + x**3*y**3/3, &
                  -y -y**5/5 + x**2/2*y + x**2*y**2/2 ]  ! exact displacement
      case(2)
        vfunc = -(lambda+mu)* [ &
                    4*x**3 + 2*x*y**3 + 2*x + 3*x*y, &
                    -24*y**3/5+3*x**2*y**2+3*x**2/2 ] - &
                mu * [ 4*x**3+2*x*y**3+2+ 3*x**3*y, &
                               y+y**2-24*y**3/5+3*x**2/2 ] ! rhs
      case default
