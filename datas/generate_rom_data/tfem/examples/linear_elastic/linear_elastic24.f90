! Linear elastic problem on a rectangle. Stretching a bar. Thermal expansion.
! Mixed displacement-pressure formulation. Axisymmetric.

program linear_elastic24
...
! constants
  ...
  real(dp), parameter :: &
    ...
    alpha = 1e-5_dp, &  ! linear thermal expansion coefficient
    dTemp = -10._dp,  &  ! temperature increase
    ul = 0._dp       ! displacement at x = L for uniform = .false.
...
! problem definition
  ...
  if ( uniform ) then
    call define_essential ( mesh, input_probdef, curves=[4], physq=1, &
      degfd=[1,0] )
  else
...
! post-processing
  ...
  call postprocessing_linear_elastic ( mesh, problem, 'le24.vtk', sol, &
