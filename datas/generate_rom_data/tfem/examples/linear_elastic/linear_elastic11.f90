! Linear elastic problem on a domain two cylinders having a jump in radius.
! Stretching and rotating.
! Standard displacement formulation. Axisymmetric with torsion.

program linear_elastic11
...
! read mesh
  ...
  call read_mesh_gmsh ( mesh, 'axis.msh', ndim=2 )
...
! problem definition
  ...
  if ( uniform ) then
    call define_essential ( mesh, input_probdef, curves=[2,6], physq=1, &
      degfd=[1,0,1] )
  else
    call define_essential ( mesh, input_probdef, curves=[2,6], physq=1 )
  end if
...
!   uniform deformation, no edge effects
    ...
    call fill_sysvector ( mesh, problem, sol, &
      curve1=6, physq=1, degsfd=[1,3], value=0._dp )
  else
!   edge effects
    call fill_sysvector ( mesh, problem, sol, &
      curve1=2, physq=1, degsfd=[1,3], vfunc=displacement_theta, vfuncnr=1 )
...
! post processing
  ...
  call postprocessing_linear_elastic ( mesh, problem, 'le11.vtk', sol, &
...
contains
  ...
  function displacement_theta ( n, nr, x )
    ...
    select case ( nr )
      ...
      case default
        write(*,'(/a,i0/)') 'Error displacement_theta: wrong function number: ', nr
        stop
    end select

  end function displacement_theta

end program linear_elastic11
