! Grid deformation: elements are "attracted" towards two circles.

program grid_deformation5

  use kind_defs_m
  use tfem_m
  use meshgen_grid_deformation_m
  use deform_mesh_ma57_m
  use subs_grid_deformation5_m
  use figplot_m
  use timer_m
...
!   away from the the zero levelset (max_f).
    ...
    lx = 1._dp,   &
    ly = 1._dp,   &
    radius(2)=[0.1,0.1], &  ! radii of the circles
!   centers of the circle
    center(2,2)=reshape([-0.105_dp,0._dp,0.105_dp,0._dp],[2,2])
...
! create object for the circle boundary (only for plotting)
  ...
  allocate ( coor(2*nnodes,2) )

  call objectscoor1 ( 1, coor(1:nnodes,:) )
  call objectscoor2 ( 1, coor(nnodes+1:2*nnodes,:) )
