...
! Spectral elements.
! This problem is similar to navier_stokes5, but now with spectral elements.

program navier_stokes15
...
! definitions
  ...
  type(oldvectors_t) :: oldvectors, oldvectors_iter, oldvectors_press
...
! problem definition
  ...
  call define_essential ( mesh, input_probdef, element=1, elnode=p+3, &
    physq=2, degfd=[1] ) ! Set one pressure degree
...
! create the structure oldvectors
  ...
  call create_oldvectors ( oldvectors_press, nsysvec=1 )
...
!   fill exact pressure in the center of the first element
    call fill_sysvector ( mesh, problem, sol_exact, physq=physqpress, &
      elgroup1=1, element=1, elnode=p+3, degfd=1, &
      value=func(nr=3,xin=[H/nx/2,H/ny/2]) )
...
!   compute pressures in the nodes

    oldvectors_press%s(1)%p => sol

    call derive_vector ( mesh, problem, pressure, elemsub=stokes_pressure, &
      coefficients=coefficients, oldvectors=oldvectors_press )

!   compute exact pressures in the nodes

    call fill_vector ( mesh, problem, pressure2, elgroup1=1, func=func, funcnr=3 )

!   write out the errors between the analytical and the numerical solution
    errv = maxval ( sqrt ( ( sol%u(vel%s) - sol_exact%u(vel%s) )**2 ) )
    errp = maxval ( sqrt ( ( pressure%u - pressure2%u )**2 ) )
...
! write to a fig file for plotting
  ...
  call plot_color_fill ( plot_options, mesh_plot, problem, 'pressure_color.fig', &
    vector=pressure )
  ...
  call plot_color_fill ( plot_options, mesh_plot, problem, 'vorticity_color.fig', &
    vector=vorticity )
...
! write vtk data
  call write_scalar_vtk ( mesh_plot, problem, vector=pressure, dataname='pressure', &
    filename='cavity_inertia.vtk' )

  call write_vector_vtk ( mesh_plot, problem, filename='cavity_inertia.vtk', &
...
! delete all data including all allocated memory
  ...
  call delete ( oldvectors, oldvectors_iter, oldvectors_press )
  call delete ( vel, pres )

end program navier_stokes15
