...
! Semi-implicit Crank-Nicolson/Adams-Bashforth time discretization
! Do a real semi-implicit Euler at the first step.
! Keep LU decomposition for later time steps to reduce CPU time.

program navier_stokes10
...
! definitions
  ...
  type(sysvector_t), target :: sol, soln, sol_exact
  ...
  real(dp) :: beta0=1.5_dp, beta1=-0.5_dp
...
!     combine previous time steps for AB2 of the non linear term
...
!     viscous term and pressure at tn
      call build_system ( mesh, problem, sysmatrix, rhsd, &
        ...
        factorvec=0.5_dp, addmatvec=.true. )
...
!       build (assemble) viscous/pressure matrix and vector from elements
        call build_system ( mesh, problem, sysmatrix, rhsd, &
          ...
          buildvector=.false., factormat=0.5_dp )
...
!       instationary term ( rho du/dt ), vector only
        ...
        call build_system ( mesh, problem, sysmatrix, rhsd, &
          elemsub=inertia_elem_dudt, coefficients=coefficients, &
          oldvectors=oldvectors, physqcol=[physqvel], physqrow=[physqvel], &
          buildmatrix=.false., addmatvec=.true. )
...
!   fill solution vector with analytical solution, for the boundary conditions
    ...
    if ( step <= numeulertimesteps ) then
      ...
      call solve_system_ma57 ( sysmatrix, rhsd, sol, lu, solver_options=so )
    end if
...
! delete all data including all allocated memory
  ...
  call delete ( mesh )
  call delete ( sol, soln, sol_exact )
