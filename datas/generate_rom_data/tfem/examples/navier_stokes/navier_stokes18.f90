! Navier-Stokes problem on a square cavity 3D velocities (curved channel).
! Unsteady flow.
! Crank-Nicolson time discretization
! Imposed flow rate in theta-direction. Axisymmetric.

program navier_stokes18
...
! definitions
  ...
  type(sysvector_t), target :: sol, sol_iter, soln
  type(sysvector_t) :: rhsd, rhsdn
...
! mesh width and heigth:
  H = 1._dp
! flow rate in third direction at t->infty
  flowrate = 1._dp
...
! write to a vtk file for further post-processing
  ...
  call write_vector_vtk ( mesh, problem, filename='ns18.vtk', &
  ...
  call write_scalar_vtk ( mesh, problem, vector=gammadot, &
    filename='ns18.vtk', dataname='gammadot', append=.true. )

  call write_tensor_vtk ( mesh, problem, filename='ns18.vtk', &
