...
! Compressible EGP model with pressure coupling to an incompressible flow.
...
! constants
  ...
  real(dp), parameter :: &
    ...
    alamJ_model = 1,     & ! adapted lambda model for h(J): 0:none 1:J^betaJ
    ...
    betaJ = 20._dp,      & ! beta exponent for the h(J) factor
...
! fill coefficients
  ...
  if ( alamJ_model == 1 ) then
    if ( alam_model == 4 ) then
      coefficients%r(501:504) = &
           [ ( G/nmodes, lambda, tau_ref, betaJ, m=1,nmodes ) ]
    else if ( alam_model == 5 ) then
      coefficients%r(501:506) = &
           [ ( G/nmodes, lambda, tau_ref1, f1, tau_ref2, betaJ, m=1,nmodes ) ]
    end if
  else
    if ( alam_model == 4 ) then
  ...
  if ( pressure_constraint ) then
    if ( any ( coefficients%i(308) == [3,4] ) ) then
!     simulate pressure constraint of compressible fluid flow
...
! initial solution b tensor
  ...
  if ( pressure_essential ) then
!   set pressure level in P1 in the first step
...
!   increase in omega
  ...
  ! set velocity_theta = domega*R on cylinder and pressure level
    call fill_sysvector ( mesh, problem, dsol, &
...
!       set dsol = 0 for pressure difference in point 1
        call fill_sysvector ( mesh, problem, dsol, &
