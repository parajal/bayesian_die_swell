...
! constants
  ...
  integer, parameter :: &
    ...
    endm   = 400,   & ! end of material model data
    model = 24,     & ! EGP compressible
    npar = 3,       & ! (number of) parameters per mode: G, lambda, tau_ref
...
! definitions
  ...
  logical :: steadystep
  integer :: step, i, iter, m, extrastep, mode1, mode2
...
! fill material data
  ...
  call fill_data ( coefficients, 'PMMA', vemodel )
...
!     pressure, velocity and diagonal block
       call build_system ( mesh, problem, sysmatrix, rhsd, &
         ...
         physqrow=[(physqb+m-1,m=mode1,mode2)], &
         physqcol=[physqpress,physqvel,(physqb+m-1,m=mode1,mode2)], &
         oldvectors=oldvectors_ve, addmatvec=.true. )
...
!     pressure and diagonal block
      call build_system ( mesh, problem, sysmatrix, rhsd, &
        elemsub=implicit_ce_supg_elem, coefficients=coefficients, &
        physqrow=[(physqb+m-1,m=mode1,mode2)], &
        physqcol=[physqpress,(physqb+m-1,m=mode1,mode2)], &
        oldvectors=oldvectors_ve, addmatvec=.true. )
