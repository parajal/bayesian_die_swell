! Startup + steady state of flow around a rotating cylinder in a square box.
! Incompressible EGP model.
...
! constants
  ...
  integer, parameter :: &
    ...
    startm = -1,    & ! -startm is index in array ra2 for material 2D data array
  ...
  logical, parameter :: &
    ...
    pressure_essential = .false., & ! p=p_level in point P1 (lower-left corner)
...
! definitions
  ...
  logical :: steadystep
  integer :: step, i, iter, m, extrastep, npar
  integer :: vertices(4) = [1,3,5,7]
  real(dp) :: alpha, epsu, epsp, epsb, Wi, K_dragx, K_dragy, K_torqz
  real(dp) :: int_press_domain(1)
...
! fill coefficients
  ...
  call create_coefficients ( coefficients, ncoefi=350, ncoefr=400, &
    ncoefra2=[npar,nmodes] )
  ...
  if ( pressure_constraint ) then
    ...
    coefficients%i(308) = 5 ! type of pressure constraint (linear)
  end if

  coefficients%r(1:400) = &
  ...
  if ( alam_model == 4 ) then
    do i = 1, nmodes
      coefficients%ra2(1)%a(:,i) = [ G/nmodes, lambda, tau_ref ]
    end do
  else if ( alam_model == 5 ) then
    do i = 1, nmodes
      coefficients%ra2(1)%a(:,i) = [ G/nmodes, lambda, tau_ref1, f1, tau_ref2 ]
    end do
  end if
...
! initial solution b tensor
 ...
 ! set velocity_theta = omega*R on cylinder in first step
  call fill_sysvector ( mesh, problem, dsol, &
    curve1=13, physq=physqvel, vfunc=vfunc, vfuncnr=1 )

  if ( pressure_essential ) then
!   set pressure level in P1 in first step
    call fill_sysvector ( mesh, problem, dsol, &
      point=1, physq=physqpress, value=p_level )
  end if
