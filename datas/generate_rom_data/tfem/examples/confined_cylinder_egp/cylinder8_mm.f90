...
! constants
  ...
  real(dp), parameter :: &
    deltat3 = 2.e-2_dp,    & ! time step for steady iteration scheme
...
!     add diagonal block of steady-state timed derivative
      call build_system ( mesh, problem, sysmatrix, rhsd, &
        ...
        physqrow=[(physqb+m-1,m=1,nmodes)], &
        physqcol=[physqvel,(physqb+m-1,m=1,nmodes)], &
...
!     add diagonal block of steady-state timed derivative
      call build_system ( mesh, problem, sysmatrix, rhsd, &
        elemsub=steady_ce_timederiv_supg_elem, coefficients=coefficients, &
        physqrow=[(physqb+m-1,m=1,nmodes)], &
        physqcol=[(physqb+m-1,m=1,nmodes)], &
        oldvectors=oldvectors_ve, addmatvec=.true. )
...
!     diagonal block
    ...
    call build_system ( mesh, problem, sysmatrix, rhsd, &
      ...
      physqrow=[(physqb+m-1,m=1,nmodes)], physqcol=[physqgrad,physqvel], &
