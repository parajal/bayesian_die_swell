...
! Problem 2: b tensor
! Multi-mode/multi-process EGP model with two subprocesses. Each subprocess
! defines its own von-Mises stress.
...
! constants
  ...
  integer, parameter :: &
    ...
    msplit = 3,     & ! subproc 1 modes 1:msplit-1,subproc 2 modes msplit:nmodes
...
!   TEST case: by splitting G of one mode to G/4 per mode for four modes
    G =        [ 1.0_dp, 1.0_dp, 1.0_dp, 1.0_dp ], & ! modulus
    lambda =   [ 1.0_dp, 1.0_dp, 1.0_dp, 1.0_dp ], & ! relaxation time
    tau_ref =  [ 1.0_dp, 1.0_dp, 1.0_dp, 1.0_dp ], & ! reference von Mises
    tau_ref1 = [ 1.0_dp, 1.0_dp, 1.0_dp, 1.0_dp ], & ! reference von Mises 1 RE
    tau_ref2 = [ 4.0_dp, 4.0_dp, 4.0_dp, 4.0_dp ], & ! reference von Mises 2 RE
    f1 =       [ 0.6_dp, 0.6_dp, 0.6_dp, 0.6_dp ] ! weight factor first term RE
...
! definitions
  ...
  integer :: icomp, step, i, nn, m, mode1, mode2
...
! fill coefficients
  ...
  if ( alam_model == 4 ) then
    coefficients%r(501:500+(2+nn)*nmodes) = &
...
!   first subprocess of modes 1 to msplit-1
    mode1 = 1; mode2 = msplit-1
    coefficients%i(26:27) = [ mode1, mode2 ]

    call build_system ( mesh, problemb, sysmatrixb, &
      m2sysvector=rhsb(:,mode1:mode2), &
      elemsub=ce_supg_elem, oldvectors=oldvectors_ve, &
      coefficients=coefficients )

!   second subprocess of modes msplit to nmodes
    mode1 = msplit; mode2 = nmodes
    coefficients%i(26:27) = [ mode1, mode2 ]
