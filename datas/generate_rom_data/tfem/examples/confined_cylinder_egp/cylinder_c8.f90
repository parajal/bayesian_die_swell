! Sequence of steady states of flow around a cylinder forfor an incompressible
! EGP model using the conformation tensor formulation.
...
!   increments in imposed average velocity
    dU(ndU) = [ 0.01_dp, 0.1_dp ], & ! increments in imposed average velocity
!   end values of the intervals in U
    Ue(ndU) = [ 0.1_dp, 0.5_dp ], & ! end values of the intervals in U
!   Newton-Raphson convergence threshold for each interval
    epsconfin(ndU) = [ 1e-8_dp, 1e-8_dp ]
