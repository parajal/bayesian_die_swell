...
! definitions
  ...
  type(subscript_t) :: cvalxx, cvalxy, cvalyy, cvalzz, cval
...
! for post processing the data
  ...
  call create_subscript ( mesh, problemc, cvalzz, physqarr=[1], degfd=4 )
...
! initial solution
  ...
  do m = 1, nmodes
    if ( logc == 0 ) then ! standard
      ...
      solcn(m)%u(cvalzz%s) = 1 ! initial cyy
...
! delete all data including all allocated memory
  ...
  call delete ( cvalxx, cvalxy, cvalyy, cvalzz, cval )
