...
! the b-tensor formulation. Explicit div tau term.
! Problem 1: Flow around a confined cylinder.
!   Periodical boundary conditions.
!   Stress-explicit formulation of the momentum balance.
! Problem 2: b tensor
! Problem 3: Equivalent plastic strain (gammap)
! Multi-mode EGP model with plastic strain softening and the option to
! use a global von Mises stress.

program cylinder1a_mm_gammap
...
! constants
  ...
  integer, parameter :: &
    ...
    ncompb = 5,         & ! number of b-tensor comp
...
!   and using a global von Mises gives the same results as the one mode problem.
    ...
    Sa =     [  7.4_dp, 7.4_dp  ], & ! state parameter Sa
    r0 =     [ 0.96_dp, 0.96_dp ], & ! fitting parameter r0
    r1 =     [ 20.0_dp, 20.0_dp ], & ! fitting parameter r1
    r2 =     [ -2.0_dp, -2.0_dp ]    ! fitting parameter r2
...
!    eta_0 = eta_s+eta_p, & ! zero-shear viscosity
  ...
  real(dp), parameter :: &
    ...
    rs_gp = 1.0_dp,  & ! real_storage for the gammap LU (HSL)
    is_gp = 1.6_dp     ! integer_storage for gammap LU (HSL)
...
! definitions
  ...
  type(input_probdef_t) :: input_probdefgp
  type(problem_t), target :: problemgp
  type(sysmatrix_t) :: sysmatrixgp
  type(subscript_t) :: gpval
  type(sysvector_t), target :: solgp, solgpm1
  type(sysvector_t) :: rhsgp
  type(vector_t) :: gammap
  type(solver_options_ma41_t) :: solver_options_gp
...
! fill coefficients
  ...
  call create_coefficients ( coefficients, ncoefi=150, &
    ncoefr=500+(6+nn)*nmodes )
  ...
  coefficients%i(71) = 1  ! b-formulation
  ...
  coefficients%i(100) = 1 ! storage gammap in separate sysvector
  coefficients%i(102) = 3 ! sysvector number of gammap at tn
  ...
  if ( alam_model == 4 ) then
    coefficients%r(501:500+(6+nn)*nmodes) = &
      [ (G(m), lambda(m), tau_ref(m), Sa(m), r0(m), r1(m), r2(m), m=1,nmodes ) ]
  else if ( alam_model == 5 ) then
    coefficients%r(501:500+(6+nn)*nmodes) = &
      [ (G(m), lambda(m), tau_ref1(m), f1(m), tau_ref2(m), &
...
! problem definition b tensor
  ...
  input_probdefb%vec_elementdof(1)%a(:,3) = 4 ! tensor for plotting
  input_probdefb%vec_elementdof(1)%a(:,4) = 5 ! tensor for plotting
...
! for post processing the data
  ...
  call create_subscript ( mesh, problemb, czz, degfd=4, vec=3 )
...
! initial solution
  ...
  do m = 1, nmodes
    ...
    solb(3,m)%u = 0 ! initial byx
    solb(4,m)%u = 1 ! initial byy
    solb(5,m)%u = 1 ! initial bzz
  end do
...
! problem definition gammap

  call create_input_probdef ( mesh, input_probdefgp, nvec=2, nphysq=1 )

  input_probdefgp%vec_elementdof(1)%a(vertices,1) = 1 ! gp
  input_probdefgp%vec_elementdof(1)%a(:,2) = 1 ! scalar for plotting

  input_probdefgp%physq = [1]
  input_probdefgp%probnr = 3

! constraint for periodical boundary conditions of gammap
  call define_constraint ( mesh, input_probdefgp, curve1=4, curve2=12, &
    discretization='collocation' )

  call problem_definition ( input_probdefgp, mesh, problemgp )


! create a vector subscript for the gammap without Lagr. multipl.
! for post processing the data

  call create_subscript ( mesh, problemgp, gpval, physqarr=[1] )


! create system vectors (solution and right-hand side) for gammap and
! initialize vectors.

  call create ( problemgp, solgp, solgpm1, rhsgp )

! initial solution

  solgp%u = 0 ! initial gammap

! create system matrix for b tensor problem

  call create_sysmatrix_structure_base ( sysmatrixgp, mesh, problemgp )
  call create_sysmatrix_structure_constraint ( sysmatrixgp, mesh, problemgp )
  call finalize_sysmatrix_structure ( sysmatrixgp )

  call create_sysmatrix_data ( sysmatrixgp )


! create the structure oldvectors_ve

  call create_oldvectors ( oldvectors_ve, nsysvec=4, nsysvec2=2, nprob=3 )

! store solution vectors and problem structures

  oldvectors_ve%s(1)%p => sol
  oldvectors_ve%s(2)%p => solm1
  oldvectors_ve%s(3)%p => solgp
  oldvectors_ve%s(4)%p => solgpm1
  oldvectors_ve%s2(1)%p => solb
  oldvectors_ve%s2(2)%p => solbm1
  oldvectors_ve%p(1)%p => problem
  oldvectors_ve%p(2)%p => problemb
  oldvectors_ve%p(3)%p => problemgp
...
!   build (assemble) matrix and vector for b-tensor problem
    ...
    if ( any( coefficients%i(22) == [6,7] ) ) then
...
!   solve b tensor and keep LU decomposition in the loop over modes/components
...
!   build (assemble) matrix and vector for gammap problem

    if ( any( coefficients%i(22) == [6,7] ) ) then
      call build_system ( mesh, problemgp, sysmatrixgp, sysvector=rhsgp, &
        elemsub=gammap_supg_elem_implicit_2nd_order, oldvectors=oldvectors_ve, &
        coefficients=coefficients )
    else
      call build_system ( mesh, problemgp, sysmatrixgp, sysvector=rhsgp, &
        elemsub=gammap_supg_elem, oldvectors=oldvectors_ve, &
        coefficients=coefficients )
    end if

!   periodical condition on gammap
    call build_system_constraint ( mesh, problemgp, sysmatrixgp, &
      sysvector=rhsgp, elemsub=stokes_constr_node_conn, &
      addmatvec=.true. )

    call check ( sysmatrixgp )


!   copy previous gammap solution to older time step

    call copy ( solgp, solgpm1 )


!   solve gammap

    solver_options_gp%real_storage=rs_gp
    solver_options_gp%integer_storage=is_gp

    call solve_system_ma41 ( sysmatrixgp, rhsgp, solgp, &
      solver_options=solver_options_gp )
...
!   write max and mean values of conformation tensor to a file
    ...
    write(11, fmt=*) step * deltat, maxval(ctensor%u(cxx%s)), &
                                    ...
                                    maxval(solgp%u(gpval%s))
    if ( printscreen ) &
        print *, 'step = ', step, 'max cxx = ', maxval(ctensor%u(cxx%s)), &
           ' max gammap = ', maxval(solgp%u(gpval%s))
...
! von Mises stress
  ...
  call create_vector ( problemb, vonmises, vec=2 )
...
!    coefficients%i(27) = 2  ! mode2
  ...
  call derive_vector ( mesh, problemb, vonmises, &
    elemsub=deriv_viscoelastic_stress_scalar, &
    coefficients=coefficients, oldvectors=oldvectors_ve )

  call write_scalar_vtk ( mesh, problemb, filename='c.vtk', &
...
! gammap
  ...
  call create_vector ( problemgp, gammap, vec=2 )

  call derive_vector ( mesh, problemgp, gammap, &
    elemsub=deriv_gammap, coefficients=coefficients, &
    oldvectors=oldvectors_ve )

  call write_scalar_vtk ( mesh, problemgp, filename='gammap.vtk', &
...
end program cylinder1a_mm_gammap
