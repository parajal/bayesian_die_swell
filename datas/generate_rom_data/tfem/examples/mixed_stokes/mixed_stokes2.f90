...
! Q1isoQp for stresses using the blend mesh 1 for interpolation.
! Blended mesh:
!  main mesh: 9-node quadrilaterals
!  blend mesh 1: (p+1)^2-node high-order quadrilaterals, equidistant nodes
! Note: for p=4 this is the famous Marchal-Crochet 4x4 mixed element.

program mixed_stokes2
...
! constants
  ...
  integer, parameter :: &
    ...
    gauss = 3,          & ! 3 x 3 integration of quads
    gaussb = 3,         & ! number of Gauss points on the boundary
    nsubint = p,        & ! p x p subintegration domains
    inttype = 0,        & ! standard rules for integration
...
! Create blended mesh
  ...
  call mesh_change ( meshQp, regular_to_macro=.true. )
...
! post processing solution
  ...
  call write_vector_vtk ( mesh, problem, sysvector=sol, &
    dataname='velocity', filename='mixed_stokes2.vtk', physq=physqvel )

  call write_tensor_vtk ( mesh_plot_Qp, problem, sysvector=sol, &
    dataname='stress', filename='mixed_stokes2_stress.vtk', &
...
! write derivatives to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh, problem, vector=pressure, &
    dataname='pressure', filename='mixed_stokes2.vtk', append=.true. )

  call write_scalar_vtk ( mesh, problem, vector=vorticity, &
    dataname='vorticity', filename='mixed_stokes2.vtk', append=.true. )

  call write_scalar_vtk ( mesh, problem, vector=divergence, &
    dataname='divergence', filename='mixed_stokes2.vtk', append=.true. )
