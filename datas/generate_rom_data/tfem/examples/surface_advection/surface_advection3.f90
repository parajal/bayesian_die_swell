...
! Second-order time integration. First time step first-order.
...
! fill coefficients
  ...
  coefficients%i(5) = 2 ! time-integration
...
! create system vectors (solution and right-hand side)
  ...
  call create_sysvector ( problem, sol, soln, solnm1, rhsd, solexact )
...
! oldvectors
  ...
  call create_oldvectors ( oldvectors, nsysvec=2, nvec=1 )
...
!   fill boundary condition at new time step
    ...
    if ( step == 1 ) then
      coefficients%i(5) = 1 ! first-order
    else
      coefficients%i(5) = 2 ! second-order
    end if
...
! delete all data including all allocated memory
  ...
  call delete ( mesh )
  call delete ( sol, soln, solnm1, rhsd, solexact )
