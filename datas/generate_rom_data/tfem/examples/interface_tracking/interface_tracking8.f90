! Tracking of an initially square interface in a flow given by a function.
! Interface movement is Lagrangian.
! Explicit second-order time integration (on element level).
! tfem mesh made using mesh_skeleton.

program interface_tracking8
...
!   create mesh of discretized with 2-node line elements
...
!   create mesh of discretized with 3-node line elements
...
! square R0xR0
  ...
  do node = 1, mesh%nnodes/4
    mesh%coor(node,1) = R0
    mesh%coor(node,2) = R0 * ( 8._dp * (node-1)/mesh%nnodes-1.0_dp)
    mesh%coor(node+mesh%nnodes/4,1) = &
                      - R0 * ( 8._dp * (node-1)/mesh%nnodes-1.0_dp)
    mesh%coor(node+mesh%nnodes/4,2) = R0
    mesh%coor(node+2*mesh%nnodes/4,1) = - R0
    mesh%coor(node+2*mesh%nnodes/4,2) = &
                      - R0 * ( 8._dp * (node-1)/mesh%nnodes-1.0_dp)
    mesh%coor(node+3*mesh%nnodes/4,1) = &
                        R0 * ( 8._dp * (node-1)/mesh%nnodes-1.0_dp)
    mesh%coor(node+3*mesh%nnodes/4,2) = - R0
  end do
...
end program interface_tracking8
