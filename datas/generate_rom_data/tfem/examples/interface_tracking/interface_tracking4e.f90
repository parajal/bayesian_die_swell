...
! Tangential grid deformation in addition to the movement in normal direction.
! Up to fourth-order explicit time integration with prediction.
! Gmsh mesh with six-node triangles.
! Error output

program interface_tracking4e
...
! constants
  ...
  real(dp), parameter :: &
    Um(ndim) = [ 0._dp, 0._dp, 0._dp ], &  ! arbitrary velocity u_m (constant)
...
! fill coefficients
  ...
  coefficients%r = 0
  coefficients%r(4:6) = Um
  coefficients%r(7) = factor

  coefficients%vfunc1(1)%p => vfunc_3D

! read gmsh file of a sphere surface discretized with 6-node triangular elements
...
end program interface_tracking4e
