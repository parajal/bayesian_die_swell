! Viscoelastic problem in a channel with a square cross section.
! 3D velocities on a 2D domain (developed flow).
! DEVSS-G/SUPG
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ nx, ny, lx, ly, timeint1, timeint2, numtimesteps, logc, &
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh, problem, vector=pressure, dataname='pressure', &
    filename='channel10.vtk' )

  call write_vector_vtk ( mesh, problem, filename='channel10.vtk', &
  ...
  call write_scalar_vtk ( mesh, problem, vector=gammadot, &
    filename='channel10.vtk', dataname='gammadot', append=.true. )

  call write_tensor_vtk ( mesh, problem, filename='channel10.vtk', &
  ...
  call write_tensor_vtk ( mesh, problemc, filename='channel10.vtk', &
