...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ nx, ny, lx, ly, timeint, numtimesteps, logc, &
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh, problem, vector=pressure, dataname='pressure', &
    filename='channel6.vtk' )

  call write_vector_vtk ( mesh, problem, filename='channel6.vtk', &
  ...
  call write_scalar_vtk ( mesh, problem, vector=gammadot, &
    filename='channel6.vtk', dataname='gammadot', append=.true. )

  call write_tensor_vtk ( mesh, problem, filename='channel6.vtk', &
  ...
  call write_tensor_vtk ( mesh, problemc, filename='channel6.vtk', &
