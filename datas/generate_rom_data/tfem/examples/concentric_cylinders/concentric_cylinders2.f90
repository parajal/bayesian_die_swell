...
! velocities
  if ( concentric ) then
    call define_constraint ( mesh, input_probdef, &
      physq=physqv, curve1=2, curve2=5, discretization='collocation', &
      exclude=3 )
  else
    call define_constraint ( mesh, input_probdef, &
      physq=physqv, curve1=2, curve2=5, discretization='collocation', &
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh, problem, vector=pressure, dataname='pressure', &
    filename='concyl2.vtk' )

  call write_vector_vtk ( mesh, problem, filename='concyl2.vtk', &
  ...
  call write_scalar_vtk ( mesh, problem, vector=gammadot, &
    filename='concyl2.vtk', dataname='gammadot', append=.true. )

  call write_tensor_vtk ( mesh, problem, filename='concyl2.vtk', &
  ...
  call write_tensor_vtk ( mesh, problemc, filename='concyl2.vtk', &
...
end program concentric_cylinders2
