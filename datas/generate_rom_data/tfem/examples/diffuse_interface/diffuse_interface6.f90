...
! definitions
  ...
  type(oldvectors_t) :: oldvectors_di, oldvectors_tec, oldvectors
  ...
  type(vector_t) , target:: velocity, pressure, concentration, vorticity
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ nr, nz, numtimesteps, rho, eta, deltat, &
...
! create mesh
  ...
  meshgen_options%x2d = &
    reshape ( [ 0.0_dp, 6.0_dp, 6.0_dp,  0.0_dp,    &
...
! fill solution vector with essential boundary conditions
  ...
  sol%u = 0 ! initialize to zero
! set velocity on all boundaries
  call fill_sysvector ( mesh, problem, sol, &
    curve1=2, curve2=3, physq=physqvel, degfd=1, func=difunc, funcnr=11 )
  call fill_sysvector ( mesh, problem, sol, &
    curve1=2, curve2=3, physq=physqvel, degfd=2, func=difunc, funcnr=10 )
! set pressure level = 0
  call fill_sysvector ( mesh, problem, sol, &
    point=3, physq=physqpress, value=0._dp )
...
!   funcnr = 4 : a supercritical and a subcritical nucleus
  ...
  call fill_sysvector ( mesh, problemdi, soldin, degfd=1, &
       node1=1, node2=mesh%nnodes, func=difunc, funcnr=6)
...
! time stepping
  ...
  do step = 1, numtimesteps
    ...
    if (step == 250)  then
       call fill_sysvector ( mesh, problem, sol, &
       curve1=2, curve2=3, physq=physqvel, value = 0.0_dp )
    end if
...
!   Tecplot output
    ...
    if (mod(step,10) == 0) then
      call write_mdata_tecplot (mesh, problem, 'dim_drop_axy.plt', dataname, &
...
end program diffuse_interface6
