...
! variables
  ...
  real(dp) :: cmax, cdiff, mumax, mudiff,  c2max
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ nx, numtimesteps, outputstep, rho, eta, deltat, &
    alpha, beta, Mcoef, kappa, epsdi, itermax, rs_di, is_di
...
! initialize vectors plus a small random perturbation.
  ...
  call fill_sysvector ( mesh, problemdi, soldin, degfd=1, &
       node1=1, node2=mesh%nnodes, func=difunc, funcnr=8 )
...
!     solve (c,mu)
      ...
      c2max = 1.0_dp - maxval(soldi%u(cval%s))
...
end program diffuse_interface7
