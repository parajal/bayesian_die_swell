...
! reinitialize b to b = sqrt(c)
  ...
  subroutine reinitialize_b_inlet
    ...
    real(dp) :: RT_inlet(size(bval_in%s),2,2)
    integer :: i
...
!   check skew norm and perform reinitialization if exceeded
    ...
    if ( any ( skew_norm_2D_b(bb_inlet) >= critval_reinit ) ) then

!     determine the square root of b*b^T

      call sqrtc_2D_b ( bb_inlet, RT=RT_inlet )
...
!     rotate bn-1 according to b
      ...
      call rotate_2D_b ( bm1_inlet, RT_inlet )
...
end program extrudate_swell2d_b
