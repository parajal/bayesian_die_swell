! Viscoelasti extrudate swell problem
! 3D flow for a Giesekus model
! DEVSS-G/SUPG
! Implicit bilinear terms of CE in momentum balance.
! second-order Gear time integration

program extrudate_swell3d_c
...
! definitions
  ...
  type(sysvector_t), dimension(ncompc,nmodes), target :: solc, solc_n, solc_nm1
  ...
  type(sysvector_t), dimension(ncompc,nmodes), target :: solc_p
...
! 2D height function for surface advection
  ...
  type(subscript_t) :: subs_surf1
  type(subscript_t) :: pos1_surf1, pos2_surf1
...
! definitions
  ...
  type(subscriptvec_t) :: cxx13, cxy13, cxz13, cyy13, cyz13, czz13
  type(subscriptvec_t) :: cxx, cxy, cxz, cyy, cyz, czz
...
!   create a vector for conformation for post processing
...
!   move quadratic nodes to cylinder
    do surface = 3,4
...
! create system vectors (solution and right-hand side) for conformation
  ...
  call create ( problemc, solc, solc_n, solc_nm1, solc_p, rhsc )
...
! initialize vectors with zero stress
  ...
  call create_subscript ( mesh, problemc, cxz, degfd=3, vec=3 )
  call create_subscript ( mesh, problemc, cyy, degfd=4, vec=3 )
  call create_subscript ( mesh, problemc, cyz, degfd=5, vec=3 )
  call create_subscript ( mesh, problemc, czz, degfd=6, vec=3 )
...
!    type(mesh_t) ::  mesh1, mesh2, mesh3, mesh4
...
!    write(filename,'(a,I0,a,i4.4,a)') 'c_', ipost, '.vtk'
...
!       call renumber_metis_sysmatrix(sysmatrixc_projc)
!    end if
...
!   problem definition conformation tensor
    ...
    call create_input_probdef ( mesh_inlet, input_probdefc_inlet, nvec=3, nphysq=1 )
...
!   initialize vectors to zero stress
    ...
    do i = 1, ncompc
      call create_subscript ( mesh_inlet, problemc_inlet, cc_in(i), surfaces=[1], &
        physqarr=[1], fillnodes=.true. )
    end do

!   create system matrix for conformation problem

    call create_sysmatrix_structure_base ( sysmatrixc_inlet, mesh_inlet, problemc_inlet )
    call create_sysmatrix_structure_constraint ( sysmatrixc_inlet, mesh_inlet, problemc_inlet )
...
!   build (assemble) matrix and vector for conformation problem
    ...
    if ( coefficients_inlet%i(22) == timeint1 ) then
      ...
      call build_system ( mesh_inlet, problemc_inlet, sysmatrixc_inlet, m2sysvector=rhsc_inlet, &
...
!   solve conformation and keep LU decomposition in the loop over components
    ...
    do icomp = 1, ncompc
      call solve_system_ma41 ( sysmatrixc_inlet, rhsc_inlet(icomp,1), solc_inlet(icomp,1), luc_inlet, &
    ...
    if (step ==1 ) then
      call postprocessing_inlet ( mesh_inlet, sol_inlet, solc_inlet, ipost=ipost_in )
      ipost_in = ipost_in + 1
    end if
...
!   write VTK files
  ...
  subroutine generate_inlet_mesh
    ...
    integer :: i, node, node1, node2, curve, nodenr2, curve2
...
!   make domain into a surface for the flow rate constraint
    ...
    call add_to_mesh ( mesh_inlet, point=[0.0_dp,Ro])
...
!   build vector only (matrix is constant)
    ...
    ! MA57 solver storage
    solver_options_ma57%integer_storage = 2.3
    solver_options_ma57%real_storage    = 2.3
    ...
    do m = 1, nmodes
      do i = 1, ncompc
        ...
        call solve_system_ma57 ( sysmatrixc_proj_inlet, rhsc_proj_inlet(i,m), &
           solc_proj_inlet(i,m), lu_exps_proj_inlet, solver_options=solver_options_ma57 )

      end do
    end do

    call delete ( lu_exps_proj_inlet )

  end subroutine solve_exps_projection_inlet

  subroutine postprocessing_inlet ( mesh_in, sol_in, solc_in, ipost  )

    type(vector_t) :: velocity_in, pressure, ctensor
...
!   create subscript for the height values
  ...
  subroutine create_surface_mesh
    ...
    integer :: i, node, node1, node2, curve, curve2, nodenr2
...
end program extrudate_swell3d_c
