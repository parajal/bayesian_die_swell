...
! variables
 ...
 real(dp), parameter :: &
    ...
    critval_reinit = 0._dp, & ! criterion for reinitializing b
...
!   solve conformation and keep LU decomposition in the loop over components
    ...
    if ( bvariant == 1 ) call reinitialize_b_inlet
...
!   write VTK files
  ...
  subroutine generate_inlet_mesh
    ...
    integer :: i, curve, node, node1, node2, curve2, nodenr2
...
!   create subscript for the height values
  ...
  subroutine create_surface_mesh
    ...
    integer :: curve, node, node1, node2, i, curve2, nodenr2
...
! reinitialize b to b = sqrt(c)
  ...
  subroutine reinitialize_b_inlet

    real(dp) :: bb_inlet(size(bval_in%s),ncompb), bm1_inlet(size(bval_in%s),ncompb)
    real(dp) :: RT_inlet(size(bval_in%s),3,3)
    integer :: i
...
!   check skew norm and perform reinitialization if exceeded
    ...
    if ( any ( skew_norm_3D_b(bb_inlet) >= critval_reinit ) ) then

!     determine the square root of b*b^T

      call sqrtc_3D_b ( bb_inlet, RT=RT_inlet )

!     obtain the solution bn-1 at previous time step

      do i = 1,ncompb
        bm1_inlet(:,i) = solbm1_inlet(i,1)%u(bval_in%s)
      end do

!     rotate bn-1 according to b

      call rotate_3D_b ( bm1_inlet, RT_inlet )

!     store b and bn-1 in solution vectors

      do i = 1,ncompb
        solb_inlet(i,1)%u(bval_in%s) = bb_inlet(:,i)
        solbm1_inlet(i,1)%u(bval_in%s) = bm1_inlet(:,i)
      end do

    end if

  end subroutine reinitialize_b_inlet
...
end program extrudate_swell3d_b
