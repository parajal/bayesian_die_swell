...
!
! This problem is similar to cylinder1, however it shows in addition the
! possibilities to output the determinant of c in three different ways:
!   1) compute detc in the element nodes and average element contributions
!   2) compute detc in the element nodes and store elementwise (no averaging)
!   3) compute detc in the Gauss points
!
...
! definitions
  ...
  type(vector_t) :: ctensor, detc, detc_ew, detc_gauss
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ timeint, eqnumtimesteps, numtimesteps, printtimesteps, &
    nfield, logq, eta_s, eta_p, lambda, deltat, rs_gup, is_gup, U
...
! problem definition Q-vectors
  ...
  call create_input_probdef ( mesh, input_probdefq, nvec=4, nphysq=1 )
  ...
  input_probdefq%vec_elementdof(1)%a(:,4) = [ (0,i=1,8), ndfq ] ! detc Gauss
...
! vector for structure tensor
  ...
  call create_vector ( problemq, detc, vec=2 )
  call create_vector ( problemq, detc_ew, vec=2, elementwise=.true. )
  call create_vector ( problemq, detc_gauss, vec=4 )
...
!     derive detc
       ...
       call derive_vector ( mesh, problemq, detc, &
         elemsub=deriv_determinant_std, &
         coefficients=coefficients, oldvectors=oldvectors_ve )

!     derive detc elementwise

       call derive_vector ( mesh, problemq, detc_ew, &
         elemsub=deriv_determinant_std, &
         coefficients=coefficients, oldvectors=oldvectors_ve )

!     derive detc in Gauss points

       call derive_vector ( mesh, problemq, detc_gauss, &
         elemsub=deriv_determinant_gauss_std, &
         coefficients=coefficients, oldvectors=oldvectors_ve )
...
!     write monitor data
      ...
      write(unit=13,fmt=*) &
        step, step*deltat, maxval(sol%u(vely%s)), maxval(ctensor%u), &
          minval(detc%u), minval(detc_ew%u), minval(detc_gauss%u)
...
! delete all data including all allocated memory
  ...
  call delete ( ctensor, detc, detc_ew, detc_gauss )
  call delete ( vely )
