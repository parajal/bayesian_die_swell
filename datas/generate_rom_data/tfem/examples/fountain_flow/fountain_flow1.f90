...
! P2 shape function for the height function.

program fountain_flow1
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh, problem, vector=pressure, dataname='pressure', &
    filename='fountain_flow1.vtk' )

  call write_vector_vtk ( mesh, problem, filename='fountain_flow1.vtk', &
    dataname='velocity_vector', vector=velocity, append=.true. )

  call write_scalar_vtk ( mesh, problem, vector=gammadot, &
    filename='fountain_flow1.vtk', dataname='gammadot', append=.true. )

  call write_tensor_vtk ( mesh, problem, filename='fountain_flow1.vtk', &
  ...
  call write_scalar_vtk ( mesh, problem_strm, sysvector=sol_strm, &
    dataname='stream_function', filename='fountain_flow1.vtk', append=.true. )
