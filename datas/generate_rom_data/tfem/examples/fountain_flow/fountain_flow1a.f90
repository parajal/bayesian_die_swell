...
! Similar to fountain_flow1, but now with:
!   P1 shape function for the height function.

program fountain_flow1a
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh, problem, vector=pressure, dataname='pressure', &
    filename='fountain_flow1a.vtk' )

  call write_vector_vtk ( mesh, problem, filename='fountain_flow1a.vtk', &
    dataname='velocity_vector', vector=velocity, append=.true. )

  call write_scalar_vtk ( mesh, problem, vector=gammadot, &
    filename='fountain_flow1a.vtk', dataname='gammadot', append=.true. )

  call write_tensor_vtk ( mesh, problem, filename='fountain_flow1a.vtk', &
  ...
  call write_scalar_vtk ( mesh, problem_strm, sysvector=sol_strm, &
    dataname='stream_function', filename='fountain_flow1a.vtk', append=.true. )
...
!   predictor for surface height
    ...
    disp(:,1) = height_sf_adv%u - mesh%coor(mesh%curves(14)%nodes,1)
    disp(:,2:ndisp) = 0
