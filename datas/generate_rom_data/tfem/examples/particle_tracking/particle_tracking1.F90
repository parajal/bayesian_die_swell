...
! definitions
  ...
  integer :: i, j, k, nod, ip, periodnr, step, img, nbx, nby
...
! post-processing
  ...
  write(filename,'(a,i3.3,a)') 'points', img, '.fig'
  ...
  do periodnr = 1, nperiod
    ...
    do step = 1, nsubsteps
      ...
      if ( mod(img,everystep) == 0 ) then
        ...
        write(filename,'(a,i3.3,a)') 'points', img/everystep, '.fig'
...
! open binary file for reading by streamfunction
...
! write to a fig file for plotting
  ...
  call extract_physvector ( mesh, problem, sol1, velocity )
  call plot_vector ( plot_options, mesh, problem, 'velocity1.fig', &
    vector=velocity )

  write(10) velocity%u

  call extract_physvector ( mesh, problem, sol2, velocity )
  call plot_vector ( plot_options, mesh, problem, 'velocity2.fig', &
    vector=velocity )
...
end program particle_tracking1
