...
! variables
  ...
  real(dp) :: &
    ...
    bpar     = 0.1_dp, & ! b parameter in the FENE-P model
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ ly, numtimesteps, logc, &
    model, eta_s, eta_p, lambda, bpar, deltat, U, &
...
! set some parameters
  ...
  G = eta_p / lambda * ( bpar + 3 ) / bpar ! modulus nkT

  flowrate = U * ly

  npar = 3 ! FENE-P number of model parameters
...
! fill coefficients
  ...
  coefficients%r(1:503) = &
    [ eta_s,    0._dp,   0._dp,   alpha, 0._dp, &
      flowrate, 0._dp,  deltat,   0._dp, 0._dp, &
      ( 0._dp, i = 11, 500 ), &
      G, lambda, bpar  &
    ]
...
! problem definition of gradient/velocity/pressure
  ...
  input_probdef%vec_elementdof(1)%a(vertices,1) = 3  ! gradients
  input_probdef%vec_elementdof(1)%a(:,2) = 2  ! velocity
  input_probdef%vec_elementdof(1)%a(:,3) = 0
  input_probdef%vec_elementdof(1)%a(9,3) = 3  ! pressure
...
! problem definition conformation tensor
  ...
  input_probdefc%vec_elementdof(1)%a(9,1) = 4  ! c
...
! initialize vectors to zero stress
  ...
  call create_sysvector ( problemc, cdot )
...
!   solve conformation and keep LU decomposition in the loop over components
    ...
    do icomp = 1, ncompc
      call solve_system_ma41 ( sysmatrixc, rhsc(icomp,1), cdot, luc, &
...
!   write monitor data
    ...
    write(unit=13,fmt=*) &
      step, step*deltat, maxval(solc(1,1)%u), &
