...
! XPP model
! NOTE: this is similar to sphere22, but now using the XPP model
...
! variables
  ...
  real(dp) :: &
    ...
    lambda_s = 0.5_dp, & ! relaxation time stretch (XPP model)
...
! namelist for input of variables; read from standard input
  ...
  namelist /comppar/ timeint1, timeint2, numtimesteps, logc, eta_s, eta_p, &
    lambda, lambda_s, nu, deltat, beta, rs_gup, is_gup, rs_c, is_c, U, restart
...
! fill coefficients
  ...
  coefficients%i(79) = 2  ! c_direct (1: no 2: yes)

  coefficients%r = &
    [ eta_s, 0._dp,   0._dp,   alpha, 0._dp, &
      0._dp, 0._dp,  deltat,   beta,  0._dp, &
      ( 0._dp, i = 11, 500 ), &
      G, lambda, lambda_s, nu &
    ]
...
!   build (assemble) matrix and vector for conformation problem
    ...
    if ( coefficients%i(22) == timeint1 ) then
      ...
      call build_system ( mesh, problemc, sysmatrixc, m2sysvector=rhsc, &
        elemsub=ce_supg_elem, oldvectors=oldvectors_ve, coefficients=coefficients )
