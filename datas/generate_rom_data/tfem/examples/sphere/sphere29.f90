...
! Equal order high-order quadrilateral elements.
! This problem is similar to sphere1, but now with high-order elements having
! equal-order continuous stabilized velocity/pressure Qp/Qp elements
!
...
! read mesh
  ...
  call read_mesh_gmsh ( mesh, filename='mesh29.msh', ndim=2 )
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh_plot, problem, vector=pressure, &
    dataname='pressure', filename='sphere29.vtk' )

  call write_vector_vtk ( mesh_plot, problem, filename='sphere29.vtk', &
    dataname='velocity_vector', vector=velocity, append=.true. )

  call write_scalar_vtk ( mesh_plot, problem, vector=vorticity, &
    dataname='vorticity', filename='sphere29.vtk', append=.true. )

  call write_scalar_vtk ( mesh_plot, problem, vector=divergence, &
    dataname='divergence', filename='sphere29.vtk', append=.true. )
