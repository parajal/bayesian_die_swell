...
! set parameters for iterative solver
  ...
  call set_solver_options ( solver_options, printlevel=2, type_prec=2, &
    ...
    droptol=1e-7_dp, fillin=4._dp, eps_rel = 1e-7_dp, eps_abs = 1.e-3_dp )
...
!   solve gradient/velocity/pressure problem
    ...
    call solve_system_sk ( sysmatrix, rhsd, sol, initsol=.true., &
...
!   solve conformation and keep LU decomposition in the loop over components
    ...
    call delete ( iluc )  ! remove LU decomposition and rebuild next time step
