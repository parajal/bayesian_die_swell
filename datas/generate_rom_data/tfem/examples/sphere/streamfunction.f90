! Axisymmetrical streamfunction problem for the sphere in a cylinder with
! Non-zero Neumann boundary conditions only necessary on the cylinder wall.

! Dirichlet in a single point (P3) psi=0
...
! curve for the wall of the tube

  call add_to_mesh ( mesh, curve=[10,11,12,13], newnr=crv )
...
! problem definition stream function
  ...
  call define_essential ( mesh, input_probdef, point=3 ) ! Dirichlet in P3
...
! build (assemble) matrix and vector from elements
  ...
  call add_boundary_elements ( mesh, problem, rhsd, curve=crv, &
  ...
  call plot_color_contour ( plot_options, mesh, problem, &
    'streamfunction_zoom.fig', sysvector=sol )

  call set_plot_options ( plot_options, xmin=-2._dp, xmax=0.1_dp, ymax=1._dp, &
    maxvalue=2.85e-4_dp, minvalue=0._dp, numlevels=9, fontsize=10 )

  call plot_color_contour ( plot_options, mesh, problem, &
    'streamfunction_zoom1.fig', sysvector=sol )
