...
! on the center line of a cylindrical container with a bottom
! (at infinity) due to an applied force F.
! At the ends fluid is not moving, representing a bottom.
...
! constants
  ...
  real(dp), parameter :: &
    ...
    F = 647.45021916228234_dp ! force applied on the sphere
...
! pressure level
  ...
  call define_constraint ( mesh, input_probdef, curve1=crvpart, &
...
!     Newton-Raphson
      call get_sysvector_constraint ( mesh, problem, sol, constraint=1, &
        addunknowns=.true., u=upi )
      up = up + upi ! update velocity of particle
...
! write to vtk
  ...
  call write_vector_vtk ( mesh, problem, vector=velocity, &
    dataname='velocity', filename='sphere35.vtk' )

  call write_scalar_vtk ( mesh, problem, vector=pressure, &
    dataname='pressure', filename='sphere35.vtk', append=.true. )

  call write_scalar_vtk ( mesh, problem, vector=vorticity, &
    dataname='vorticity', filename='sphere35.vtk', append=.true. )

  call write_scalar_vtk ( mesh, problem, vector=gammadot, &
    dataname='gammadot', filename='sphere35.vtk', append=.true. )

  call write_scalar_vtk ( mesh, problem, vector=viscosity, &
    dataname='viscosity', filename='sphere35.vtk', append=.true. )

  call write_scalar_vtk ( mesh, problem, vector=vmstress, &
    dataname='vmstress', filename='sphere35.vtk', append=.true. )
...
! constraint matrix for a particle with force (collocation)
