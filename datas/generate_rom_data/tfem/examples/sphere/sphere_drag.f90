...
! curve for drag on the sphere
  ...
  call add_to_mesh ( mesh, curve=[3,4,5,6], newnr=crv )
...
! viscous stress
  ...
  call derive_vector ( mesh, problem, stress_tensor, &
    ...
    oldvectors=oldvectors_d, elcurves=[crv] ) ! only include curve crv
...
! integrate drag force
  ...
  call integrate_boundary_elements ( mesh, problem, dragv, &
    elemsub=stokes_drag, curve=crv, coefficients=coefficients, &
...
! conformation tensor
  ...
  call derive_vector ( mesh, problemc, conformation_tensor(1), &
    ...
    oldvectors=oldvectors_dve, elcurves=[crv] ) ! only include curve crv
...
! integrate drag force
  ...
  call integrate_boundary_elements ( mesh, problemc, dragve, &
    elemsub=viscoelastic_drag, curve=crv, coefficients=coefficients, &
