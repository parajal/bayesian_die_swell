...
! DEVSS-G/SUPG
! Explicit bilinear terms of CE in momentum balance.
...
! constants
  ...
  integer, parameter :: &
    ...
    ncompv_P2 = 8,    & ! number of components for P2 projection:
                        ! x,y-coordinates and x,y-velocities at n and n-1
    ncompv_P1 = 14,   & ! number of components for P1 projection:
                        ! conformation components and gradients at n and n-1
...
! variables
  ...
  integer :: &
    ...
    timeint2 = 5,       & ! second-order, semi-implicit Gear/Karniadakis
...
!   copy old value (already here, so solc_np1 is used in the upG problem!)
...
!   copy old values
!   note: solc is already copied in the time integration loop)
...
!   create oldvectors
    ...
    oldvectors_ve%v(2)%p => meshvel_np1

!   Note: historically, ALE in TFEM was velocity based instead of
!   displacement based as is used in this example. Therefore, the mesh velocity
!   at n+1 was not known, and the convection velocity was predicted by:
!   uhat_n+1 = 2 * (u_n - u_mesh_n) - (u_n_m1 - u_mesh_m1).
!   In the displacement based ALE, the mesh velocity at n+1 can be determined
!   using a BDF scheme and is known at n+1, so the convection velocity can
!   be predicted by uhat_n+1 = ( 2 * u_n - u_n_m1 ) - u_mesh_p1.
!   The implementation in TFEM, is based on the velocity based ALE. However, it
!   is possible to use the known mesh velocity at n+1 by pointing both v(1) and
!   v(2) in oldvectors_ve to meshvel_np1 instead of pointing v(1) to meshvel_n
!   and v(2) to meshvel_nm1.
...
!   build (assemble) vector for gradient/velocity/pressure problem
    if ( step >= 1 ) then
      call build_system ( mesh_np1, problem, sysmatrix, rhsd, &
        elemsub=rhs_divtau, oldvectors=oldvectors_ve, physqrow=[2], &
        physqcol=[2], buildmatrix=.false., coefficients=coefficients, &
        addmatvec=.true. )
    end if
...
!   solve vpG system
    ...
    call solve_system_ma57 ( sysmatrix, rhsd, sol_np1, &
...
!   transfer the velocities at n
    call transfer_data ( mesh_np1_temp, problem_temp, &
      sysvector1=sol_n_temp, vector2=vec_p, physq1=[2], degfd1=[1,2], &
      degfd2=[3,4] )

!   transfer the coordinates at n-1
    call transfer_data ( mesh_np1_temp, problem_temp, &
      vector1=coords_nm1_temp, vector2=vec_p, degfd2=[5,6] )

!   transfer the velocities at n-1
    call transfer_data ( mesh_np1_temp, problem_temp, &
      sysvector1=sol_nm1_temp, vector2=vec_p, physq1=[2], degfd1=[1,2], &
      degfd2=[7,8] )
...
! P1 project the conformation components and velocity gradients on the new mesh
  ...
  subroutine project_old_solutions_P1
    ...
    type(subscript_t) :: G_upg1, G_upg2, G_upg3, G_upg4
    type(subscript_t) :: cc
...
!   transfer the gradients at n
!   note the different problems for problem1 and problem2!!!!
    call transfer_data ( mesh_np1_temp, problem_temp, problemc_temp, &
      sysvector1=sol_n_temp, vector2=vec_p, physq1=[1], degfd1=[1,2,3,4], &
      degfd2=[7,8,9,10] )

!   transfer the gradients at n-1
    call transfer_data ( mesh_np1_temp, problem_temp, problemc_temp, &
      sysvector1=sol_nm1_temp, vector2=vec_p, physq1=[1], degfd1=[1,2,3,4], &
      degfd2=[11,12,13,14] )
...
!   update gradients
    call create_subscript ( mesh_np1, problem, G_upg1, physqarr=[1], degfd=1 )
    call create_subscript ( mesh_np1, problem, G_upg2, physqarr=[1], degfd=2 )
    call create_subscript ( mesh_np1, problem, G_upg3, physqarr=[1], degfd=3 )
    call create_subscript ( mesh_np1, problem, G_upg4, physqarr=[1], degfd=4 )
    sol_n%u(G_upg1%s) = sol_proj(7)%u
    sol_n%u(G_upg2%s) = sol_proj(8)%u
    sol_n%u(G_upg3%s) = sol_proj(9)%u
    sol_n%u(G_upg4%s) = sol_proj(10)%u
    sol_nm1%u(G_upg1%s) = sol_proj(11)%u
    sol_nm1%u(G_upg2%s) = sol_proj(12)%u
    sol_nm1%u(G_upg3%s) = sol_proj(13)%u
    sol_nm1%u(G_upg4%s) = sol_proj(14)%u
    call delete ( G_upg1, G_upg2, G_upg3, G_upg4 )
