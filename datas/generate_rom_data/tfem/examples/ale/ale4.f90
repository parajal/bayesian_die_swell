...
! Particle updater uses a predictor/corrector scheme
...
! definitions
  ...
  real(dp) :: pp(3), ppn(3), ppnm1(3), pp_pred(3), pp_predn(3)
...
! start time integration
  ...
  integration: do step = 1, numtimesteps
    ...
    if ( step == 1 ) then
      ...
      ppn = pp
      pp_pred = pp + deltat*up  ! In the current problem up=0 at t=0
...
!     update mesh nodes: solve a Laplace's equation
      ...
      call update_mesh_nodes ( mesh, problem_pois, pp_pred(1:2)-pp_predn(1:2) )
...
!   update particle position (corrector)

    pp = ppn + ( up + un ) * deltat / 2
