...
! Second-order Gear with Newton iteration for inertia terms.
! DEVSS-G/SUPG
! Semi-implicit Gear with velocity predicition for the CE.
...
! constants
  ...
  integer, parameter :: &
    ...
    timeint1 = 1,           & ! first-order implicit Euler time integration,
                              ! first time step
    timeint2 = 5,           & ! second-order implicit Gear time integration
    ...
    maxnumiterations = 100, & ! maximum number of iterations allowed to obtain
                              ! convergence
    num_picard = 1            ! number of Picard iterations, followed by
                              ! Newton iterations
...
! definitions
  ...
  type(sysvector_t), target :: sol_iter, soln, sol_hat, solnm1
  ...
  integer :: i, m, icomp, step, iterstep, ipost=0
...
! read P2 and P1 gmsh mesh
  call read_mesh_gmsh ( mesh, filename='4x1_yp-0.125_M1_P2.msh', ndim=2, &
    physgeom=.true. )
  call read_mesh_gmsh ( mesh1, filename='4x1_yp-0.125_M1_P1.msh', ndim=2, &
    physgeom=.true. )
...
! create subscript for the mesh velocity vector
  call create_subscript_vector ( mesh, problem, meshvelx, vec=physqvel, degfd=1 )
  call create_subscript_vector ( mesh, problem, meshvely, vec=physqvel, degfd=2 )
...
!     update mesh nodes: solve a Laplace's equation (vertically only)
    ...
    xp(2) = pp(2)  ! xp is used in elementc
...
!   build (assemble) matrix and vector for conformation problem
    call build_system ( mesh, problemc, sysmatrixc, m2sysvector=rhsc, &
      elemsub=ce_supg_elem, oldvectors=oldvectors, coefficients=coefficients )
