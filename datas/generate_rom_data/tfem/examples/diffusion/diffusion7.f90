...
! conditions. There is a circular region in the domain, where the
...
! objects for plotting the nodesets
  ...
  call plot_points_curves ( plot_options, mesh, 'curves_df7.fig' )

  if ( plot_nodesets ) then

    call plot_mesh_nodeset ( nodeset=1, filename='mesh_outside_df7.fig' )
    call plot_mesh_nodeset ( nodeset=2, filename='mesh_inside_df7.fig' )

  else

    call plot_mesh ( plot_options, mesh, 'mesh_df7.fig' )

    plot_options%objectpointcolor=4
    plot_options%objectpointsize=0.4
    call plot_objects ( plot_options, mesh, 'mesh_df7.fig', append=.true., &
      object1=1 )
...
! sample scalar on cylinder
  ...
  call sample_scalar_on_cylinder ( layer=1, filename='sample_c_out_df7.out' )
  call sample_scalar_on_cylinder ( layer=2, filename='sample_c_in_df7.out' )

! sample gradient on cylinder

  call sample_gradient_on_cylinder ( layer=1, filename='sample_gc_out_df7.out' )
  call sample_gradient_on_cylinder ( layer=2, filename='sample_gc_in_df7.out' )
...
! post-processing: create mesh_plot for plotting
  ...
  call plot_mesh ( plot_options, mesh_plot, 'mesh_plot_df7.fig' )
...
! sample and plot scalar on mesh_plot
  ...
  call sample_scalar
  call plot_contour_on_mesh_plot ( filename='scalar_df7.fig', vector=scalar)
...
! write to a vtk file for further post-processing
  ...
  call write_scalar_vtk ( mesh_plot, problem_plot, vector=scalar, &
    dataname='scalar', filename='diffusion7.vtk' )

  call write_vector_vtk ( mesh_plot, problem_plot, &
    filename='diffusion7.vtk', &
