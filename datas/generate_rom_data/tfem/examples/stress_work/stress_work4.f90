...
!   write monitor data
    ...
    write(unit=13,fmt=*) &
      ...
      minval(solc(2,1)%u(cval%s)), maxval(solc(3,1)%u(cval%s)), &
      minval(solc(4,1)%u(cval%s)), maxval(solc(5,1)%u(cval%s)), &
      minval(solc(6,1)%u(cval%s)), &
...
! create exact solution for velocity
  ...
  call fill_sysvector ( mesh, problem, sol_exact, &
    node1=1, node2=mesh%nnodes, physq=physqv, degfd=1, func=func, funcnr=5 )
  call fill_sysvector ( mesh, problem, sol_exact, &
    node1=1, node2=mesh%nnodes, physq=physqv, degfd=3, func=func, funcnr=6 )
...
! print maximum difference of sol-solexact to standard output
  ...
  print *, 'max u_t error', maxval( abs(sol%u(svel3%s)-sol_exact%u(svel3%s)) )

  if ( abs(Uavg) <= 1e-10 ) then

!   create exact solution for temperature (only for pure swirl flow).

    call create_sysvector ( problemt, solt_exact )

    call fill_sysvector ( mesh, problemt, solt_exact, &
      node1=1, node2=mesh%nnodes, physq=1, func=func, funcnr=7 )
...
! write vtk
  ...
  call write_vector_vtk ( mesh, problem, filename='sol.vtk', &
    dataname='velocity', sysvector=sol, physq=physqv, assume3D=.true. )
